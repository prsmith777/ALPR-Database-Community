        ...
        {detector.map((camera) => (
          <Card key={`detector:${camera.cameraName}`}>
            <CardContent className="flex items-start justify-between gap-4 p-4">
              ...
              <Button asChild variant="outline" size="sm"><Link href="/visual_search">Review legacy camera index</Link></Button>
            </CardContent>
          </Card>
        ))}
      </div>
  ...
  const reviewAssociation = async (clusterId, plateNumber, decision) => {
    ...
    try {
      ...
      setMessage(decision === "confirm"
        ...
        : `${plateNumber} was rejected for Legacy Vehicle #${clusterId}.`);
    } catch (error) { setMessage(error.message); }
    finally { setBusy(""); }
  };
    ...
    <div className="space-y-8">
      <div className="rounded-lg border border-amber-500/40 bg-amber-500/10 p-4 text-sm">
        <div className="font-semibold">
          {view === "profiles" ? "Legacy Profiles (ReID v1)" : "Legacy Needs Review (ReID v1)"}
        </div>
        <p className="mt-1 text-muted-foreground">
          This page shows legacy read-owned plate-image clusters and assignments. Legacy Vehicle numbers are not ReID v2 profile IDs.
        </p>
      </div>
      {view === "profiles" && <Card className="border-blue-500/30 bg-blue-500/5">
        <CardHeader>
          <CardTitle className="flex items-center gap-2"><Layers3 className="h-5 w-5" /> Legacy ReID v1 identity foundation</CardTitle>
          <CardDescription>
            Legacy ReID v1 groups plate-camera captures without using plate text. Outstanding v1 decisions are organized in Legacy Needs Review.
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
          <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
            <div className="rounded-md border bg-background p-3"><div className="text-2xl font-semibold">{data.stats.totalClusters}</div><div className="text-xs text-muted-foreground">legacy profiles</div></div>
      ...
      {view === "review" && <section className="space-y-4">
        <div><h2 className="text-2xl font-semibold">Legacy Needs Review</h2><p className="text-sm text-muted-foreground">Choose one legacy v1 queue. Counts cover the full database and each queue keeps its own page.</p></div>
        <div className="flex flex-wrap gap-2 rounded-lg border bg-card p-3">
          <Button asChild variant={activeQueue === "vehicle" ? "default" : "outline"}>
            <Link href="/visual_search/vehicles/review?queue=vehicle">Legacy Vehicle Matches <Badge variant="secondary" className="ml-2">{data.stats.pendingReviews}</Badge></Link>
          </Button>
          <Button asChild variant={activeQueue === "plates" ? "default" : "outline"}>
            <Link href="/visual_search/vehicles/review?queue=plates">Legacy Plate Associations <Badge variant="secondary" className="ml-2">{data.stats.pendingPlateAssociations}</Badge></Link>
          </Button>
          <Button asChild variant={activeQueue === "direction" ? "default" : "outline"}>
            <Link href="/visual_search/vehicles/review?queue=direction">Legacy Direction Fallback <Badge variant="secondary" className="ml-2">{data.stats.pendingDirectionReviews}</Badge></Link>
          </Button>
          {data.canManageSettings && <Button asChild variant={activeQueue === "setup" ? "default" : "outline"}>
            <Link href="/visual_search/vehicles/review?queue=setup">Legacy Setup Attention</Link>
          </Button>}
        </div>

        {activeQueue === "vehicle" && <Card>
          <CardHeader><CardTitle className="text-lg">Legacy vehicle matches</CardTitle><CardDescription>Confirm a suggested v1 grouping or separate the capture into its own legacy profile.</CardDescription></CardHeader>
          <CardContent className="space-y-4">
              ...
              <div className="grid gap-4 xl:grid-cols-2">
                {data.suggestions.map((suggestion) => (
                  <Card key={suggestion.readId}>
                    <CardHeader className="pb-3"><div className="flex items-center justify-between gap-3"><CardTitle className="text-base">Suggested Legacy Vehicle #{suggestion.clusterId}</CardTitle><Badge variant="outline">{percent(suggestion.similarity)} v1 ReID</Badge></div></CardHeader>
                    <CardContent className="space-y-4">
                      ...
                      {data.canReview && <div className="grid grid-cols-2 gap-2">
                        <Button disabled={Boolean(busy)} onClick={() => reviewCluster(suggestion.readId, "confirm")}>
                          {busy === `cluster:${suggestion.readId}:confirm` ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : <CheckCircle2 className="mr-2 h-4 w-4" />} Confirm legacy vehicle
                        </Button>
                        <Button variant="outline" disabled={Boolean(busy)} onClick={() => reviewCluster(suggestion.readId, "separate")}>
                          {busy === `cluster:${suggestion.readId}:separate` ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : <Split className="mr-2 h-4 w-4" />} Different legacy vehicle
                        </Button>
                      </div>}
                    </CardContent>
                  </Card>
        ...
        {activeQueue === "plates" && <Card>
          <CardHeader><CardTitle className="text-lg">Legacy plate associations</CardTitle><CardDescription>Review every proposed effective-plate link to a legacy v1 profile without searching through profiles.</CardDescription></CardHeader>
          <CardContent className="space-y-4">
              ...
              <div className="grid gap-4 xl:grid-cols-2">
                {data.plateAssociationReviews.map((association) => (
                  <Card key={association.id}>
                    <CardHeader className="pb-3"><div className="flex items-start justify-between gap-3"><div><CardTitle className="text-base">Legacy Vehicle #{association.clusterId} · <span className="font-mono">{association.plateNumber}</span></CardTitle><CardDescription>{association.evidenceCount} confirmed capture{association.evidenceCount === 1 ? "" : "s"} · {percent(association.confidence)} mean v1 ReID</CardDescription></div><Badge variant="outline">Suggested</Badge></div></CardHeader>
                    <CardContent className="space-y-4">
                      <div className="grid grid-cols-2 gap-3">
        ...
        {activeQueue === "direction" && <Card>
          <CardHeader><CardTitle className="text-lg">Legacy direction-fallback reviews</CardTitle><CardDescription>Unlabeled, unknown v1 plate-image captures are balanced across configured cameras. A review immediately becomes legacy fallback calibration evidence.</CardDescription></CardHeader>
          <CardContent className="space-y-4">
        ...
        {activeQueue === "setup" && data.canManageSettings && <div className="space-y-4">
          ...
          {data.calibration && !data.calibration.recommendation && <Card>
            <CardHeader><CardTitle className="flex items-center gap-2 text-lg"><Gauge className="h-5 w-5" />Optional legacy ReID v1 calibration</CardTitle><CardDescription>Same/different v1 labels improve local threshold analysis but are not a pending identity decision.</CardDescription></CardHeader>
            <CardContent className="flex flex-wrap items-center justify-between gap-4">
              ...
              <Button asChild variant="outline"><Link href="/visual_search">Open Legacy Visual Search</Link></Button>
            </CardContent>
          </Card>}
        </div>}
      </section>}
      ...
      {view === "profiles" && <section className="space-y-4">
        <div><h2 className="text-xl font-semibold">Legacy Profiles</h2><p className="text-sm text-muted-foreground">Legacy ReID v1 profiles are paginated independently from all review queues.</p></div>
        <Card>
        ...
        {data.clusters.length === 0 ? <EmptyReview>No legacy profiles match these filters.</EmptyReview> : (
          <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3">
            {data.clusters.map((cluster) => (
              <Card key={cluster.id} className="overflow-hidden">
                ...
                <CardContent className="space-y-3 p-4">
                  <div className="flex items-center justify-between"><div className="font-semibold">Legacy Vehicle #{cluster.id}</div><Badge variant="secondary">{cluster.status}</Badge></div>
                  ...
                  <Button asChild variant="outline" className="w-full"><Link href={`/visual_search/vehicles/${cluster.id}`}>Open legacy profile <ArrowRight className="ml-2 h-4 w-4" /></Link></Button>
                </CardContent>
              </Card>
            ))}
          </div>
