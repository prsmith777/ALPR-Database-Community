...
function overviewFrom(result) {
  if (!result?.success || !result.data?.overview) {
    throw new Error(result?.error || "Unable to load canonical crop embeddings.");
  }
...
export default function VehicleAssetEmbeddingPanel({ initialOverview = null }) {
  ...
  const embeddingFinished = count(counts.ready) + count(counts.alreadyCurrent)
    + count(counts.sourceChanged) + count(counts.invalid) + count(counts.failed);
  const finished = status === "previewing" ? previewFinished : embeddingFinished;
  ...
  const canEmbed = ["ready", "running"].includes(status)
  ...
  useEffect(() => {
    ...
    const timer = window.setInterval(async () => {
      try {
        const next = overviewFrom(await getVehicleAssetEmbeddingOverview());
        ...
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div>
            <CardTitle className="flex items-center gap-2">
              <BrainCircuit className="h-5 w-5" /> Canonical crop embeddings
            </CardTitle>
            <CardDescription className="mt-1 max-w-3xl">
              Encode each current identity-eligible canonical vehicle crop once with the local vehicle ReID model.
            </CardDescription>
          </div>
      ...
      <CardContent className="space-y-5">
        <div className="rounded-md border border-blue-500/30 bg-blue-500/5 p-3 text-sm">
          ...
          <p className="mt-1 text-xs text-muted-foreground">
            This creates immutable 512-value vectors in PostgreSQL. It does not replace current ReID, alter vehicle assignments, call Plate Recognizer, or send data outside ALPR.
          </p>
        </div>

        <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
          <Metric label="eligible current crops" value={count(catalog.eligibleCrops).toLocaleString()} />
          <Metric label="stored embeddings" value={count(catalog.embeddingCount).toLocaleString()} />
          <Metric label="embedding storage" value={formatBytes(catalog.embeddingBytes)} />
          <Metric label="values per embedding" value="512" />
        </div>
        ...
        {run ? (
          <div className="space-y-4 rounded-md border p-4">
            <div className="flex flex-wrap items-start justify-between gap-3">
              <div>
                <div className="font-medium">Embedding campaign #{run.id}</div>
                <div className="text-xs text-muted-foreground">
                  Frozen through crop derivative #{count(run.maxDerivativeId).toLocaleString()} · {run.modelName}
                </div>
              </div>
            ...
            <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-6">
              ...
              <Metric label="embedded" value={(count(counts.ready) + count(counts.alreadyCurrent)).toLocaleString()} />
            ...
            <div className="space-y-2">
              ...
              <Progress value={progress} aria-label={`${progress}% of canonical crop embedding campaign complete`} />
            </div>
            <div className="flex flex-wrap items-end gap-3">
              ...
              <Button
                disabled={Boolean(busy) || !canEmbed}
                onClick={() => perform("confirm", () => confirmVehicleAssetEmbeddingBatch({
                  runId: run.id,
                  previewFingerprint: run.previewFingerprint,
                  limit: Number(batchSize),
                }), (result) => `Queued ${count(result.data?.confirmation?.queued).toLocaleString()} crop embedding(s).`)}
              >
                {busy === "confirm" ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : <Play className="mr-2 h-4 w-4" />}
                Embed next batch ({Math.min(Number(batchSize), count(counts.previewed))})
              </Button>
              <Button
                variant="secondary"
                disabled={Boolean(busy) || !canPause}
                onClick={() => perform("pause", () => setVehicleAssetEmbeddingPaused({
                  runId: run.id, paused: status !== "paused",
                }), status === "paused" ? "Crop embedding resumed." : "Crop embedding paused.")}
              >
              ...
              <Button
                ...
                onClick={() => perform("cancel", () => cancelVehicleAssetEmbeddingCampaign({ runId: run.id }),
                  "Remaining crop embedding work was cancelled; completed embeddings were preserved.")}
              >
        ...
        <div className="flex flex-wrap gap-3">
          <Button
            ...
            onClick={() => perform("preview", previewVehicleAssetEmbeddings,
              "Embedding preview started. The local model runs, but no embedding rows are stored until a batch is confirmed.")}
          >
            {busy === "preview" ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : <BrainCircuit className="mr-2 h-4 w-4" />}
            {run ? "Create delta embedding preview" : "Create embedding preview"}
          </Button>
          <Button
            variant="outline"
            disabled={Boolean(busy)}
            onClick={() => perform("refresh", getVehicleAssetEmbeddingOverview, "Embedding status refreshed.")}
          >
                ...
                <div key={item.jobId} className="flex flex-wrap items-center justify-between gap-2 rounded-md border p-3 text-sm">
                  ...
                  <Button
                    ...
                    onClick={() => perform(`retry:${item.jobId}`,
                      () => retryVehicleAssetEmbeddingJob({ jobId: item.jobId }),
                      "The crop embedding was queued for its one bounded operator retry.")}
                  >
