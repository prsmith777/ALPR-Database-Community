...
test("canonical crop embedding compatibility remains without private processing controls", async () => {
  const [instrumentation, actions, settings, loader] = await Promise.all([
  ...
  assert.doesNotMatch(settings, /VehicleAssetEmbeddingPanel/);
  assert.doesNotMatch(loader, /getVehicleAssetEmbeddingOverview/);
