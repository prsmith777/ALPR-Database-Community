...
test("Downloads page and API expose authenticated filter-respecting exports", async () => {
  ...
  assert.match(filters, /id="plate-database-page-size"/);
  assert.ok(
    filters.indexOf('id="plate-database-page-size"') <
      filters.indexOf('id="plate-database-search-options"'),
    "database page size belongs beside the Search options header"
  );
  assert.match(filters, /\{hasActiveFilters && \(/);
  assert.match(form, /aria-controls="plate-export-search-options"/);
  assert.match(form, /LiveFeedDateRangeFilter/);
  assert.match(form, /FilterHourRange/);
  assert.match(form, /\{hasActiveFilters && \(/);
  assert.match(form, /Up to[\s\S]*?50,000[\s\S]*?rows/);
});
