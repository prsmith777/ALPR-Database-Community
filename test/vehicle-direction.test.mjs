...
test("direction schema and Community administrator setup are durable and camera driven", async () => {
  ...
  assert.match(settings, /Front-view direction label/);
  assert.match(settings, /Rear-view direction label/);
  assert.match(settings, /Minimum confidence/);
  assert.match(settings, /no fixed camera names/i);
  ...
  assert.doesNotMatch(settings, /Street LPR|Gate LPR/);
  ...
  assert.doesNotMatch(settings, /Historical direction backfill|re-evaluation/i);
