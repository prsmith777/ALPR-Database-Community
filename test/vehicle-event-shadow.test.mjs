...
test("two distinct canonical images require matching direction and capture timing", () => {
  const anchor = observation({ overview_context: "entry", read_camera_name: "Gate LPR 1" });
  const companion = observation({
    read_id: 202,
    asset_id: 902,
    overview_context: "entry",
    read_camera_name: "Gate LPR 2",
...
test("shadow compatibility remains without private processing controls", async () => {
  const [loader, settings, actions] = await Promise.all([
  ...
  assert.doesNotMatch(loader, /getVehicleEventShadowOverview/);
  assert.doesNotMatch(settings, /VehicleEventShadowPanel/);
  for (const action of [
