...
test("plate conditions reuse the shared exact and fuzzy matching profiles", () => {
  const exact = evaluateNotificationCondition(
    condition("plate_match", "matches", { plate: "TSTO01", mode: "off" }),
    { event: acceptedRead }
  );
  const strict = evaluateNotificationCondition(
    condition("plate_match", "matches", { plate: "TSTO01", mode: "strict" }),
    { event: acceptedRead }
  );
...
test("plate conditions support contains, wildcard, OCR-only, and bounded edit-distance strategies", () => {
  const contains = evaluateNotificationCondition(
    condition("plate_match", "matches", { plate: "ST0", strategy: "contains" }),
    { event: acceptedRead }
  );
  const wildcard = evaluateNotificationCondition(
    condition("plate_match", "matches", { plate: "TS?0*", strategy: "wildcard" }),
    { event: acceptedRead }
  );
  const ocr = evaluateNotificationCondition(
    condition("plate_match", "matches", { plate: "TSTO01", strategy: "ocr_confusion" }),
    { event: acceptedRead }
  );
  const edited = evaluateNotificationCondition(
    condition("plate_match", "matches", { plate: "TST002", strategy: "edit_distance", maximumDistance: 1 }),
    { event: acceptedRead }
  );
...
test("known-name conditions preserve legacy MQTT name matching", () => {
  const matching = evaluateNotificationCondition(
    condition("known_name", "equals", { names: ["Resident's Sedan"] }),
    { event: { ...acceptedRead, knownName: "Resident's Sedan" } }
  );
  const mismatch = evaluateNotificationCondition(
    condition("known_name", "in", { names: ["Delivery Van"] }),
    { event: { ...acceptedRead, known_name: "Resident's Sedan" } }
  );
