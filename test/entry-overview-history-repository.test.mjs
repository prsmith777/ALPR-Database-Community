...
test("Entry history migration is preview-first, bounded, restartable, and adds no automatic queue update", async () => {
  ...
  const section = migration.slice(migration.indexOf("-- Direction-independent Gate Overview history"));
  ...
  assert.match(section, /source_camera_name[^]*Gate Overview/);
  assert.match(section, /source_camera_short_name[^]*Overview01/);
  ...
  assert.match(section, /Disabled Gate Overview history profile snapshots cannot be revived/);
...
test("Entry history profile persistence fixes Overview01 identity and versions timing changes", async () => {
  let current = null;
  ...
  assert.equal(first.source_camera_name, "Gate Overview");
  assert.equal(first.source_camera_short_name, "Overview01");
...
test("Entry history claim is oldest-first and globally yields to live work", async () => {
  ...
  assert.equal(claimed.overview_source_camera_short_name, "Overview01");
  assert.equal(claimed.entry_history_direction_label, null);
});

test("generic historical queue cannot sample Gate LPR cameras", async () => {
  const pool = mockPool((text) => {
    assert.match(text, /LOWER\(BTRIM\(camera_name\)\) NOT IN \('gate lpr 1','gate lpr 2'\)/);
    return { rows: [], rowCount: 0 };
  });
  const repository = new BlueIrisVehicleFrameRepository(pool);
  await assert.rejects(
    repository.queueHistorical({ cameraName: " gate LPR 2 " }),
    /dedicated Gate Overview \(Overview01\) backfill/,
  );
