...
test("plate ingestion promotes current mapped crossings while preserving legacy shadow data", async () => {
  ...
  assert.match(settings, /Configure both ordered crossings in Blue Iris/);
  assert.match(settings, /blue-iris-front-trigger/);
  assert.match(settings, /blue-iris-rear-trigger/);
