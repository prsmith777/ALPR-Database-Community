...
test("release information resolves a commit-pinned deployment image", () => {
  ...
  assert.equal(release.version, "0.1.36");
  assert.equal(release.manualVersion, "3.2");
  ...
  assert.equal(
    release.notes.title,
    "Full-page usability and settings correctness"
  );
  ...
  assert.match(notes, /mobile More menu/i);
  assert.match(notes, /current-release check/i);
  assert.match(notes, /Docker environment variables/i);
  assert.match(notes, /Vehicle Intelligence navigation/i);
});
...
test("clean-history Docker builds use safe self-hosted release metadata", async () => {
  const metadata = await readBuildMetadata("test/fixtures/intentionally-missing-git");

  assert.deepEqual(metadata, { gitSha: null, channel: "self-hosted" });
  assert.match(serializeReleaseMetadata(metadata), /self-hosted/);
});
