...
test("receipt explorer is protected and reachable from System Logs", async () => {
  ...
  assert.match(page, /IngressReceiptsPage\(\{ searchParams \}\)/);
  assert.match(page, /getIntegrationIngressReceipts\(initialFilters\)/);
  ...
  assert.match(viewer, /href=\{`\/logs\?requestId=.*&expand=first/);
  assert.match(viewer, /href=\{`\/logs\?readId=.*&expand=first/);
  assert.match(viewer, /href=\{`\/live_feed\?readId=/);
  assert.match(viewer, /initialExpandFirst[\s\S]*initialPage\?\.receipts\?\.\[0\]\?\.id/);
  assert.match(viewer, /copyTextToClipboard\(receipt\.requestId\)/);
  ...
  assert.match(wrapper, /readId: displayedParams\.get\("readId"\)/);
