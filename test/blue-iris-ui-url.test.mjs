...
test("vehicle-view playback targets the overview camera at the captured frame time", () => {
  ...
  assert.equal(
    path,
    "ui3.htm?tab=timeline&cam=Street+Overview&timeline=1787422831456&maximize=1"
  );
  assert.equal(
    buildBlueIrisUiUrl("192.168.0.167:81", path),
    "http://192.168.0.167:81/ui3.htm?tab=timeline&cam=Street+Overview&timeline=1787422831456&maximize=1"
  );
