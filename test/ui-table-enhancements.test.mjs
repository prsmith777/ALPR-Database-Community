...
test("live feed review status filtering is multi-select, URL-backed, and server-side", async () => {
  ...
  assert.match(wrapper, /displayedParams[\s\S]*?\.getAll\("reviewStatus"\)/);
...
test("live feed direction is visible, correctable, and filterable by semantic camera label", async () => {
  ...
  assert.match(wrapper, /displayedParams\.getAll\("direction"\)/);
...
test("the image viewer summarizes known-plate and tag associations", async () => {
  ...
  assert.match(
    table,
    />Observed<\/div>[\s\S]*?>Effective<\/div>[\s\S]*?>Review status<\/div>[\s\S]*?>Count<\/div>[\s\S]*?>Camera<\/div>[\s\S]*?>Known plate<\/div>[\s\S]*?>Tags<\/div>[\s\S]*?>Time<\/div>[\s\S]*?>Vehicle<\/div>/
  );
