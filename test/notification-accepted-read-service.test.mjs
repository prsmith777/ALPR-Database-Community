  ...
  return {
    ...
    conditionTree: {
      ...
      children: [
        ...
        { kind: "condition", conditionType: "camera", operator: "in", value: { names: ["Gate LPR 1"] } },
      ],
    },
    actions: [
      {
  ...
  return {
    ...
    mqttRepository: {
      async discoverCamera() {
        return { id: 11, cameraName: "Gate LPR 1", cameraKey: "gate-lpr-1", topicOverride: "" };
      },
...
test("an enabled unified MQTT rule records its decision and queues the durable MQTT outbox", async () => {
  ...
  assert.equal(state.envelopes[0].topic, "Blue Iris/ALPR/gate-lpr-1");
...
test("accepted-read evaluation supplies repository count metrics", async () => {
  ...
  const result = await service.processAcceptedRead({ id: 36500, plate_number: "069YQZ", camera_name: "Gate LPR 1", timestamp: "2026-07-22T20:00:00.000Z" });
  assert.equal(result.status, "queued");
