...
test("webhook target policy blocks loopback and private networks unless explicitly allowed", async () => {
  const privateAddress = ["192", "168", "0", "20"].join(".");
  ...
  assert.equal(classifyAddress(privateAddress), "private");
  await assert.rejects(validateWebhookDestination("https://127.0.0.1/hook"), /forbidden/i);
  await assert.rejects(validateWebhookDestination(`https://${privateAddress}/hook`), /private-network/i);
  const allowed = await validateWebhookDestination(`http://${privateAddress}/hook`, {
    allow_http: true,
    allow_private_networks: true,
  });
  assert.equal(allowed.hostname, privateAddress);
