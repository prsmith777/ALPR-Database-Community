  ...
  const queue = new BlueIrisVehicleFrameQueue({
    ...
    clientFactory: () => ({
      async testConnection() {
        ...
        return {
          cameras: [
            { id: "Cam136", name: "Gate LPR 1", online: true, enabled: true },
...
test("Gate Overview resolves its reviewed Overview01 binding and preserves Entry context", async () => {
  const claims = [{
    id: 902,
    camera_name: "Gate LPR 1",
    ...
    overview_source_camera_name: "Gate Overview",
    overview_source_camera_short_name: "Overview01",
  ...
  const queue = new BlueIrisVehicleFrameQueue({
    repository: {
      ...
      async markFailed() { assert.fail("a reviewed Overview01 binding must not fail"); },
    },
    fileStorage: {},
    loadConfig: async () => configured,
    clientFactory: () => ({
      async testConnection() {
        connectionCount += 1;
        return { cameras: [{ id: "Overview01", name: "Gate Overview" }] };
      },
    }),
  ...
  assert.equal(processed.camera, "Overview01");
  assert.equal(processed.profile.overview_context, "entry");
  assert.equal(processed.profile.source_camera_short_name, "Overview01");
});

test("Gate Overview rejects a blank short-camera binding before Blue Iris initialization", async () => {
  const claims = [{
    id: 904,
    camera_name: "Gate LPR 1",
    timestamp: "2026-08-10T18:00:00Z",
...
test("Gate Overview rejects a display-name and Overview01 mismatch without exporting", async () => {
  const claims = [{
    id: 905,
  ...
  const queue = new BlueIrisVehicleFrameQueue({
    ...
    clientFactory: () => ({
      async testConnection() {
        return { cameras: [
          { id: "Cam149", name: "Gate Overview" },
          { id: "Overview01", name: "Different Camera" },
        ] };
      },
  ...
  assert.equal(failure.retryable, false);
  assert.equal(failure.selectionMetadata.sourceCameraShortName, "Overview01");
});

test("Gate Overview rejects a missing display-name alias even when Overview01 exists", async () => {
  const claims = [{
    id: 906,
    camera_name: "Gate LPR 2",
    timestamp: "2026-08-10T18:00:00Z",
...
test("ambiguous Entry profiles fail closed before creating a Blue Iris client", async () => {
  ...
  assert.equal(failure.selectionMetadata.overviewContext, "entry");
  assert.equal(failure.selectionMetadata.sourceCameraId, "Overview01");
});
...
test("historical Entry work uses the exact Overview01 binding after live queues are empty", async () => {
  const read = claimedEntryHistoryRead();
  const claimOrder = [];
  let processed = null;
  const queue = new BlueIrisVehicleFrameQueue({
  ...
  const queue = new BlueIrisVehicleFrameQueue({
    ...
    clientFactory: () => ({
      async testConnection() {
        return { cameras: [
          { id: "Cam149", name: "Gate Overview" },
          { id: "Overview01", name: "Wrong Camera" },
        ] };
      },
    }),
    timelineExportFactory: () => ({
...
test("an active Gate LPR fallback runs after Street sharing and before another overview export", async () => {
  let overviewClaimed = false;
...
test("overview status reports the active read queue and stable export ledger", async () => {
  ...
  const responses = [
    ...
    { rows: [{ source_camera_name: "Gate Overview" }, { source_camera_name: "Street Overview" }] },
    { rows: [{
      read_id: 39667,
      plate_number: "ABC123",
      camera_name: "Gate LPR 1",
  ...
  assert.deepEqual(status.observedSources, ["Gate Overview", "Street Overview"]);
  assert.equal(status.recentJobs[0].readId, 39667);
  assert.equal(status.recentJobs[0].overviewContext, "entry");
  assert.equal(status.recentJobs[0].sourceCameraName, "Gate Overview");
  assert.equal(status.recentJobs[0].sourceCameraId, "Overview01");
...
test("profile persistence canonicalizes case variants under a normalized primary lock", async () => {
  ...
  const repository = new BlueIrisVehicleFrameRepository({
    async query(sql, parameters) {
      ...
      if (/SELECT id, source_camera_name, plate_camera_name, direction_label[\s\S]*FOR UPDATE/.test(sql)) {
        return { rows: [{
          id: 43,
          source_camera_name: "Gate Overview",
          plate_camera_name: "Gate LPR 1",
      ...
      if (/INSERT INTO public\.vehicle_overview_pair_profiles/.test(sql)) {
        ...
        return { rows: [{
          id: 43,
          source_camera_name: "Gate Overview",
          source_camera_short_name: "Overview01",
          plate_camera_name: "Gate LPR 1",
          direction_label: "Entering",
          source_role: "primary",
          overview_context: "entry",
  ...
  assert.equal(saved.source_camera_name, "Gate Overview");
  assert.ok(statements.some((sql) => /pg_advisory_xact_lock\(hashtext\(\$1\)\)/.test(sql)));
  assert.equal(upsertParameters[0], "Gate Overview");
  assert.equal(upsertParameters[1], "Gate LPR 1");
  assert.equal(upsertParameters[2], "Entering");
  assert.equal(upsertParameters[4], "entry");
  assert.equal(upsertParameters[5], "Overview01");
});

test("camera alias indexing rejects duplicate display names while retaining unique short IDs", () => {
  const index = new Map();
  const first = { id: "Overview01", name: "Gate Overview" };
  const second = { id: "Cam144", name: "Gate Overview" };
  ...
  assert.equal(blueIrisVehicleFrameQueueInternals.uniqueCamera(index, "Gate Overview"), null);
  assert.equal(blueIrisVehicleFrameQueueInternals.uniqueCamera(index, "Overview01")?.id, "Overview01");
});
