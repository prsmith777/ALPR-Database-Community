...
test("Gate Overview publishes distinct Overview01 provenance through the primary pipeline", async () => {
  let readyFrame = null;
  ...
  const service = new BlueIrisVehicleFrameService({
    ...
    repository: {
      ...
      async markFailed() { assert.fail("a valid Gate Overview frame must not fail"); },
    },
    fileStorage: {
  ...
  const result = await service.processOverviewRead({
    read: overviewRead({ camera_name: "Gate LPR 1", bi_trigger_direction_label: "Entering" }),
    profile: overviewProfile({
  ...
  assert.equal(readyFrame.selectionMetadata.sourceCameraName, "Gate Overview");
  assert.equal(readyFrame.selectionMetadata.sourceCameraId, "Overview01");
  assert.equal(readyFrame.selectionMetadata.sourceCameraShortName, "Overview01");
});
...
test("compose database startup fails closed on any migration error", async () => {
  ...
  for (const compose of composeFiles) {
    ...
    assert.match(compose, /--single-transaction/);
    assert.match(compose, /ON_ERROR_STOP=1/);
    assert.match(compose, /pg_isready -h 127\.0\.0\.1/);
    assert.doesNotMatch(compose, /docker-entrypoint\.sh postgres &/);
  }

  const mainCompose = composeFiles[0];
  assert.match(mainCompose, /condition: service_completed_successfully/);

  const migrationHelper = await fs.readFile(
    new URL("../scripts/postgres-major-migration.mjs", import.meta.url),
    "utf8"
  );
  assert.match(
    migrationHelper,
