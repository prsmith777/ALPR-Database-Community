...
test("disabled MQTT still discovers the camera but never queues delivery", async () => {
  ...
  assert.equal(result.camera.cameraKey, "gate-lpr-1");
...
test("an accepted persisted read builds the stable payload and queues one delivery", async () => {
  ...
  assert.equal(enqueueCall.envelope.cameraKey, "gate-lpr-1");
  assert.equal(enqueueCall.envelope.topic, "Blue Iris/ALPR/gate-lpr-1");
  ...
  assert.equal(enqueueCall.envelope.payload.plate_number, "TSTO01");
  assert.equal(enqueueCall.envelope.payload.matched_plate_number, "TSTO01");
  assert.equal(enqueueCall.envelope.payload.plate_name, "Resident's Sedan");
  assert.equal(enqueueCall.envelope.payload.tags, "Family, Resident");
  assert.equal(enqueueCall.envelope.payload.camera_key, "gate-lpr-1");
