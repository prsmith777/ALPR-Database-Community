...
test("rule option lists contain safe brokers, cameras, known identities, and tags", () => {
  const options = mqttRuleAdminInternals.mapOptions({
    ...
    knownPlates: [
      { plate_number: "TSTO01", name: "Resident's Sedan", tags: ["Family"] },
    ],
    knownNames: [{ name: "Resident's Sedan" }],
    tags: [{ name: "Family" }],
  });
  ...
  assert.equal(options.cameras[0].cameraKey, "gate-lpr-1");
  assert.equal(options.knownPlates[0].plateNumber, "TSTO01");
  assert.deepEqual(options.knownNames, ["Resident's Sedan"]);
