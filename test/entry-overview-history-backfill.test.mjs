...
test("monochrome evidence is nighttime while color evidence may reach Overview01", async () => {
...
test("a claimed historical read maps to the immutable Overview01 profile and lifecycle", async () => {
  const read = {
    id: 901,
  ...
  assert.equal(profile.source_camera_name, "Gate Overview");
  assert.equal(profile.source_camera_short_name, "Overview01");
