...
const deliveryRow = {
  ...
  event_id: "read-41-gate-lpr-1",
  read_id: 41,
  camera_id: 7,
  camera_key: "gate-lpr-1",
  camera_name: "Gate LPR 1",
  broker_id: 3,
  topic: "Blue Iris/ALPR/gate-lpr-1",
...
test("runtime context maps settings, known plates, rules, cameras, and broker fields", async () => {
  ...
  assert.equal(context.rules[0].broker.broker, "192.0.2.10");
...
test("camera discovery updates the existing case-insensitive camera identity", async () => {
  ...
  assert.equal(camera.cameraKey, "gate-lpr-1");
...
test("camera discovery resolves stable-key collisions without changing display names", async () => {
  ...
  const client = makeClient((sql, params) => {
    ...
    if (sql.startsWith("INSERT INTO public.mqtt_cameras")) {
      insertCount += 1;
      if (insertCount === 1) {
        assert.equal(params[1], "gate-lpr-1");
        return { rows: [] };
      }
      assert.equal(params[1], "gate-lpr-1-2");
      return {
        rows: [
          {
            id: 8,
            camera_name: "Gate LPR 1",
            camera_key: "gate-lpr-1-2",
            enabled: true,
  ...
  assert.equal(camera.cameraKey, "gate-lpr-1-2");
});
