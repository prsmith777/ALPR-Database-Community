...
test("Blue Iris server addresses are normalized without accepting embedded credentials or paths", () => {
  assert.equal(normalizeBlueIrisBaseUrl("192.0.2.167:81"), "http://192.0.2.167:81");
  assert.equal(normalizeBlueIrisBaseUrl("https://blueiris.local:443/"), "https://blueiris.local");
  assert.throws(
    () => normalizeBlueIrisBaseUrl("http://admin:secret@192.0.2.167:81"),
...
test("Blue Iris login uses the challenge session and never places credentials in the request", async () => {
  ...
  const client = new BlueIrisClient(
    { host: "192.0.2.167:81", username: "alpr", password: "private-password" },
    {
      fetchImpl: async (_url, options) => {
        ...
        return jsonResponse({
          ...
          data: { systemName: "Example Site", version: "6.0.9.6" },
        });
      },
    }
  );
  ...
  assert.equal(data.systemName, "Example Site");
...
test("Blue Iris connection test returns a sanitized camera inventory", async () => {
  const responses = [
    ...
    { result: "success", data: { systemName: "Example Site", version: "6.0.9.6" } },
    {
      result: "success",
      data: [
        { optionValue: "Street_LPR_2", optionDisplay: "Street LPR 2", isOnline: true },
        { optionValue: "Entry_LPR_1", optionDisplay: "+Gate LPR 1", isOnline: false },
      ],
    },
  ];
  const client = new BlueIrisClient(
  ...
  assert.deepEqual(await client.testConnection(), {
    systemName: "Example Site",
    version: "6.0.9.6",
    cameraCount: 2,
    cameras: [
      { id: "Street_LPR_2", name: "Street LPR 2", online: true, enabled: true },
      { id: "Entry_LPR_1", name: "Gate LPR 1", online: false, enabled: true },
    ],
  });
});
