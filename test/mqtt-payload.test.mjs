...
test("known fuzzy matches preserve OCR evidence and expose canonical identity", () => {
  ...
  assert.equal(payload.plate_number, "TST0-01");
  assert.equal(payload.plate_number_normalized, "TST001");
  ...
  assert.equal(payload.camera, "Gate LPR 1");
  assert.equal(payload.camera_key, "gate-lpr-1");
...
test("unknown tagged plates preserve event tags without becoming known plates", () => {
  const payload = buildMqttPlateReadPayload({
    ...
    publication: {
      ...
      ruleNames: ["Gate LPR 1 Match Delivery Tag"],
    },
    settings: DENVER_SETTINGS,
  });
...
test("different camera reads get independent event IDs even one millisecond apart", () => {
  const first = buildMqttPlateReadPayload({
    ...
    camera: { camera_name: "Gate LPR 1", camera_key: "gate-lpr-1" },
    settings: DENVER_SETTINGS,
  });
  const second = buildMqttPlateReadPayload({
    ...
    camera: { camera_name: "Gate LPR 2", camera_key: "gate-lpr-2" },
    settings: DENVER_SETTINGS,
  });
  ...
  assert.equal(first.camera_key, "gate-lpr-1");
  assert.equal(second.camera_key, "gate-lpr-2");
