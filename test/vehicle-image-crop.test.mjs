...
test("crop compatibility remains available without exposing private processing controls", async () => {
  const [cleanup, reconciliation, repository, instrumentation, actions, settings, loader] = await Promise.all([
  ...
  assert.doesNotMatch(settings, /VehicleImageCropPanel/);
  assert.doesNotMatch(loader, /getVehicleImageCropOverview/);
});
