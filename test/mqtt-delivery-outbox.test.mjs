...
const BASE_PUBLICATION = {
  ...
  payload: {
    event_id: "read-87321-gate-lpr-1",
    plate_number: "TST001",
    matched_plate_number: "TSTO01",
  },
};
...
test("delivery envelopes normalize IDs, QoS, retention, payload, and retry limits", () => {
  ...
  assert.equal(envelope.cameraKey, "gate-lpr-1");
  assert.equal(envelope.cameraName, "Gate LPR 1");
...
test("delivery envelopes reject malformed identities, topics, payloads, and dedupe keys", () => {
  assert.throws(
    () => normalizeDeliveryEnvelope({ ...BASE_PUBLICATION, cameraKey: "Gate LPR 1" }),
    /camera key/i
  );
  assert.throws(
