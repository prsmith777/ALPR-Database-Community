...
test("MQTT test publishing queues one standard payload through the durable outbox", async () => {
  ...
  assert.equal(delivery.cameraKey, "gate-lpr-1");
  assert.equal(delivery.brokerId, 4);
  assert.equal(delivery.topic, "Blue Iris/ALPR/gate-lpr-1");
  ...
  assert.equal(delivery.payload.camera, "Gate LPR 1");
  assert.equal(delivery.payload.camera_key, "gate-lpr-1");
