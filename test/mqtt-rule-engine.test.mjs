...
const entryCamera1 = {
  ...
  camera_key: "gate-lpr-1",
};

const entryCamera2 = {
  id: 12,
  camera_name: "Gate LPR 2",
  camera_key: "gate-lpr-2",
};
...
test("exact-plate rules can optionally accept one-character OCR mistakes", () => {
  ...
  assert.equal(fuzzy.matchedPlateNumber, "TSTO01");
  assert.equal(fuzzy.matchDistance, 1);
  assert.equal(fuzzy.candidate.name, "Resident's Sedan");
});
...
test("MQTT rules use the shared plate-matching profiles", () => {
  ...
  assert.equal(strict.matchedPlateNumber, "TSTO01");
});
...
test("known-plate, known-name, and tag rules match the canonical identity", () => {
  const rules = [
    ...
    baseRule({ id: 2, match_type: "known_name", match_value: "Resident's Sedan" }),
    baseRule({ id: 3, match_type: "tag", match_value: "family" }),
  ];

  const result = evaluateMqttRules(rules, {
    observedPlate: "tsto01",
    camera: entryCamera1,
    knownPlates,
  });
...
test("a unique fuzzy known match can satisfy a name or tag rule", () => {
  ...
  assert.equal(nameResult.matchedPlateNumber, "TSTO01");
  assert.equal(tagResult.matchedPlateNumber, "TSTO01");
});
...
test("matching rules for one camera and destination consolidate into one publish", () => {
  ...
  assert.equal(result.publications[0].topic, "Blue Iris/ALPR/gate-lpr-1");
  assert.deepEqual(result.publications[0].ruleNames, [
    "Family",
    "Known vehicles",
    "Resident's Sedan",
  ]);
  assert.equal(result.publications[0].matchedPlateNumber, "TSTO01");
...
test("generic any-plate routing yields to a stronger fuzzy known identity", () => {
  ...
  assert.equal(publication.matchedPlateNumber, "TSTO01");
  assert.equal(publication.identityConflict, false);
  assert.equal(publication.candidate.name, "Resident's Sedan");
  ...
  assert.equal(payload.plate_number, "TST001");
  assert.equal(payload.matched_plate_number, "TSTO01");
  assert.equal(payload.plate_name, "Resident's Sedan");
...
test("different camera observations always plan independent camera topics", () => {
  ...
  assert.equal(first.publications[0].topic, "Blue Iris/ALPR/gate-lpr-1");
  assert.equal(second.publications[0].topic, "Blue Iris/ALPR/gate-lpr-2");
});
...
test("invalid destinations are rejected without blocking other rules", () => {
  ...
  assert.equal(result.publications.length, 1);
  assert.equal(result.publications[0].topic, "alpr/gate-lpr-1");
