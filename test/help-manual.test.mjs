...
test("the user guide is structured, searchable, and role-aware", () => {
  assert.equal(HELP_MANUAL.manualVersion, "1.99");
...
test("the guide covers required workflows and clearly labels planned features", () => {
  ...
  for (const required of [
    ...
    "entry overview",
    "cam143",
    "archive log evidence",
    "cleanup never runs on a schedule",
    "build the canonical overview catalog",
    "creates no canonical files or read links",
    "makes no plate recognizer",
    "frozen prior image",
    "unrelated vehicle later in the clip",
  ]) {
...
test("production releases require help and roadmap updates", async () => {
  ...
  assert.match(roadmap, /Release candidate baseline — August 15, 2026/);
