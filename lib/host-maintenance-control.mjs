...
export async function requestHostMaintenanceExecution({executor,actor,previewToken,confirmation,now=new Date()}={}){
  ...
  try{await client.query("BEGIN");
    ...
    const config=(await client.query(`SELECT circuit_breaker_open,circuit_breaker_generation FROM public.host_maintenance_config
      WHERE category=$1 FOR UPDATE`,[preview.category])).rows?.[0];
    if(!config)throw new Error("Unsupported host maintenance category");
    if(config.circuit_breaker_open)throw new Error("Category circuit breaker is open; acknowledge it before requesting cleanup");
    const acknowledgement=(await client.query(`SELECT breaker_generation,created_at FROM public.host_maintenance_acknowledgements
      WHERE category=$1 ORDER BY breaker_generation DESC LIMIT 1`,[preview.category])).rows?.[0];
    const generation=Number(config.circuit_breaker_generation);
    if(!Number.isSafeInteger(generation)||generation<0||(generation===0&&acknowledgement)||(generation>0&&!acknowledgement))
      throw new Error("Host maintenance acknowledgement evidence is invalid");
    if(acknowledgement){const acknowledgedGeneration=Number(acknowledgement.breaker_generation);
      const previewTime=new Date(preview.created_at).getTime();const acknowledgedTime=new Date(acknowledgement.created_at).getTime();
      if(acknowledgedGeneration!==generation||!Number.isFinite(previewTime)||!Number.isFinite(acknowledgedTime))
        throw new Error("Host maintenance acknowledgement evidence is invalid");
      if(previewTime<=acknowledgedTime)throw new Error("Queue a fresh host maintenance preview after acknowledging this failure");}
...
export async function acknowledgeHostMaintenanceBreaker({executor,actor,category,confirmation,now=new Date()}={}){
  ...
  try{await client.query("BEGIN");const bindings=await assertDatabaseIdentity(client,expectedBindings());const config=(await client.query("SELECT * FROM public.host_maintenance_config WHERE category=$1 FOR UPDATE",[normalized])).rows[0];
    ...
    const breakerOpenedAt=new Date(config.circuit_breaker_opened_at);const breakerOpenedTime=breakerOpenedAt.getTime();const breakerGeneration=Number(config.circuit_breaker_generation);
    if(config.circuit_breaker_opened_at==null||!Number.isFinite(breakerOpenedTime)||!Number.isSafeInteger(breakerGeneration)||breakerGeneration<=0)throw new Error("Host maintenance breaker evidence is invalid");
    const environmentId=bindings.environmentId;
    const worker=await assertHealthyWorker(client,now,bindings);
    const heartbeatTime=new Date(worker.heartbeat_at).getTime();const inventoryTime=new Date(worker.inventory_measured_at).getTime();
    if(!Number.isFinite(heartbeatTime)||!Number.isFinite(inventoryTime)||heartbeatTime<=breakerOpenedTime||inventoryTime<=breakerOpenedTime)
    ...
    const failed=(await client.query(`SELECT i.id,r.worker_generation AS failed_worker_generation,r.created_at AS failure_receipt_at
      ...
      WHERE i.category=$1 AND i.environment_id=$2 AND i.database_identity=$3 AND i.status='failed' AND i.intent_type IN('execute','scheduled')
        AND i.completed_at=$4::timestamptz AND r.created_at=$4::timestamptz
      ORDER BY i.id DESC LIMIT 1`,[normalized,environmentId,bindings.databaseIdentity,config.circuit_breaker_opened_at])).rows[0];
    if(!failed)throw new Error("No failed destructive intent is available for acknowledgement");
    const failureReceiptTime=new Date(failed.failure_receipt_at).getTime();
    if(!Number.isFinite(failureReceiptTime)||inventoryTime<=failureReceiptTime)throw new Error("A post-failure worker inventory is required before acknowledgement");
    ...
    await client.query("COMMIT");return{category:normalized,acknowledged:true,breakerGeneration};
  }catch(error){try{await client.query("ROLLBACK");}catch{}throw error;}finally{client.release();}
}
