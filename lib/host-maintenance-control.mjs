...
async function assertHealthyWorker(database,now,bindings=expectedBindings()){
  if(!bindings.environmentId||!bindings.databaseIdentity)throw new Error("Host maintenance is unavailable");
  const result=await database.query(`SELECT database_identity,heartbeat_at,worker_generation,inventory_revision,inventory_measured_at FROM public.host_maintenance_worker_state
...
export async function requestHostMaintenancePreview({executor,actor,category,now=new Date()}={}){
  ...
  const bindings=expectedBindings(); await assertHealthyWorker(database,now,bindings);
  const result=await database.query(`INSERT INTO public.host_maintenance_intents(intent_type,category,environment_id,database_identity,status,actor_user_id,requested_at)
    VALUES('preview',$1,$2,$3,'pending',$4,$5) RETURNING id`,[normalizeHostMaintenanceCategory(category),bindings.environmentId,bindings.databaseIdentity,actorId(actor),now]);
  return{requestId:Number(result.rows[0].id),status:"pending",hostWorkerRequired:true};
}
...
export async function getHostMaintenanceRequestStatus({executor,requestId,actor}={}){
  ...
  const result=await database.query(`WITH delivered AS (
    ...
    ) SELECT i.id,i.category,i.status,i.candidate_count,i.candidate_bytes,i.reclaimed_bytes,i.last_error,
      ...
      [Number(requestId),actorId(actor),expectedEnvironmentId()||"",expectedDatabaseIdentity()||""]);
  const row=result.rows?.[0]; if(!row)throw new Error("Host maintenance request is invalid");
  return{requestId:Number(row.id),category:row.category,status:row.status,previewToken:row.opaque_token||null,expiresAt:row.expires_at||null,
...
export async function requestHostMaintenanceExecution({executor,actor,previewToken,confirmation,now=new Date()}={}){
  ...
  try{await client.query("BEGIN");
    const bindings=expectedBindings();if(!bindings.environmentId||!bindings.databaseIdentity)throw new Error("Host maintenance is unavailable");
...
export async function setScheduledHostMaintenance({executor,actor,category,enabled,confirmation="",intervalSeconds=604800,retainedVerifiedCount=5,minimumAgeDays=30,now=new Date()}={}){
  ...
  try{await client.query("BEGIN");const current=await client.query("SELECT * FROM public.host_maintenance_config WHERE category=$1 FOR UPDATE",[normalized]);
...
export async function acknowledgeHostMaintenanceBreaker({executor,actor,category,confirmation,now=new Date()}={}){
  ...
  const database=executor||await defaultPool();const client=await database.connect();
  try{await client.query("BEGIN");const config=(await client.query("SELECT * FROM public.host_maintenance_config WHERE category=$1 FOR UPDATE",[normalized])).rows[0];
    if(!config?.circuit_breaker_open)throw new Error("Category circuit breaker is not open");
    const bindings=expectedBindings();const environmentId=bindings.environmentId;
...
export async function getHostMaintenanceOverview({executor,now=new Date(),workerStaleSeconds=HOST_WORKER_STALE_SECONDS}={}){
 const database=executor||await defaultPool();const bindings=expectedBindings();const expected=bindings.environmentId;const[configs,intents,runs,worker]=await Promise.all([
  ...
  database.query("SELECT environment_id,database_identity,worker_generation,heartbeat_at,inventory_revision,inventory_measured_at,last_error FROM public.host_maintenance_worker_state WHERE environment_id=$1 AND database_identity=$2 LIMIT 1",[expected||"",bindings.databaseIdentity||""]) ]);
  ...
  const workerShape=!row?{status:"unavailable",ageSeconds:null,note:"Install the separate fixed host worker; the web application has no host access."}
  ...
  return{configs:configs.rows.map(configShape),intents:publicIntents,runs:publicRuns,worker:workerShape,
...
export async function enqueueDueScheduledHostMaintenance({executor,now=new Date()}={}){const database=executor||await defaultPool();const bindings=expectedBindings();
  if(!bindings.environmentId||!bindings.databaseIdentity)return{status:"unavailable"};const result=await database.query(`WITH due AS (
...
export const hostMaintenanceControlInternals=Object.freeze({configShape});
