...
function mapState(row, shadowRule = null) {
  ...
  const approved = shadowRule?.status === "approved" && shadowRule?.latestReview?.current === true;
  const blockers = [];
  if (!runtimeSupported) blockers.push("A live unified delivery adapter is not available for this channel.");
  if (runtimeSupported && !configurationMatches) {
...
export class NotificationCutoverRepository {
  ...
  async cutover({ ruleId, actor = null, matchingSettings = {} } = {}) {
    ...
    try {
      ...
      await client.query("SELECT pg_advisory_xact_lock(hashtext('alpr_notification_cutover'), $1::integer)", [parsedRuleId]);
      ...
      if (!reviewed || reviewed.status !== "approved" || !reviewed.latestReview?.current) {
