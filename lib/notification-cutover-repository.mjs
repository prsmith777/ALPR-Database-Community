...
const CUTOVER_STATE_QUERY = `
  SELECT m.id AS migration_id, m.source_type, m.source_id, m.target_rule_id,
         ...
         CASE m.source_type
           ...
           WHEN 'pushover' THEN (
             SELECT jsonb_build_object('priority', priority)
         ...
         COALESCE(BOOL_AND(
           (m.source_type = 'mqtt' AND ch.channel_type = 'mqtt') OR
           (m.source_type = 'pushover' AND ch.channel_type = 'pushover')
         ), FALSE) AS runtime_supported,
         COALESCE(
           JSONB_AGG(
             jsonb_build_object(
  ...
  LEFT JOIN public.notification_actions a ON a.rule_id = r.id AND a.retired_at IS NULL
  LEFT JOIN public.notification_channels ch ON ch.id = a.channel_id
  WHERE m.retired_at IS NULL AND m.finalized_at IS NULL
  GROUP BY m.id, r.id
...
function pushoverConfigurationMatches(sourceConfiguration, deliveries) {
  if (!sourceConfiguration || !Array.isArray(deliveries) || deliveries.length !== 1) return false;
  const delivery = deliveries[0] || {};
  const configuration = delivery.configuration || {};
  return (
    delivery.channelType === "pushover" &&
    delivery.credentialReference === "settings:notifications.pushover" &&
    Number(sourceConfiguration.priority ?? 1) === Number(configuration.priority ?? 1)
  );
}

function deliveryTypesMatchSource(sourceType, deliveries) {
  const expected = sourceType === "mqtt" ? "mqtt" : sourceType === "pushover" ? "pushover" : null;
  return Boolean(expected) && deliveries.length > 0 && deliveries.every((row) => row.channel_type === expected);
}
...
function mapState(row, shadowRule = null) {
  ...
  const runtimeSupported =
    ...
    ["mqtt", "pushover"].includes(row.source_type) &&
    Number(row.target_cooldown_seconds || 0) === 0;
  const configurationMatches = row.source_type === "mqtt"
    ? mqttConfigurationMatches(row.source_configuration, row.delivery_configurations)
    : row.source_type === "pushover"
      ? pushoverConfigurationMatches(row.source_configuration, row.delivery_configurations)
      : false;
  const approved =
  ...
  if (runtimeSupported && state !== "source_removed" && !configurationMatches) {
    blockers.push(`The unified ${row.source_type === "mqtt" ? "MQTT destination" : "Pushover priority"} no longer matches the legacy source rule.`);
  }
...
async function lockOrphanedMigration(client, ruleId) {
  const mappingResult = await client.query(
    `
      ...
      FROM public.notification_rule_migrations m
      JOIN public.notification_rules r ON r.id = m.target_rule_id
      WHERE r.id = $1::bigint AND m.retired_at IS NULL AND m.finalized_at IS NULL
      FOR UPDATE OF m, r
    `,
    [ruleId]
  );
  ...
  const deliveries = await client.query(
    `
      ...
      WHERE a.rule_id = $1::bigint AND a.retired_at IS NULL
      ORDER BY a.position, a.id
      FOR UPDATE OF a, ch
    `,
    [ruleId]
...
function allPushover(deliveries) {
  return deliveries.length > 0 && deliveries.every((row) => row.channel_type === "pushover");
}
...
function lockedPushoverConfigurationMatches(locked) {
  return pushoverConfigurationMatches(
    { priority: locked.source.priority },
    locked.deliveries.map((row) => ({
      channelType: row.channel_type,
      credentialReference: row.credential_reference,
      configuration: row.configuration,
...
async function setLegacySourceEnabled(client, locked, enabled) {
  if (locked.mapping.source_type === "mqtt") {
    await client.query("UPDATE public.mqtt_rules SET enabled = $2::boolean WHERE id = $1::bigint", [locked.mapping.source_id, enabled]);
    return;
  }
  if (locked.mapping.source_type === "pushover") {
    await client.query("UPDATE public.plate_notifications SET enabled = $2::boolean WHERE id = $1::bigint", [locked.mapping.source_id, enabled]);
    return;
  }
...
export class NotificationCutoverRepository {
  ...
  async cutover({ ruleId, actor = null, matchingSettings = {} } = {}) {
    ...
    try {
      ...
      if (!deliveryTypesMatchSource(locked.mapping.source_type, locked.deliveries)) {
        throw new Error("A live unified delivery adapter is not available for this channel");
      }
      if (Number(locked.mapping.cooldown_seconds || 0) !== 0) {
        throw new Error("A live unified delivery adapter is not available for this channel");
      }
      const configurationMatches = locked.mapping.source_type === "mqtt"
        ? lockedMqttConfigurationMatches(locked)
        : lockedPushoverConfigurationMatches(locked);
      if (!configurationMatches) {
        throw new Error(`Unified ${locked.mapping.source_type === "mqtt" ? "MQTT destination" : "Pushover priority"} no longer matches the legacy source rule`);
      }
      ...
      await setLegacySourceEnabled(client, locked, false);
      await client.query(
      ...
      await client.query(`
        UPDATE public.notification_actions SET enabled = TRUE
  ...
  async rollback({ ruleId, actor = null } = {}) {
    ...
    try {
      ...
      await client.query(`
        UPDATE public.notification_actions SET enabled = FALSE
        WHERE rule_id = $1::bigint AND retired_at IS NULL
      `, [parsedRuleId]);
      await client.query(
      ...
      await setLegacySourceEnabled(client, locked, true);
      await recordEvent(client, {
        locked,
        direction: "rollback",
        actor,
...
export const notificationCutoverRepositoryInternals = Object.freeze({
  ...
  allMqtt,
  allPushover,
  deliveryTypesMatchSource,
  lockOrphanedMigration,
  lockCutover,
  lockedMqttConfigurationMatches,
  lockedPushoverConfigurationMatches,
  mapState,
  mqttConfigurationMatches,
  pushoverConfigurationMatches,
  setLegacySourceEnabled,
  stateName,
});
