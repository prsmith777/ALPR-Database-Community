...
export async function ensureMaintenanceJobState({
  query,
  jobName = RETENTION_MAINTENANCE_JOB,
  enabled,
  intervalSeconds,
  initialDelaySeconds,
} = {}) {
  ...
  await query(
    `INSERT INTO public.maintenance_job_state (
     ...
     ON CONFLICT (job_name) DO UPDATE SET
       ...
       next_run_at = CASE
         ...
         WHEN maintenance_job_state.status = 'failed'
    ...
    [jobName, Boolean(enabled), intervalSeconds, initialDelaySeconds]
  );
}

export async function getMaintenanceJobStatus({ query, jobName = RETENTION_MAINTENANCE_JOB } = {}) {
