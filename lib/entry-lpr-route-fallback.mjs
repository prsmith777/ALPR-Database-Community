...
function entryOverviewPayload(source) {
  ...
  if (
    ...
    || key(metadata?.sourceCameraName) !== "gate overview"
    || key(metadata?.sourceCameraShortName ?? metadata?.sourceCameraId) !== "overview01"
  ) return null;
  return {
...
export function buildEntryLprFallbackDecisions({ targets = [], sourcesByTarget = new Map(), now = Date.now() } = {}) {
  ...
  for (const target of targets) {
    ...
    if (!best) {
      ...
      // Identity/timing evidence may arrive before its direction-authoritative Overview01
...
export class EntryLprRouteFallbackService {
  ...
  async processNext() {
    ...
    try {
      ...
      this.logger?.error?.("Gate LPR fallback failed", {
        sourceReadId: Number(claim.payload_read_id),
        targetReadId: Number(claim.target_read_id),
