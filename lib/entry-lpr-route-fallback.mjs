...
const ENTRY_FALLBACK_ALGORITHM = "entry-lpr-route-fallback-v2-entry-overview";
...
function usableSource(source, route) {
  if (
    ...
    || source?.bi_trigger_direction_status !== "ready"
    || key(source?.bi_trigger_direction_label) !== key(route.sourceDirectionLabel)
...
function identityEvidence(target, sources) {
  ...
  if (
    ...
    return {
      class: exactCount === sourcePlates.length ? "exact" : "exact_with_dual_camera_fuzzy_corroboration",
      penalty: exactCount === sourcePlates.length ? 0 : 0.08,
      targetPlate,
      sourcePlates,
    };
  }
...
function candidateEvents(target, route, sourceReads) {
  ...
  for (const anchor of candidates) {
    ...
    if (new Set(selected.map((source) => key(source.camera_name))).size < route.minimumSourceCount) continue;
    ...
    events.push({
      ...
      target,
      sources: selected,
      chosenSource: rankedSources[0],
      chosenPayload: rankedPayloads[0]?.payload || null,
      evidence,
      eventAt,
      actualDeltaMs,
      timingErrorMs,
  ...
  return events.sort((left, right) => left.score - right.score || left.eventAt - right.eventAt);
}
...
export function buildEntryLprFallbackDecisions({ targets = [], sourcesByTarget = new Map(), now = Date.now() } = {}) {
  ...
  for (const target of targets) {
    ...
    if (!best) {
      ...
      selected = null;
    } else if (runnerUp && runnerUp.score - best.score < ENTRY_FALLBACK_MARGIN) {
      status = "rejected";
      reason = "ENTRY_FALLBACK_AMBIGUOUS";
      selected = null;
    } else if (!best.chosenPayload) {
      // The two Entry reads prove identity and timing, but a route decision is
      // intentionally not frozen until one of those exact reads owns a fully
      // validated Cam143 result. This lets a later discovery cycle succeed
      // after the primary Entry Overview worker completes without ever
      // falling back to a plate-camera image.
      continue;
    }
