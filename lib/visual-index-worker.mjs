...
export class VisualIndexWorker {
  ...
  async runOnce() {
    ...
    const directionRemaining = Number(this.status.direction?.pending || 0);
    if (remaining === 0 && directionRemaining === 0) {
      this.phase = "idle";
      return { delayMs };
    }

    const processingDirections = directionRemaining > 0
      && typeof this.service.backfillDirectionBatch === "function"
      && (remaining === 0 || this.lastBatch?.kind !== "vehicle-direction");
    this.phase = processingDirections ? "direction-backfill" : "indexing";
    ...
    this.status = processingDirections
      ? await this.service.getStatus()
      : result.status || await this.service.getStatus();
    this.lastBatch = {
      kind: processingDirections ? "vehicle-direction" : "visual-index",
      at: isoTime(finished),
