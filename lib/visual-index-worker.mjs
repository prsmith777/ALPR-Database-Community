...
export class VisualIndexWorker {
  ...
  async runOnce() {
    ...
    if (remaining === 0 && directionRemaining === 0 && vehicleTypeRemaining === 0) {
      this.phase = "idle";
      return { delayMs };
    }
    ...
    const processingVehicleTypes = canProcessVehicleTypes
      && (!canProcessDirections || this.lastBatch?.kind === "vehicle-direction");
    const processingDirections = canProcessDirections
    ...
    this.status = processingDirections || processingVehicleTypes
