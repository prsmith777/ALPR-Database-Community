...
export class CaptureAssetService {
  ...
  async getDirectionSetup(cameraName = null) {
    const [profiles, backfill, indexStatus, blueIrisTriggerDirection] = await Promise.all([
      ...
      typeof this.repository.getBlueIrisTriggerDirectionStatus === "function"
        ? this.repository.getBlueIrisTriggerDirectionStatus()
  ...
  async getStatus() {
    ...
    const direction = typeof this.repository.getDirectionBackfillStatus === "function"
      ...
      : undefined;
    const vehicleTypePending = typeof this.repository.getPendingVehicleAttributeCount === "function"
      ? await this.repository.getPendingVehicleAttributeCount({
          attributeKey: "body_type",
          provider: VEHICLE_TYPE_PROVIDER,
          modelVersion: VEHICLE_TYPE_MODEL,
        })
      : 0;
    const vehicleColorPending = typeof this.repository.getPendingVehicleAttributeCount === "function"
      ? await this.repository.getPendingVehicleAttributeCount({
          attributeKey: "color",
          provider: VEHICLE_COLOR_PROVIDER,
          modelVersion: VEHICLE_COLOR_MODEL,
