...
export class CaptureAssetService {
  ...
  async indexRead(read) {
    ...
    try {
      ...
      try {
        await this.refreshDirectionObservation(read.id);
  ...
  async getDirectionSetup(cameraName = null) {
    const profiles = await this.repository.listDirectionProfiles(VEHICLE_REID_MODEL);
  ...
  async refreshDirectionObservation(readId) {
  ...
  async refreshCameraDirection(cameraName) {
    ...
    for (const asset of assets) {
      const classified = classifyVehicleOrientation({
        embedding: asset.vehicle_embedding,
        samples,
        minimumConfidence: Number(profile.minimum_confidence),
      });
  ...
  async clusterRecentUnassigned(limit = 100) {
    ...
    const attributes = await this.analyzeRecentVehicleColors(bounded);
