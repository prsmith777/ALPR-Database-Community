...
export class CaptureAssetService {
  constructor({
    ...
    this.changeNotifier = typeof changeNotifier === "function" ? changeNotifier : null;
    this.logger = logger;
    this.batchPromise = null;
    this.directionBatchPromise = null;
  }

  async getV1ProducerControl(repository = this.repository) {
    if (typeof repository.getV1ProducerControl !== "function") {
      // Preserve the interface used by test doubles and external repositories.
      // The bundled PostgreSQL repository always supplies the durable control.
      return { mode: "v1_primary", state: "active", revision: 1 };
    }
    const control = await repository.getV1ProducerControl();
    return {
      mode: String(control?.mode || ""),
      state: String(control?.v1_producer_state || "active"),
      revision: Number(control?.v1_producer_revision || 1),
      reason: control?.v1_producer_reason || null,
      changedAt: control?.v1_producer_changed_at || null,
    };
  }

  async requireV1ProducerActive(repository = this.repository) {
    const control = await this.getV1ProducerControl(repository);
    if (control.state === "stopped") {
      const error = new Error(
        "The retained ReID v1 producer is stopped while authoritative ReID v2 remains primary."
      );
      error.code = "VEHICLE_REID_V1_PRODUCER_STOPPED";
      throw error;
    }
    return control;
  }
  ...
  async indexRead(read) {
    ...
    await this.requireV1ProducerActive();
    ...
    try {
      ...
      const writeReadyAsset = async (writerRepository = this.repository) => {
        // Recheck after taking the shared derived-storage writer lock. The
        // producer-stop transition takes the matching exclusive lock, so no
        // legacy file or row can cross the audited transition boundary.
        await this.requireV1ProducerActive(writerRepository);
      ...
      if (this.changeNotifier) {
        try {
          await this.changeNotifier({ readId: Number(read.id), reason: "visual_processing" });
        } catch (notificationError) {
          this.logger?.warn?.("Live Feed change notification skipped", {
            readId: Number(read.id),
            code: String(notificationError?.code || "LIVE_FEED_NOTIFICATION_FAILED"),
          });
        }
      }
      return this.repository.getAsset(read.id);
    } catch (error) {
      if (error?.code === "VEHICLE_REID_V1_PRODUCER_STOPPED") throw error;
  ...
  async getVehicleClusterOverview(options = {}) {
    const view = ["profiles", "review"].includes(options.view) ? options.view : "all";
    const reviewQueue = ["vehicle", "plates", "direction", "setup"].includes(options.reviewQueue)
      ? options.reviewQueue
      : view === "review" ? "vehicle" : "all";
    const needsDirectionProfiles = view === "all" || view === "profiles"
      || (view === "review" && reviewQueue === "setup");
    const needsSetupAttention = view === "all" || (view === "review" && reviewQueue === "setup");
    const [result, directionProfiles, visualProfiles, detectionRows, calibration] = await Promise.all([
      ...
      needsDirectionProfiles ? this.repository.listDirectionProfiles(VEHICLE_REID_MODEL) : Promise.resolve([]),
      needsSetupAttention ? this.repository.listCameraProfiles() : Promise.resolve([]),
      needsSetupAttention ? this.repository.listCameraDetectionStats() : Promise.resolve([]),
      needsSetupAttention ? this.getCalibrationSummary() : Promise.resolve(null),
    ]);
  ...
  async getStatus({ includeDirection = true } = {}) {
    const [status, direction, vehicleTypePending, vehicleColorPending, v1Producer] = await Promise.all([
