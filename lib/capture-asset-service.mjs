...
function normalizedPlate(value) {
  ...
  return plate.length >= 2 ? plate : null;
}

function samePlate(left, right) {
  const normalizedLeft = normalizedPlate(left);
  return Boolean(normalizedLeft && normalizedLeft === normalizedPlate(right));
}
  ...
  return candidates
    .map((candidate) => {
      ...
      const hasColorSignals = isColorSignature(source.color_signature)
        && source.color_signature_version === COLOR_SIGNATURE_VERSION
        && isColorSignature(candidate.color_signature)
        && candidate.color_signature_version === COLOR_SIGNATURE_VERSION;
      const plateConfirmed = samePlate(source.plate_number, candidate.plate_number);
      const explanation = explainSimilarity({
        ...
        colorDistance: hasColorSignals
          ...
          : null,
        colorReliability: hasColorSignals
          ? colorSignatureReliability(source.color_signature)
          : null,
        plateConfirmed,
      });
      return { ...publicAsset(candidate), ...explanation };
    })
    .filter((candidate) => candidate.exact || candidate.plateConfirmed || candidate.distance <= DEFAULT_MAX_HAMMING_DISTANCE)
    .sort((left, right) =>
      Number(right.exact) - Number(left.exact) ||
      Number(right.plateConfirmed) - Number(left.plateConfirmed) ||
...
export class CaptureAssetService {
  ...
  async withColorSignature(asset) {
    if (asset?.color_signature_version === COLOR_SIGNATURE_VERSION && isColorSignature(asset?.color_signature)) {
      return asset;
    }
    ...
    try {
      ...
      return {
        ...asset,
        color_signature: await this.colorSignatureForBuffer(image),
  ...
  async rankWithAvailableColor(source, candidates, limit) {
    const exactIdentity = candidates.find((candidate) =>
      source.source_sha256 && source.source_sha256 === candidate.source_sha256
    );
    const identifiedSource = source.plate_number || !exactIdentity?.plate_number
      ? source
      : { ...source, plate_number: exactIdentity.plate_number };
    const plausible = candidates
      .map((candidate) => ({
        candidate,
        distance: hammingDistance(identifiedSource.perceptual_hash, candidate.perceptual_hash),
        exact: Boolean(identifiedSource.source_sha256 && identifiedSource.source_sha256 === candidate.source_sha256),
        plateConfirmed: samePlate(identifiedSource.plate_number, candidate.plate_number),
      }))
      .filter((entry) => entry.exact || entry.plateConfirmed || entry.distance <= DEFAULT_MAX_HAMMING_DISTANCE)
      .sort((left, right) =>
        Number(right.exact) - Number(left.exact) ||
        Number(right.plateConfirmed) - Number(left.plateConfirmed) ||
        left.distance - right.distance
    ...
    const colorSource = await this.withColorSignature(identifiedSource);
