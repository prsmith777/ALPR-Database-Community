...
export class CaptureAssetService {
  ...
  async getVehicleClusterOverview({ featureKey = null } = {}) {
    const normalizedFeatureKey = featureKey
      ? normalizeDistinctiveFeatures([featureKey])[0]
      : null;
    const result = await this.repository.listVehicleClusterOverview({ featureKey: normalizedFeatureKey });
    ...
    return {
      ...
      clusters: result.clusters.map((cluster) => ({
        ...
        distinctiveFeatures: (Array.isArray(cluster.distinctive_features)
          ? cluster.distinctive_features : []).map((feature) => ({
          ...feature,
          label: distinctiveFeatureLabel(feature.key),
        })),
      ...
      algorithmVersion: VEHICLE_CLUSTER_ALGORITHM,
      mode: "shadow",
      featureCatalog: VEHICLE_DISTINCTIVE_FEATURES,
    };
  }
  ...
  async getVehicleProfile(clusterId) {
    ...
    return {
      ...
      distinctiveFeatures: result.distinctiveFeatures.map((feature) => ({
        key: feature.feature_key,
        label: distinctiveFeatureLabel(feature.feature_key),
        captureCount: Number(feature.capture_count || 0),
        confidence: feature.confidence === null ? null : Number(feature.confidence),
        humanReviewed: feature.human_reviewed === true,
      })),
      captures: result.captures.map((capture) => ({
        ...
        distinctiveFeatures: (Array.isArray(capture.distinctive_features)
          ? capture.distinctive_features : []).map((feature) => ({
          ...feature,
          label: distinctiveFeatureLabel(feature.key),
        })),
      })),
  ...
  async reviewVehicleDistinctiveFeatures({ readId, features, actor }) {
    ...
    return this.repository.reviewVehicleDistinctiveFeatures({
      readId: normalizedReadId,
      features: normalizeDistinctiveFeatures(features),
      actor,
    });
  }
