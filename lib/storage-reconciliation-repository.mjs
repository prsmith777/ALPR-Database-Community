  ...
  const result = await client.query(
    `SELECT DISTINCT path
     FROM (
       ...
       UNION ALL
       SELECT vehicle_image_path AS path FROM public.plate_reads WHERE vehicle_image_path = ANY($1::text[])
       UNION ALL
       SELECT frame_path AS path FROM public.vehicle_overview_candidates WHERE frame_path = ANY($1::text[])
       UNION ALL
       ...
       UNION ALL
       SELECT storage_path AS path FROM public.vehicle_image_assets WHERE storage_path = ANY($1::text[])
       UNION ALL
       SELECT storage_path AS path FROM public.vehicle_image_derivatives WHERE storage_path = ANY($1::text[])
     ) referenced_paths
  ...
  try {
    ...
    const bounds = await client.query(
      `SELECT
         ...
         COALESCE((SELECT MAX(id) FROM public.vehicle_image_assets), 0)::bigint
           AS max_vehicle_image_asset_id,
         COALESCE((SELECT MAX(id) FROM public.vehicle_image_derivatives), 0)::bigint
           AS max_vehicle_image_derivative_id`
    );
    const run = await client.query(
      `INSERT INTO public.storage_reconciliation_runs (
         status, phase, max_plate_read_id, max_capture_asset_id,
         max_vehicle_image_asset_id, max_vehicle_image_derivative_id
       ) VALUES ('running', 'filesystem', $1, $2, $3, $4)
       RETURNING *`,
      [
        bounds.rows[0].max_plate_read_id,
        bounds.rows[0].max_capture_asset_id,
        bounds.rows[0].max_vehicle_image_asset_id,
        bounds.rows[0].max_vehicle_image_derivative_id,
      ]
    );
...
function plateReadReferences(rows) {
  return rows.flatMap((row) => [
    ...
    row.vehicle_image_path && { relativePath: row.vehicle_image_path, referenceType: "plate-read-vehicle", ownerId: row.id },
  ].filter(Boolean));
}
...
function vehicleImageAssetReferences(rows) {
  return rows.flatMap((row) => [
    row.storage_path && {
      relativePath: row.storage_path,
      referenceType: "vehicle-image-asset",
      ownerId: row.id,
    },
  ].filter(Boolean));
}

function vehicleImageDerivativeReferences(rows) {
  return rows.flatMap((row) => [
    row.storage_path && {
      relativePath: row.storage_path,
      referenceType: "vehicle-image-derivative",
      ownerId: row.id,
    },
  ].filter(Boolean));
}
...
async function processReferencePhase(client, run, { baseDir, batchSize, phase }) {
  const phaseConfiguration = {
    "plate-reads": {
      table: "plate_reads",
      cursorColumn: "plate_read_cursor",
      upperBound: run.max_plate_read_id,
      selectColumns: "id, image_path, thumbnail_path, vehicle_image_path",
      references: plateReadReferences,
      nextPhase: "capture-assets",
    },
    "capture-assets": {
      table: "capture_assets",
      cursorColumn: "capture_asset_cursor",
      upperBound: run.max_capture_asset_id,
      selectColumns: "id, source_image_path, derived_path",
      references: captureAssetReferences,
      nextPhase: "vehicle-image-assets",
    },
    "vehicle-image-assets": {
      table: "vehicle_image_assets",
      cursorColumn: "vehicle_image_asset_cursor",
      upperBound: run.max_vehicle_image_asset_id,
      selectColumns: "id, storage_path",
      references: vehicleImageAssetReferences,
      nextPhase: "vehicle-image-derivatives",
    },
    "vehicle-image-derivatives": {
      table: "vehicle_image_derivatives",
      cursorColumn: "vehicle_image_derivative_cursor",
      upperBound: run.max_vehicle_image_derivative_id,
      selectColumns: "id, storage_path",
      references: vehicleImageDerivativeReferences,
      nextPhase: "completed",
    },
  }[phase];
  if (!phaseConfiguration) throw new Error("Storage reconciliation reference phase is invalid");
  const {
    table,
    cursorColumn,
    upperBound,
...
export async function getStorageReconciliationStatus({ query } = {}) {
  ...
  return {
    ...
    run: run ? {
      ...
      vehicleImageAssetCursor: Number(run.vehicle_image_asset_cursor) || 0,
      vehicleImageDerivativeCursor: Number(run.vehicle_image_derivative_cursor) || 0,
    } : null,
    findings,
  };
}
