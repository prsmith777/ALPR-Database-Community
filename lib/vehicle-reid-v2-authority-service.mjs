...
function publicControl(row) {
  ...
  return {
    ...
    v1ProducerState: row.v1_producer_state || "active",
    v1ProducerRevision: number(row.v1_producer_revision) || 1,
    v1ProducerReason: row.v1_producer_reason || null,
    v1ProducerChangedAt: row.v1_producer_changed_at || null,
  };
}
...
function publicOverview(raw) {
  ...
  return {
    ...
    v1Retained: {
      assets: number(raw?.v1Counts?.assets),
      readyAssets: number(raw?.v1Counts?.ready_assets),
      derivedFiles: number(raw?.v1Counts?.derived_files),
      matchReviews: number(raw?.v1Counts?.match_reviews),
      clusters: number(raw?.v1Counts?.clusters),
      assignments: number(raw?.v1Counts?.assignments),
      plateAssociations: number(raw?.v1Counts?.plate_associations),
      lastWriteAt: raw?.v1Counts?.last_write_at || null,
    },
...
export class VehicleReidV2AuthorityService {
  ...
  async getControl() {
    return publicControl(await this.repository.getControl());
  }
  ...
  async transitionV1Producer(input = {}) {
    const control = await this.repository.transitionV1Producer(input);
    return { operation: { control: publicControl(control) }, overview: await this.getOverview() };
  }
  ...
  async getProfile(profileId) {
    ...
    return {
      profile: {
        ...publicProfile({
          ...result.profile,
          member_count: result.profile.member_count,
          read_count: result.profile.read_count,
        }),
