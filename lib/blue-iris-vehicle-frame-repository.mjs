    ...
    return this.withTransaction(async (repository) => {
      const result = await repository.pool.query(
        `WITH eligible AS MATERIALIZED (
         ...
         UPDATE public.vehicle_overview_framing_repair_jobs jobs
         SET status = $3, retryable = $4, next_attempt_at = $5::timestamptz,
             claim_token = NULL, heartbeat_at = NULL, processing_deadline_at = NULL,
             hard_deadline_at = NULL, error_code = $6, error_details = $7::jsonb,
             completed_at = CASE WHEN $3 IN ('preserved','source_changed','unavailable')
               OR ($3 = 'failed' AND $4 = FALSE) THEN CURRENT_TIMESTAMP ELSE NULL END,
             updated_at = CURRENT_TIMESTAMP
