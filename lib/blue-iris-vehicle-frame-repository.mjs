  ...
  async getEntryOverviewBackfillRun(runId, { jobLimit = 100 } = {}) {
    ...
    const jobsResult = await this.pool.query(
      `SELECT id, run_id, read_id, semantic_key, profile_id, profile_key,
              ...
              retryable, error_code, error_details, prior_state_fingerprint,
      ...
      `WITH candidate AS (
         ...
         WHERE runs.status = 'running'
           ...
           AND ($1::boolean = FALSE OR NOT EXISTS (
             ...
             WHERE COALESCE(live.vehicle_image_queue_kind, 'live') IN ('live','manual','overview')
               AND (
                 live.vehicle_image_status IN ('pending','processing')
                 OR (live.vehicle_image_status = 'failed' AND live.vehicle_image_retryable = TRUE)
               )
           ))
