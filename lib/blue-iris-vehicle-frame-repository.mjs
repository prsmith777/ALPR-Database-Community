...
const ENTRY_HISTORY_CAMERA_NAME = "Gate Overview";
const ENTRY_HISTORY_CAMERA_SHORT_NAME = "Overview01";
...
const ENTRY_HISTORY_PLATE_CAMERAS = new Map([
  ["gate lpr 1", "Gate LPR 1"],
  ["gate lpr 2", "Gate LPR 2"],
]);
...
function normalizeEntryHistoryCamera(value) {
  ...
  if (!canonical) throw new Error("Entry history supports only Gate LPR 1 and Gate LPR 2.");
  return canonical;
}
...
function normalizeEntryHistoryCameras(values = ["Gate LPR 1", "Gate LPR 2"]) {
  const input = Array.isArray(values) ? values : [values];
  const cameras = [...new Set(input.map(normalizeEntryHistoryCamera))].sort();
  if (!cameras.length) throw new Error("At least one Gate LPR camera is required.");
  return cameras;
}
...
export class BlueIrisVehicleFrameRepository {
  ...
  async syncBlueIrisCameraInventory(cameras = []) {
    const normalized = [];
    const seen = new Set();
    for (const camera of Array.isArray(cameras) ? cameras : []) {
      const shortName = String(camera?.id || "").trim();
      const displayName = String(camera?.name || camera?.id || "").trim();
      const key = shortName.toLowerCase();
      if (!shortName || !displayName || seen.has(key)) continue;
      seen.add(key);
      normalized.push({
        short_name: shortName,
        display_name: displayName,
        online: camera?.online === true,
        enabled: camera?.enabled !== false,
      });
    }
    if (!normalized.length) return { observed: 0, retired: 0 };

    return this.withTransaction(async (repository) => {
      const payload = JSON.stringify(normalized);
      const upserted = await repository.pool.query(
        `WITH incoming AS (
           SELECT BTRIM(item.short_name) AS short_name,
                  BTRIM(item.display_name) AS display_name,
                  item.online, item.enabled
           FROM jsonb_to_recordset($1::jsonb) AS item(
             short_name text, display_name text, online boolean, enabled boolean
           )
           WHERE NULLIF(BTRIM(item.short_name), '') IS NOT NULL
             AND NULLIF(BTRIM(item.display_name), '') IS NOT NULL
         )
         INSERT INTO public.blue_iris_camera_inventory (
           short_name, display_name, online, enabled, present,
           first_seen_at, last_seen_at, updated_at
         )
         SELECT short_name, display_name, online, enabled, TRUE,
         ...
         ON CONFLICT (short_name) DO UPDATE SET
           ...
           updated_at = CASE WHEN ROW(
             blue_iris_camera_inventory.display_name,
             blue_iris_camera_inventory.online,
             blue_iris_camera_inventory.enabled,
             blue_iris_camera_inventory.present
           ) IS DISTINCT FROM ROW(
             EXCLUDED.display_name, EXCLUDED.online, EXCLUDED.enabled, TRUE
           ) THEN CURRENT_TIMESTAMP ELSE blue_iris_camera_inventory.updated_at END
         RETURNING short_name`,
        [payload],
      );
      const retired = await repository.pool.query(
        `WITH incoming AS (
           SELECT LOWER(BTRIM(item.short_name)) AS short_name
           FROM jsonb_to_recordset($1::jsonb) AS item(short_name text)
           WHERE NULLIF(BTRIM(item.short_name), '') IS NOT NULL
         )
         UPDATE public.blue_iris_camera_inventory inventory
         SET present = FALSE, updated_at = CURRENT_TIMESTAMP
         WHERE inventory.present = TRUE
           AND NOT EXISTS (
             SELECT 1 FROM incoming
             WHERE incoming.short_name = LOWER(BTRIM(inventory.short_name))
           )
         RETURNING inventory.short_name`,
        [payload],
      );
      return {
        observed: upserted.rows?.length || 0,
        retired: retired.rows?.length || 0,
      };
    });
  }
    ...
    return this.withTransaction(async (repository) => {
      ...
      const inserted = await repository.pool.query(
        `INSERT INTO public.vehicle_entry_overview_history_profiles (
           ...
           'Gate Overview','Overview01',$2,$3,3000,$4,TRUE,$5
         )
         RETURNING *`,
  ...
  async previewEntryOverviewBackfillRun({
    startAt,
    endAt,
    plateCameraNames = ["Gate LPR 1", "Gate LPR 2"],
    batchSize = 50,
        ...
        const inserted = await repository.pool.query(
          `INSERT INTO public.vehicle_entry_overview_backfill_jobs (
           ...
           SELECT $1, item.read_id, item.semantic_key, item.profile_id,
                  ...
                  'entry_overview_history', 'entry', 'Gate Overview', 'Overview01',
  ...
  async retryEntryOverviewBackfillJob(jobId) {
    ...
    if (!Number.isSafeInteger(normalizedJobId) || normalizedJobId <= 0) {
      throw new Error("A valid failed Gate Overview import is required.");
    }
    return this.withTransaction(async (repository) => {
      ...
      if (!job) throw new Error("This failed Gate Overview import was not found.");
      if (job.status !== "failed" || job.retryable === true || Number(job.attempt_count) < 2) {
        throw new Error("Only a terminal Gate Overview import failure can be retried.");
      }
      ...
      if (active.rows?.[0]?.has_active_history === true) {
        throw new Error("Wait for the current Gate Overview history work to finish before retrying this import.");
      }
  ...
  async setEntryFallbackMode(mode, {
    ...
    if (normalizedPayloadMode !== null && !["off", "shadow", "active"].includes(normalizedPayloadMode)) {
      throw new Error("Gate Overview route payload mode must be off, shadow, or active.");
    }
    return this.withTransaction(async (repository) => {
      const actorId = Number.isSafeInteger(Number(actor?.id)) && Number(actor.id) > 0
        ? Number(actor.id)
    ...
    const result = await this.pool.query(
      `WITH candidate AS (
         ...
         WHERE (
           ...
           AND LOWER(BTRIM(payload.vehicle_image_selection_metadata->>'sourceCameraName')) = 'gate overview'
      ...
      `WITH copied AS (
         ...
         SET vehicle_image_status = 'ready',
             ...
             vehicle_image_selection_metadata = jsonb_build_object(
               ...
               'sourceCameraName', 'Gate Overview',
               'sourceCameraId', 'Overview01',
               'sourceCameraShortName', 'Overview01',
  ...
  async queueHistorical({ cameraName = null, startDate = null, endDate = null, replaceExisting = false } = {}) {
    if (isEntryHistoryPlateCamera(cameraName)) {
      throw new Error(
        "Gate LPR history must use the dedicated Gate Overview (Overview01) backfill."
      );
    }
    const result = await this.pool.query(
      `UPDATE public.plate_reads
       ...
       WHERE (($4::boolean = FALSE AND vehicle_image_path IS NULL)
          ...
          AND camera_name IS NOT NULL
          AND BTRIM(camera_name) <> ''
          AND LOWER(BTRIM(camera_name)) NOT IN ('gate lpr 1','gate lpr 2')
