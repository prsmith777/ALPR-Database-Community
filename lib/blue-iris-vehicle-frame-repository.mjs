...
export class BlueIrisVehicleFrameRepository {
  ...
  async markTimelineExportDownloaded(exportToken, media = {}) {
    const result = await this.pool.query(
      `UPDATE public.blue_iris_timeline_exports
       SET status = 'delete_pending', remote_uri = COALESCE($2, remote_uri),
  ...
  async markTimelineExportDeleted(exportToken) {
    const result = await this.pool.query(
       `UPDATE public.blue_iris_timeline_exports
  ...
  async markTimelineExportFailed(exportToken, {
    ...
    const result = await this.pool.query(
      `UPDATE public.blue_iris_timeline_exports
        SET status = CASE WHEN $4::boolean AND remote_path IS NOT NULL
                          THEN 'delete_pending' ELSE 'failed' END,
            error_code = $2, error_details = $3::jsonb,
            next_delete_attempt_at = CASE
              WHEN $4::boolean AND remote_path IS NOT NULL
              THEN COALESCE(next_delete_attempt_at, CURRENT_TIMESTAMP + INTERVAL '30 seconds')
  ...
  async claimTimelineExportsNeedingCleanup({ limit = 20 } = {}) {
    const boundedLimit = Math.min(100, Math.max(1, Number.parseInt(String(limit), 10) || 20));
    const result = await this.pool.query(
      `WITH candidates AS (
         SELECT exports.id
         FROM public.blue_iris_timeline_exports exports
         JOIN public.plate_reads reads ON reads.id = exports.read_id
         WHERE exports.remote_path IS NOT NULL
            AND exports.deleted_at IS NULL
            AND exports.delete_attempt_count < 10
            AND COALESCE(exports.next_delete_attempt_at, exports.updated_at) <= CURRENT_TIMESTAMP
           AND (
             exports.status = 'delete_pending'
             OR (exports.status = 'deleting'
               AND exports.updated_at <= CURRENT_TIMESTAMP - INTERVAL '10 minutes')
              OR (exports.status IN ('exporting','ready') AND (
                reads.vehicle_image_claim_token IS DISTINCT FROM exports.claim_token
                OR exports.hard_deadline_at <= CURRENT_TIMESTAMP
                OR COALESCE(reads.vehicle_image_processing_deadline_at, exports.updated_at)
                    <= CURRENT_TIMESTAMP
             ))
           )
         ORDER BY exports.updated_at, exports.id
         FOR UPDATE OF exports SKIP LOCKED
         LIMIT $1
       )
       UPDATE public.blue_iris_timeline_exports exports
        SET status = 'deleting',
            delete_attempt_count = exports.delete_attempt_count + 1,
            next_delete_attempt_at = CURRENT_TIMESTAMP
              + LEAST(INTERVAL '30 minutes',
                INTERVAL '30 seconds' * POWER(2, LEAST(exports.delete_attempt_count, 6))),
            updated_at = CURRENT_TIMESTAMP
       FROM candidates
