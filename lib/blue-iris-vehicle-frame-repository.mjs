...
export class BlueIrisVehicleFrameRepository {
  ...
  async findNearestRead({ cameraName, timestamp, toleranceSeconds = 3 }) {
    const result = await this.pool.query(
      `SELECT id, plate_number, camera_name, "timestamp", image_path, crop_coordinates, vehicle_image_path
    ...
    const result = await this.pool.query(
      `WITH candidate AS (
       ...
       RETURNING reads.id, reads.plate_number, reads.camera_name, reads."timestamp",
                 reads.image_path, reads.crop_coordinates, reads.vehicle_image_path, reads.vehicle_image_queue_kind,
