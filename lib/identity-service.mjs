...
const INVALID_LOGIN_PASSWORD_HASH =
  "$2b$10$Rw4zjygOeCXLOP.dYTJVUeZ9x3IwTMOalCVaEO0zQm26a4AscDRlG";
...
export function hashLoginThrottleSubject(username) {
  return crypto
    .createHash("sha256")
    .update(`named-user:${validateUsername(username)}`, "utf8")
    .digest("hex");
}
...
export class IdentityService {
  ...
  async authenticate({ username, password, userAgent }) {
    ...
    const throttleSubject = hashLoginThrottleSubject(normalizedUsername);
    const existingThrottle = await this.repository.getLoginThrottle(throttleSubject);
    if (
      existingThrottle?.blockedUntil &&
      new Date(existingThrottle.blockedUntil).getTime() > this.now().getTime()
    ) {
      return {
        rateLimited: true,
        retryAfterSeconds: Math.max(
          1,
          Math.ceil(
            (new Date(existingThrottle.blockedUntil).getTime() -
              this.now().getTime()) /
              1000
          )
        ),
    ...
    const passwordMatches = Boolean(
      password &&
        (await this.passwordVerifier(
          String(password),
          user?.password_hash || INVALID_LOGIN_PASSWORD_HASH
        ))
    );
    const valid = Boolean(
      user &&
    ...
    if (!valid) {
      const throttle = await this.repository.recordFailedLogin({
        userId: user?.id || null,
        username: normalizedUsername,
        userAgent: sanitizeUserAgent(userAgent),
        subjectHash: throttleSubject,
        failureLimit: LOGIN_FAILURE_LIMIT,
        windowSeconds: LOGIN_FAILURE_WINDOW_MS / 1000,
        blockSeconds: LOGIN_BLOCK_DURATION_MS / 1000,
      });
      if (throttle?.blocked) {
        return {
          rateLimited: true,
    ...
    await this.repository.clearLoginThrottle(throttleSubject);
    const session = this.createSessionMaterial();
  ...
  async createUser({ actor, username, displayName, password, role }) {
    ...
    const roleName = validateRole(role);
    ...
    return await this.repository.createUser({
      ...
      passwordHash,
      roleName,
      mustChangePassword: roleName !== "administrator",
    });
  }
