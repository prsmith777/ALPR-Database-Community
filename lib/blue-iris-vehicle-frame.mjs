...
export function attachStoredPlateAnchor(candidates, {
  ...
  for (const magnitude of offsetMagnitudes) {
    ...
    for (const offsetMs of frameOffsets) {
      ...
      const best = ranked[0];
      if (!best) continue;
      const threshold = 0.08 + Math.min(1, magnitude / Math.max(1, boundedOffset)) * 0.1;
...
export function selectGuardedVehicleFrame(candidates) {
  ...
  const motionTrack = motionAnchor === anchor
    ? track
    : softTrackCandidates(candidates, motionAnchor);
...
export class BlueIrisVehicleFrameService {
    ...
    for (const offsetMs of offsets) {
      try {
        ...
        const dimensions = {
          ...
          plateBox: offsetMs === 0 ? scaledPlateBox : null,
        };
  ...
  async selectBestFrame({
    ...
    const motionObservation = await this.motionAnalyzer({
      frames,
      track: selection.motionTrack,
      plateBox,
      anchorOffsetMs: selection.motionAnchor?.offsetMs ?? null,
      anchorSource: selection.motionAnchor?.motionAnchorSource
        || (selection.motionAnchor?.detection?.containsPlate ? "stored_plate_containment" : null),
      anchorDistance: selection.motionAnchor?.motionAnchorDistance ?? null,
      imageProcessor: this.imageProcessor,
    });
