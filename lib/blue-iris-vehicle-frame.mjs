...
export class BlueIrisVehicleFrameService {
  ...
  async refetchOverviewFrame({ camera, selected }) {
    ...
    const metadata = await this.imageProcessor(frame.buffer).metadata();
    if (!metadata.width || !metadata.height) {
      throw new BlueIrisError("FINAL_FRAME_INVALID", "Blue Iris final Vehicle View dimensions are unavailable.");
    }
    const stats = await this.imageProcessor(frame.buffer).stats();
    ...
    const eligibility = await assessDirectionImageEligibility(frame.buffer, {
    ...
    const detections = typeof this.detector.detectAll === "function"
      ? await this.detector.detectAll(frame.buffer, dimensions)
      : [await this.detector.detect(frame.buffer, dimensions)].filter(Boolean);
    ...
    if (identitySimilarity < 0.82 || !sameVehicle) {
      throw new BlueIrisError(
        ...
        "The maximum-resolution refetch did not validate as the selected Vehicle View.",
        { details: {
  ...
  async processOverviewRead({ read, profile, camera, alreadyClaimed = false }) {
    ...
    try {
      ...
      for (let attempt = 1; attempt <= OVERVIEW_FINAL_REFETCH_MAX_ATTEMPTS; attempt += 1) {
        try {
        ...
        try {
          const ready = await writerRepository.markReady(read.id, {
            ...
            selectionMetadata: {
              ...
              profileRevision: profile.updated_at || null,
              sourceTimestamp,
              ...
              finalImage: {
                ...
                requestedWidth: OVERVIEW_FINAL_WIDTH,
                requestedHeight: OVERVIEW_FINAL_HEIGHT,
            ...
            claimToken,
            profileSnapshot: { id: profile.id, updatedAt: profile.updated_at || null },
          });
          if (!ready) {
            await this.fileStorage.deleteImage(framePath, null);
            if (typeof writerRepository.releaseOverviewReadClaim === "function") {
              await writerRepository.releaseOverviewReadClaim(read.id, claimToken);
            }
            return false;
          }
        } catch (error) {
      ...
      const freshRecordingRetry = error?.code === "RECORDING_UNAVAILABLE"
        && attemptCount < 2
        && Number.isFinite(readAgeMs)
        && readAgeMs <= 5 * 60_000;
      const terminal = !freshRecordingRetry && [
        "RECORDING_UNAVAILABLE",
        "VEHICLE_NOT_VISIBLE",
        "MULTIPLE_VEHICLES_VISIBLE",
        "NIGHTTIME_UNAVAILABLE",
      ].includes(error?.code);
      const nextAttemptAt = freshRecordingRetry
        ? new Date(Date.now() + 15_000).toISOString()
        : null;
      const failed = await this.repository.markFailed(read.id, {
        status: terminal ? "unavailable" : "failed",
        errorCode: error?.code || "FRAME_SELECTION_FAILED",
        retryable: freshRecordingRetry || !terminal,
        claimToken,
        nextAttemptAt,
        profileSnapshot: { id: profile.id, updatedAt: profile.updated_at || null },
        selectionMetadata: {
          ...
          requestedEndOffsetMs: sampleOffsetsMs.at(-1),
          failure: {
            code: error?.code || "FRAME_SELECTION_FAILED",
            telemetry: error?.details || null,
            boundedRecordingRetry: freshRecordingRetry,
            nextAttemptAt,
          },
        },
      });
      if (!failed) {
        if (typeof this.repository.releaseOverviewReadClaim === "function") {
          await this.repository.releaseOverviewReadClaim(read.id, claimToken);
        }
        return { kind: "overview_read", status: "superseded", readId: Number(read.id) };
      }
      return {
        kind: "overview_read",
        status: freshRecordingRetry ? "retry_scheduled" : terminal ? "unavailable" : "failed",
        readId: Number(read.id),
        errorCode: error?.code || "FRAME_SELECTION_FAILED",
        nextAttemptAt,
      };
