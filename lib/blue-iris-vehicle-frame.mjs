...
export const VEHICLE_FRAME_SELECTION_ALGORITHM = "blue-iris-vehicle-frame-v2";
...
export function vehicleFrameScoreBreakdown(detection, quality = {}) {
  ...
  const completeness = clamp((edgeMargin - 0.002) / 0.038, 0, 1);
  ...
  const rawScore = confidence * 0.18
    ...
    + exposure * 0.1
    + contrast * 0.07;
  const truncationMultiplier = completenessTier === 0 ? 0.35 : completenessTier === 1 ? 0.78 : 1;
  return {
...
function trackedCandidates(candidates, anchor) {
  if (!anchor) return [];
  ...
  const track = [{ ...anchor, trackSimilarity: 1 }];
  const offsets = [...byOffset.keys()]
    .filter((offset) => offset !== anchor.offsetMs)
    .sort((left, right) => Math.abs(left - anchor.offsetMs) - Math.abs(right - anchor.offsetMs));

  for (const offset of offsets) {
    const frameCandidates = byOffset.get(offset) || [];
    if (!anchor.embedding) {
      if (frameCandidates.length === 1) track.push({ ...frameCandidates[0], trackSimilarity: null });
      continue;
    }
    const ranked = frameCandidates.map((candidate) => {
      const similarities = candidate.embedding
        ? track.map((member) => cosineSimilarity(member.embedding, candidate.embedding)).filter(Number.isFinite)
        : [];
      return {
        candidate,
        similarity: similarities.length ? Math.max(...similarities) : null,
      };
    }).filter((entry) => Number.isFinite(entry.similarity))
      .sort((left, right) => right.similarity - left.similarity);
    if (ranked[0]?.similarity >= 0.62) {
      track.push({ ...ranked[0].candidate, trackSimilarity: Number(ranked[0].similarity.toFixed(6)) });
    }
  }
  return track;
}
...
export function selectBestTrackedVehicleFrame(candidates) {
  ...
  const track = trackedCandidates(candidates, anchor);
  const complete = track.filter((candidate) => candidate.scoreBreakdown.completenessTier === 2);
  const nearComplete = track.filter((candidate) => candidate.scoreBreakdown.completenessTier === 1);
  const pool = complete.length ? complete : nearComplete.length ? nearComplete : track;
  const best = [...pool].sort((left, right) => (
    right.score - left.score
    || Number(right.trackSimilarity || 0) - Number(left.trackSimilarity || 0)
    || Math.abs(left.offsetMs) - Math.abs(right.offsetMs)
  ))[0] || null;
  return { best, anchor, trackedCount: track.length };
}
...
export class BlueIrisVehicleFrameService {
  ...
  async analyzeOffsets({ camera, baseMs, offsets, plateBox = null }) {
    const candidates = [];
    const failures = [];
  ...
  async selectBestFrame({ camera, timestamp, plateBox = null }) {
    ...
    const shouldExtend = this.extensionOffsetsMs.length > 0 && (
      !selection.best
      || selection.best.scoreBreakdown.completenessTier < 2
      || selection.best.quality.sharpnessScore < 0.35
      || selection.best.score < 0.62
    );
    if (shouldExtend) {
    ...
    if (selection.best) {
      return {
        best: selection.best,
        sampledCount: this.sampleOffsetsMs.length + (shouldExtend ? this.extensionOffsetsMs.length : 0),
        detectedCount: candidates.length,
        trackedCount: selection.trackedCount,
        anchorOffsetMs: selection.anchor?.offsetMs ?? null,
        expandedSampling: shouldExtend,
      };
    }
    const sampledCount = this.sampleOffsetsMs.length + (shouldExtend ? this.extensionOffsetsMs.length : 0);
