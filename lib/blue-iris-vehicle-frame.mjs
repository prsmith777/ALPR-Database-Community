...
// Overview vehicles may occupy the useful part of the frame for less than a
// second. Sample the bounded six-second Blue Iris alert window at 100 ms while
// retaining only the selected JPEG.
export const OVERVIEW_VEHICLE_FRAME_SAMPLE_OFFSETS_MS = Object.freeze(
  Array.from({ length: 61 }, (_, index) => -500 + index * 100)
);
...
export function selectAnchoredOverviewVehicleFrame(candidates, { toleranceMs = 1_500 } = {}) {
  ...
  const nearestDistanceMs = Math.min(
    ...anchorWindowCandidates.map((candidate) => Math.abs(candidate.offsetMs))
  );
  const anchorCandidates = anchorWindowCandidates.filter(
    (candidate) => Math.abs(candidate.offsetMs) === nearestDistanceMs
  );
  ...
  const completeTrack = track.filter((candidate) => detectionCompletenessTier(candidate.detection) > 0);
  const best = [...completeTrack].sort((left, right) => (
    Number(right.score || 0) - Number(left.score || 0)
    || Number(right.baselineScore || 0) - Number(left.baselineScore || 0)
    || Math.abs(left.offsetMs) - Math.abs(right.offsetMs)
  ))[0] || null;
  return {
    ...
    selectionReason: best ? "overview_anchor_track" : "overview_anchor_track_incomplete",
  };
}
