...
export const VEHICLE_FRAME_SELECTION_ALGORITHM = "blue-iris-vehicle-frame-v3";
...
export function selectBestTrackedVehicleFrame(candidates) {
  ...
  const wellFramed = track.filter((candidate) => candidate.scoreBreakdown.completenessTier === 3);
...
export class BlueIrisVehicleFrameService {
  ...
  async selectBestFrame({ camera, timestamp, plateBox = null }) {
    ...
    const selectionNeedsMoreSearch = (current) => (
      !current.best
      || current.best.scoreBreakdown.completenessTier < 3
      || current.best.quality.sharpnessScore < 0.35
      || current.best.score < 0.62
    );
