...
import {
  ...
  normalizedOverviewContext,
  OVERVIEW_ASSOCIATION_ALGORITHM,
  overviewSourceTimestamp,
  primaryOverviewSourceKind,
} from "./vehicle-overview-association.mjs";

export const VEHICLE_FRAME_SELECTION_ALGORITHM = "blue-iris-vehicle-frame-v7-repair-target-binding";
...
export const OVERVIEW_FINAL_RECOVERY_MAX_CANDIDATES = 7;
export const ENTRY_OVERVIEW_HISTORY_TIMELINE_ALGORITHM_REVISION =
  "entry-overview-history-timeline-v1";
...
function detectionArea(detection) {
  ...
  const normalized = Number(detection.area);
  if (Number.isFinite(normalized) && normalized >= 0) return normalized;
  return Math.max(0, Number(detection.right) - Number(detection.left))
    * Math.max(0, Number(detection.bottom) - Number(detection.top));
}

function finalFrameCompleteness(selectedDetection, finalDetection) {
  const selectedTier = detectionCompletenessTier(selectedDetection);
  const finalTier = detectionCompletenessTier(finalDetection);
  const selectedEdgeMargin = detectionEdgeMargin(selectedDetection);
  const finalEdgeMargin = detectionEdgeMargin(finalDetection);
  const selectedEdgeContacts = detectionEdgeContactCount(selectedDetection);
  const finalEdgeContacts = detectionEdgeContactCount(finalDetection);
  const selectedArea = detectionArea(selectedDetection);
  const finalArea = detectionArea(finalDetection);
  const areaRetention = selectedArea > 0 ? finalArea / selectedArea : null;
  const regressed = finalEdgeContacts > selectedEdgeContacts
    || (selectedTier >= 2 && finalTier < 2)
    || (selectedEdgeMargin >= 0.04
      && finalEdgeMargin < 0.025
      && finalEdgeMargin < selectedEdgeMargin * 0.5)
    || (selectedTier >= 2
      && Number.isFinite(areaRetention)
      && areaRetention < 0.72
      && finalEdgeMargin < selectedEdgeMargin);
  return {
    acceptable: !regressed,
    selectedTier,
    finalTier,
    selectedEdgeMargin,
    finalEdgeMargin,
    selectedEdgeContacts,
    finalEdgeContacts,
    areaRetention,
  };
}

function overviewFinalCandidates(selection) {
  const best = selection?.best || null;
  if (!best) return [];
  const rawTargetSimilarityFloor = selection?.telemetry?.targetCandidateSimilarityFloor;
  const targetSimilarityFloor = Number(rawTargetSimilarityFloor);
  const targetBound = rawTargetSimilarityFloor != null && Number.isFinite(targetSimilarityFloor);
  const seen = new Set();
  const candidates = [];
  const add = (candidate) => {
    const offset = Number(candidate?.offsetMs);
    if (!candidate || !Number.isFinite(offset) || seen.has(offset)) return;
    seen.add(offset);
  ...
  [...(selection?.track || [])]
    .filter((candidate) => candidate !== best
      && (!targetBound || Number(candidate?.targetSimilarity) >= targetSimilarityFloor))
    .sort((left, right) => (
      detectionCompletenessTier(right.detection) - detectionCompletenessTier(left.detection)
      || Number(right.score || 0) - Number(left.score || 0)
      || Math.abs(Number(left.offsetMs) - Number(best.offsetMs))
        - Math.abs(Number(right.offsetMs) - Number(best.offsetMs))
    ))
    .forEach(add);
  return candidates.slice(0, OVERVIEW_FINAL_RECOVERY_MAX_CANDIDATES);
}
...
function overviewAnchorTrackEvaluation(candidates, toleranceMs) {
  ...
  const evaluatedTracks = anchorWindowCandidates.map(
    (anchor) => evaluateOverviewAnchorTrack(candidates, anchor)
  );
  const viableTracks = distinctOverviewTracks(evaluatedTracks.filter((track) => track.viable));
  return { anchorWindowCandidates, evaluatedTracks, viableTracks };
}

function targetTrackEvidence(track, targetEmbedding) {
  const trackedCandidates = track.track.map((candidate) => ({
    ...candidate,
    targetSimilarity: candidate.embedding
      ? cosineSimilarity(targetEmbedding, candidate.embedding)
      : null,
  }));
  const similarities = trackedCandidates.map((candidate) => candidate.targetSimilarity)
    .filter(Number.isFinite)
    .sort((left, right) => right - left);
  const strongest = similarities.slice(0, 3);
  const meanStrongest = strongest.length
    ? strongest.reduce((sum, value) => sum + value, 0) / strongest.length
    : null;
  const targetMaxSimilarity = similarities.length ? similarities[0] : null;
  const candidateSimilarityFloor = Number.isFinite(targetMaxSimilarity)
    ? Math.max(0.5, targetMaxSimilarity - 0.15)
    : 1;
  const targetCandidates = trackedCandidates.filter(
    (candidate) => Number(candidate.targetSimilarity) >= candidateSimilarityFloor
  );
  const best = [...targetCandidates].sort((left, right) => (
    detectionCompletenessTier(right.detection) - detectionCompletenessTier(left.detection)
    || Number(right.score || 0) - Number(left.score || 0)
    || Number(right.targetSimilarity || 0) - Number(left.targetSimilarity || 0)
    || Math.abs(left.offsetMs) - Math.abs(right.offsetMs)
  ))[0] || null;
  return {
    ...track,
    track: trackedCandidates,
    best,
    targetMaxSimilarity,
    targetMeanSimilarity: meanStrongest,
    targetMatchCount: similarities.filter((similarity) => similarity >= 0.35).length,
    targetCandidateCount: targetCandidates.length,
    targetCandidateSimilarityFloor: candidateSimilarityFloor,
  };
}

function targetTrackIsPlausible(track) {
  return Number(track.targetMaxSimilarity) >= 0.5
    && Number(track.targetMeanSimilarity) >= 0.4
    && Number(track.targetCandidateCount) >= 2;
}

export function selectAnchoredOverviewVehicleFrame(candidates, { toleranceMs = 1_500 } = {}) {
  const { anchorWindowCandidates, evaluatedTracks, viableTracks } =
    overviewAnchorTrackEvaluation(candidates, toleranceMs);
...
export function selectTargetedOverviewRepairVehicleFrame(candidates, {
  toleranceMs = 1_500,
  ...
  if (!anchorWindowCandidates.length) {
    return {
      ...
      trackedCount: 0,
      viableTrackCount: 0,
      targetMatchedTrackCount: 0,
      inToleranceCount: 0,
      selectionReason: "overview_repair_target_outside_tolerance",
    };
  }
  if (!targetEmbedding?.length) {
    return {
      ...
      baseline: null,
      challenger: null,
      anchor: anchorWindowCandidates[0],
      track: [],
      trackedCount: 0,
      viableTrackCount: viableTracks.length,
      targetMatchedTrackCount: 0,
      inToleranceCount: anchorWindowCandidates.length,
      selectionReason: "overview_repair_target_reference_unavailable",
    };
  }
  const matchedTracks = viableTracks
    .map((track) => targetTrackEvidence(track, targetEmbedding))
    .filter(targetTrackIsPlausible)
    .sort((left, right) => (
      Number(right.targetMeanSimilarity) - Number(left.targetMeanSimilarity)
      || Number(right.targetMaxSimilarity) - Number(left.targetMaxSimilarity)
      || Number(right.best?.score || 0) - Number(left.best?.score || 0)
    ));
  const bestMatch = matchedTracks[0] || null;
  const runnerUp = matchedTracks[1] || null;
  const targetAmbiguous = Boolean(bestMatch && runnerUp
    && Number(bestMatch.targetMeanSimilarity) - Number(runnerUp.targetMeanSimilarity) < 0.08
    && Number(bestMatch.targetMaxSimilarity) - Number(runnerUp.targetMaxSimilarity) < 0.08);
  if (targetAmbiguous) {
    return {
      status: "ambiguous",
      best: null,
      ...
      trackedCount: 0,
      viableTrackCount: viableTracks.length,
      targetMatchedTrackCount: matchedTracks.length,
      inToleranceCount: anchorWindowCandidates.length,
      selectionReason: "overview_repair_target_ambiguous",
    };
  }
  ...
  return {
    ...
    baseline: null,
    challenger: best,
    anchor: bestMatch?.anchor || anchorWindowCandidates[0],
    track: bestMatch?.track || [],
    trackedCount: bestMatch?.track.length || 0,
    viableTrackCount: viableTracks.length,
    targetMatchedTrackCount: matchedTracks.length,
    targetMaxSimilarity: bestMatch?.targetMaxSimilarity ?? null,
    targetMeanSimilarity: bestMatch?.targetMeanSimilarity ?? null,
    targetMatchCount: bestMatch?.targetMatchCount ?? 0,
    targetCandidateCount: bestMatch?.targetCandidateCount ?? 0,
    targetCandidateSimilarity: best?.targetSimilarity ?? null,
    targetCandidateSimilarityFloor: bestMatch?.targetCandidateSimilarityFloor ?? null,
    inToleranceCount: anchorWindowCandidates.length,
    selectionReason: best
      ? "overview_repair_target_track"
      : evaluatedTracks.length
        ? "overview_repair_target_not_matched"
        : "overview_repair_target_incomplete",
  };
}
...
function normalizedDetectionBox(value) {
  let parsed = value;
  ...
  return { ...box, area: detectionArea(box) };
}
...
function overviewFailureTelemetry({ offsets, candidates, failures, analysis, selection }) {
  ...
  return {
    ...
    targetMatchedTrackCount: Number(selection?.targetMatchedTrackCount || 0),
    targetMaxSimilarity: selection?.targetMaxSimilarity != null
      && Number.isFinite(Number(selection.targetMaxSimilarity))
      ? Number(Number(selection.targetMaxSimilarity).toFixed(6))
      : null,
    targetMeanSimilarity: selection?.targetMeanSimilarity != null
      && Number.isFinite(Number(selection.targetMeanSimilarity))
      ? Number(Number(selection.targetMeanSimilarity).toFixed(6))
      : null,
    targetMatchCount: Number(selection?.targetMatchCount || 0),
    targetCandidateCount: Number(selection?.targetCandidateCount || 0),
    targetCandidateSimilarity: selection?.targetCandidateSimilarity != null
      && Number.isFinite(Number(selection.targetCandidateSimilarity))
      ? Number(Number(selection.targetCandidateSimilarity).toFixed(6))
      : null,
    targetCandidateSimilarityFloor: selection?.targetCandidateSimilarityFloor != null
      && Number.isFinite(Number(selection.targetCandidateSimilarityFloor))
      ? Number(Number(selection.targetCandidateSimilarityFloor).toFixed(6))
      : null,
...
function boundedFinalRefetchFailure(error, attempt) {
  ...
  for (const key of ["selectedCompletenessTier", "finalCompletenessTier", "finalEdgeContacts"]) {
    const value = Number(error?.details?.[key]);
    if (Number.isInteger(value) && value >= 0) details[key] = value;
  }
  return {
    attempt,
    status: "failed",
...
export class BlueIrisVehicleFrameService {
  ...
  async prepareOverviewFramingRepairTarget(read) {
    const imagePath = String(read?.framing_repair_prior_image_path || "").trim();
    const detection = normalizedDetectionBox(read?.framing_repair_prior_detection_box);
    const frozenWidth = Number(read?.framing_repair_prior_image_width);
    const frozenHeight = Number(read?.framing_repair_prior_image_height);
    if (!imagePath || !detection
      || !Number.isInteger(frozenWidth) || frozenWidth <= 0
      || !Number.isInteger(frozenHeight) || frozenHeight <= 0
      || typeof this.fileStorage?.getImage !== "function"
      || typeof this.detector?.embedDetection !== "function") {
      throw new BlueIrisError(
        "OVERVIEW_REPAIR_TARGET_UNAVAILABLE",
        "The frozen prior Vehicle View cannot provide target identity evidence."
      );
    }
    try {
      const buffer = await this.fileStorage.getImage(imagePath);
      if (!buffer?.length) {
        throw new Error("The frozen prior Vehicle View is unavailable.");
      }
      const metadata = await this.imageProcessor(buffer).metadata();
      if (Number(metadata?.width) !== frozenWidth || Number(metadata?.height) !== frozenHeight) {
        throw new Error("The frozen prior Vehicle View dimensions changed.");
      }
      const embedded = await this.detector.embedDetection(buffer, detection, {
        imageWidth: frozenWidth,
        imageHeight: frozenHeight,
      });
      if (!embedded?.embedding?.length) {
        throw new Error("The frozen prior Vehicle View has no vehicle identity embedding.");
      }
      return {
  ...
  async validateOverviewFinalFrame({ buffer, timestamp, selected, mode }) {
    ...
    if (eligibility.monochrome === true) {
      throw new BlueIrisError("NIGHTTIME_UNAVAILABLE", "The configured overview camera is monochrome at night.");
    }
    ...
    const completeness = finalFrameCompleteness(
      selected.detection,
      matchingDetection.detection
    );
    if (!completeness.acceptable) {
      throw new BlueIrisError(
        "FINAL_FRAME_INCOMPLETE",
        "The final Vehicle View clipped a vehicle that was complete in the selected analysis frame.",
        { details: {
          identitySimilarity: Number(identitySimilarity.toFixed(6)),
          ...
          selectedCompletenessTier: completeness.selectedTier,
          finalCompletenessTier: completeness.finalTier,
          selectedEdgeMargin: Number(completeness.selectedEdgeMargin.toFixed(6)),
          finalEdgeMargin: Number(completeness.finalEdgeMargin.toFixed(6)),
          finalEdgeContacts: completeness.finalEdgeContacts,
          areaRetention: completeness.areaRetention == null
            ? null
            : Number(completeness.areaRetention.toFixed(6)),
        } }
      );
    }
    return {
      ...
      detection: matchingDetection.detection,
      detectionBox: detectionBox(matchingDetection.detection),
      detectionConfidence: Number(matchingDetection.detection.confidence || 0),
      identitySimilarity: Number(identitySimilarity.toFixed(6)),
      detectionOverlap: Number(matchingDetection.overlap.toFixed(6)),
      detectionContinuity: Number(matchingDetection.continuity.toFixed(6)),
      completenessTier: completeness.finalTier,
      edgeMargin: Number(completeness.finalEdgeMargin.toFixed(6)),
      edgeContacts: completeness.finalEdgeContacts,
      areaRetention: completeness.areaRetention == null
        ? null
        : Number(completeness.areaRetention.toFixed(6)),
      mode,
        ...
        if (requireColor) {
          ...
          if (eligibility.monochrome === true) {
            ...
            throw new BlueIrisError(
              ...
              "The configured overview camera returned a monochrome nighttime frame."
            );
          }
        }
        successfulSamples += 1;
  ...
  async selectBestFrame({
    ...
    const anchoredSelection = selectionMode === "overview_anchor"
      || selectionMode === "overview_framing_repair";
    const selectFrame = (items) => selectionMode === "overview_framing_repair"
      ? selectTargetedOverviewRepairVehicleFrame(items, {
          toleranceMs: anchorToleranceMs,
          targetEmbedding,
        })
    ...
    const shouldExtend = !anchoredSelection
    ...
    const shouldDeepExtend = !anchoredSelection
    ...
    if (sourceNighttime) {
      throw new BlueIrisError(
        ...
        "The configured overview camera is monochrome at night, so Vehicle View is unavailable.",
        { details: telemetry }
      );
    }
  ...
  async processOverviewRead({
    ...
    const overviewContext = normalizedOverviewContext(profile.overview_context);
    const sourceKind = primaryOverviewSourceKind(profile);
    const profileKind = String(profile.profile_kind || "").trim();
    const historyMode = profileKind === "entry_history";
    const framingRepairMode = profileKind === "framing_repair";
    const exportProfileKind = historyMode
      ? "entry_history"
      : framingRepairMode ? "framing_repair" : null;
    const exportProfileIdentity = historyMode
      ? String(profile.profile_key || profile.profile_identity || "").trim() || null
      : framingRepairMode
        ? String(profile.profile_identity || "").trim() || null
        : null;
    const resultKind = String(lifecycle?.kind || (historyMode
      ? "entry_overview_backfill"
      : "overview_read"));
    const configuredShortName = String(profile.source_camera_short_name || "").trim();
    ...
    const sendHeartbeat = async () => {
      ...
      try {
        const active = typeof lifecycle?.heartbeat === "function"
          ? await lifecycle.heartbeat({ read, claimToken })
          : await this.repository.heartbeatOverviewRead(read.id, claimToken);
    ...
    const canHeartbeat = typeof lifecycle?.heartbeat === "function"
      || typeof this.repository.heartbeatOverviewRead === "function";
    const heartbeatTimer = canHeartbeat
      ? setInterval(() => {
          sendHeartbeat().catch(() => {});
        }, 15_000)
      : null;
    ...
    try {
      if (overviewContext === "entry" && !configuredShortName) {
        throw new BlueIrisError(
          "OVERVIEW_CAMERA_BINDING_INVALID",
          "Entry overview profiles require the Blue Iris camera short name."
        );
      }
      if (configuredShortName
        && configuredShortName.toLowerCase() !== String(camera || "").trim().toLowerCase()) {
        throw new BlueIrisError(
          "OVERVIEW_CAMERA_BINDING_MISMATCH",
          "The configured overview camera does not match its reviewed Blue Iris short name."
        );
      }
      if (canHeartbeat) {
        await sendHeartbeat();
      ...
      assertActive();
      const repairTarget = framingRepairMode
        ? await this.prepareOverviewFramingRepairTarget(read)
        : null;
      assertActive();
      ...
      if (this.timelineExportService) {
        ...
        timelineExport = await this.timelineExportService.acquire({
          ...
          profileIdentity: exportProfileIdentity,
          algorithmRevision: historyMode
            ? ENTRY_OVERVIEW_HISTORY_TIMELINE_ALGORITHM_REVISION
            : undefined,
          assertActive,
        });
      ...
      const selected = await this.selectBestFrame({
        ...
        plateBox: null,
        selectionMode: framingRepairMode ? "overview_framing_repair" : "overview_anchor",
        anchorToleranceMs: profile.tolerance_ms,
        targetEmbedding: repairTarget?.embedding || null,
        sampleOffsetsMs,
        requireColor: true,
        frameProvider,
      });
      assertActive();
      let finalFrame;
      let finalCandidate = selected.best;
      let finalRefetchError = null;
      const finalRefetchAttempts = [];
      if (timelineExport) {
        const candidates = overviewFinalCandidates(selected);
        for (let index = 0; index < candidates.length; index += 1) {
          const candidate = candidates[index];
          const attempt = index + 1;
          try {
            assertActive();
            const selectedOffsetMs = candidate.offsetMs - sampleOffsetsMs[0];
            ...
            finalFrame = await this.validateOverviewFinalFrame({
              ...
              selected: candidate,
              mode: index === 0 ? "timeline_export" : "timeline_export_recovered",
            });
            finalCandidate = candidate;
            assertActive();
            finalRefetchAttempts.push({
              attempt,
              candidateOffsetMs: Number(candidate.offsetMs),
              status: "ready",
              ...
              completenessTier: finalFrame.completenessTier ?? null,
              edgeMargin: finalFrame.edgeMargin ?? null,
            });
            break;
          } catch (error) {
            ...
            finalRefetchAttempts.push({
              ...boundedFinalRefetchFailure(error, attempt),
              candidateOffsetMs: Number(candidate.offsetMs),
            });
          }
        }
        if (!finalFrame) throw finalRefetchError || new BlueIrisError(
          "FINAL_FRAME_INCOMPLETE",
          "No complete full-resolution Vehicle View survived final validation."
        );
      }
      for (let attempt = 1;
      ...
      if (!finalFrame) {
        finalFrame = {
          ...
          detection: selected.best.detection,
          detectionBox: detectionBox(selected.best.detection),
          detectionConfidence: Number(selected.best.detection.confidence || 0),
          completenessTier: detectionCompletenessTier(selected.best.detection),
          edgeMargin: Number(detectionEdgeMargin(selected.best.detection).toFixed(6)),
          edgeContacts: detectionEdgeContactCount(selected.best.detection),
      ...
      const publishedDetection = finalFrame.detection || finalCandidate.detection;
      const framePath = derivedPath(read, finalFrame.timestamp, claimToken);
      const previousPath = read.vehicle_image_path;
      const writeReadyFrame = async (writerRepository = this.repository) => {
        assertActive();
        ...
        try {
          ...
          const readyFrame = {
            framePath,
            frameTimestamp: finalFrame.timestamp,
            frameScore: finalCandidate.score,
            detectionConfidence: finalFrame.detectionConfidence
              ?? publishedDetection.confidence,
            detectionBox: finalFrame.detectionBox || detectionBox(publishedDetection),
            ...
            selectionMetadata: {
              ...
              sourceCameraShortName: configuredShortName || null,
              plateCameraName: read.camera_name,
              directionLabel: historyMode ? null : read.bi_trigger_direction_label,
              anchorMode: historyMode ? "entry_lpr_timestamp" : "direction_profile",
              profileKind: exportProfileKind || "direction_profile",
              backfillRunId: historyMode
                ? Number(
                    read.entry_overview_backfill_run_id
                    || read.entry_history_run_id
                    || 0
                  ) || null
                : null,
              backfillJobId: historyMode
                ? Number(
                    read.entry_overview_backfill_job_id
                    || read.entry_history_job_id
                    || 0
                  ) || null
                : null,
              daylightEvidence: historyMode
                ? read.entry_overview_daylight_evidence || null
                : null,
              ...
              initialSelectedOffsetMs: selected.best.offsetMs,
              selectedOffsetMs: finalCandidate.offsetMs,
              anchorOffsetMs: selected.anchorOffsetMs,
              detectedCount: selected.detectedCount,
              trackedCount: selected.trackedCount,
              trackSimilarity: finalCandidate.trackSimilarity,
              continuityScore: finalCandidate.continuityScore,
              selectionReason: selected.selectionReason,
              targetMaxSimilarity: selected.telemetry?.targetMaxSimilarity ?? null,
              targetMeanSimilarity: selected.telemetry?.targetMeanSimilarity ?? null,
              targetMatchCount: selected.telemetry?.targetMatchCount ?? 0,
              targetCandidateCount: selected.telemetry?.targetCandidateCount ?? 0,
              targetCandidateSimilarity: finalCandidate.targetSimilarity
                ?? selected.telemetry?.targetCandidateSimilarity
                ?? null,
              targetCandidateSimilarityFloor:
                selected.telemetry?.targetCandidateSimilarityFloor ?? null,
              quality: finalCandidate.quality,
              scoreBreakdown: finalCandidate.scoreBreakdown,
              ...
              acquisition: timelineExport ? {
                ...
                completenessTier: finalFrame.completenessTier ?? null,
                edgeMargin: finalFrame.edgeMargin ?? null,
                edgeContacts: finalFrame.edgeContacts ?? null,
                areaRetention: finalFrame.areaRetention ?? null,
                recoveredFromInitialSelection: finalCandidate.offsetMs !== selected.best.offsetMs,
          ...
          const ready = typeof lifecycle?.markReady === "function"
            ...
            : await writerRepository.markReady(read.id, readyFrame, readyOptions);
          if (!ready) {
            await this.fileStorage.deleteImage(framePath, null);
            return false;
          }
      ...
      const failure = {
        ...
        selectionMetadata: {
          ...
          profileKind: exportProfileKind || "direction_profile",
          backfillRunId: historyMode
            ? Number(read.entry_overview_backfill_run_id || read.entry_history_run_id || 0) || null
            : null,
          backfillJobId: historyMode
            ? Number(read.entry_overview_backfill_job_id || read.entry_history_job_id || 0) || null
            : null,
          daylightEvidence: historyMode
            ? read.entry_overview_daylight_evidence || null
            : null,
      ...
      const failed = typeof lifecycle?.markFailed === "function"
        ? await lifecycle.markFailed({ read, failure })
        : await this.repository.markFailed(read.id, failure);
      if (!failed) {
        return { kind: resultKind, status: "superseded", readId: Number(read.id) };
      }
      return {
        kind: resultKind,
  ...
  async associateOverviewCandidate(candidate) {
    const profiles = (await this.repository.listOverviewPairProfiles(candidate.source_camera_name))
      .filter((profile) => (
        ...
        && normalizedOverviewContext(profile.overview_context) === "street"
      ));
    if (!profiles.length) {
