...
export const HOST_MAINTENANCE_IMAGE_POLICY_CONFIRMATION = "SET IMAGE RETIREMENT GRACE";
...
export function normalizeHostMaintenanceInventory(value = {}) {
  ...
  const images = (value.docker?.images || []).map((item) => ({
    ...
    currentWorker: item.currentWorker === true,
    imageClass: item.imageClass === "host-worker" ? "host-worker" : "application",
  }));
  ...
  return {
    ...
    workerImageLedgerComplete: value.workerImageLedgerComplete === true,
    authoritativeCurrentReleaseCount: bytes(value.authoritativeCurrentReleaseCount ?? 0, "Authoritative current release count"),
    authoritativeCurrentWorkerCount: bytes(value.authoritativeCurrentWorkerCount ?? 0, "Authoritative current worker count"),
    leases: {
...
export function buildHostMaintenancePlan(inventoryValue, {
  ...
  const parsedAgeDays = Number.parseInt(String(minimumAgeDays ?? ""), 10);
  const configuredAgeDays = Number.isFinite(parsedAgeDays) ? Math.min(365, parsedAgeDays) : null;
  const keepAgeDays = Math.max(30, configuredAgeDays ?? 30);
  const cacheAgeDays = Math.max(7, configuredAgeDays ?? 7);
  const imageGraceDays = Math.max(1, configuredAgeDays ?? 7);
  ...
  const buildCacheCandidates = inventory.docker.dedicatedNamespace || inventory.docker.dedicatedHost
    ...
    : [];
  const imageGraceCutoff = new Date(new Date(now).getTime() - imageGraceDays * 86_400_000);
  const imageCandidates = inventory.catalogComplete && inventory.releaseLedgerComplete && inventory.workerImageLedgerComplete &&
    inventory.authoritativeCurrentReleaseCount === 1 && inventory.authoritativeCurrentWorkerCount === 1 &&
    inventory.docker.images.filter((image)=>image.currentRelease).length===1 &&
    inventory.docker.images.filter((image)=>image.currentWorker).length===1 &&
    ...
    ? inventory.docker.images.filter((image) =>
        ...
        !image.deployedRelease && !image.currentWorker && image.backupIds.length === 0 &&
