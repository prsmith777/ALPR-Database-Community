...
import { buildHostBackupCatalog } from "./host-backup-catalog.mjs";
import {
...
const DATABASE_BACKUP_CAPABILITY="database-backup-create-v1";
const hasDatabaseBackupCapability=(adapter)=>typeof adapter?.backup==="function"&&Array.isArray(adapter?.capabilities)&&adapter.capabilities.includes(DATABASE_BACKUP_CAPABILITY);
async function heartbeat(pool,inventory,workerId,now,databaseBackupSupported=false){
  await pool.query(`INSERT INTO public.host_maintenance_worker_state
    (environment_id,database_identity,worker_generation,worker_id,heartbeat_at,inventory_revision,inventory_measured_at,database_backup_capability,database_backup_capability_at,last_error,updated_at)
    VALUES($1,$2,$3,$4,$5::timestamptz,$6,$7,$8::varchar(80),CASE WHEN $8::varchar(80) IS NULL THEN NULL ELSE $5::timestamptz END,NULL,$5::timestamptz) ON CONFLICT(environment_id) DO UPDATE SET database_identity=EXCLUDED.database_identity,
      worker_generation=EXCLUDED.worker_generation,worker_id=EXCLUDED.worker_id,heartbeat_at=EXCLUDED.heartbeat_at,
      inventory_revision=EXCLUDED.inventory_revision,inventory_measured_at=EXCLUDED.inventory_measured_at,
      database_backup_capability=EXCLUDED.database_backup_capability,database_backup_capability_at=EXCLUDED.database_backup_capability_at,
      last_error=NULL,updated_at=EXCLUDED.updated_at`,
    [inventory.environmentId,inventory.databaseIdentity,inventory.workerGeneration,String(workerId).slice(0,255),now,inventory.revision,inventory.measuredAt,databaseBackupSupported===true?DATABASE_BACKUP_CAPABILITY:null]);
}
async function persistBackupCatalogSnapshot(pool,inventory,now){
  const catalog=buildHostBackupCatalog(inventory);
  const entries=catalog.entries.map((entry)=>({
    opaque_id:entry.id,identity:entry.identity,bytes:entry.bytes,created_at:entry.createdAt,
    checksum_verified:entry.checksumVerified,checksum_sha256:/^[0-9a-f]{64}$/.test(entry.checksumSha256)?entry.checksumSha256:null,
    explicitly_protected:entry.protected,current_release:entry.currentRelease,rollback_chain:entry.rollbackChain,image_ids:entry.imageIds,
    backup_environment_id:entry.environmentId,backup_database_identity:entry.databaseIdentity,release_id:entry.releaseId,
    schema_version:entry.schemaVersion,postgres_format:entry.postgresFormat,device:entry.device,inode:entry.inode,
    modified_at:entry.modifiedAt,partial:entry.partial,symlink:entry.symlink,hardlink_count:entry.hardlinkCount,
  }));
  const result=await pool.query(`WITH inserted_snapshot AS (
      INSERT INTO public.host_backup_catalog_snapshots
      ...
      ON CONFLICT(environment_id,database_identity,inventory_revision,catalog_revision) DO NOTHING RETURNING id
    ), bound_snapshot AS (
      SELECT id FROM inserted_snapshot UNION ALL
      SELECT id FROM public.host_backup_catalog_snapshots
       WHERE environment_id=$2 AND database_identity=$3 AND inventory_revision=$5 AND catalog_revision=$6
       ORDER BY id DESC LIMIT 1
    ), inserted_entries AS (
      INSERT INTO public.host_backup_catalog_entries
        (snapshot_id,opaque_id,identity,bytes,backup_created_at,checksum_verified,checksum_sha256,explicitly_protected,current_release,
         rollback_chain,image_ids,backup_environment_id,backup_database_identity,release_id,schema_version,postgres_format,device,inode,
         modified_at,partial,symlink,hardlink_count)
      SELECT snapshot.id,entry.opaque_id,entry.identity,entry.bytes,entry.created_at,entry.checksum_verified,entry.checksum_sha256,
        entry.explicitly_protected,entry.current_release,entry.rollback_chain,entry.image_ids,entry.backup_environment_id,
        entry.backup_database_identity,entry.release_id,entry.schema_version,entry.postgres_format,entry.device,entry.inode,
        entry.modified_at,entry.partial,entry.symlink,entry.hardlink_count
      FROM bound_snapshot snapshot CROSS JOIN jsonb_to_recordset($18::jsonb) AS entry(
        opaque_id varchar(200),identity varchar(200),bytes bigint,created_at timestamptz,checksum_verified boolean,checksum_sha256 char(64),
        explicitly_protected boolean,current_release boolean,rollback_chain boolean,image_ids jsonb,backup_environment_id varchar(200),
        backup_database_identity varchar(200),release_id varchar(200),schema_version varchar(200),postgres_format varchar(200),
        device varchar(200),inode varchar(200),modified_at timestamptz,partial boolean,symlink boolean,hardlink_count integer)
      ON CONFLICT(snapshot_id,opaque_id) DO NOTHING RETURNING id
    )
    SELECT snapshot.id,
      (SELECT COUNT(*)::bigint FROM public.host_backup_catalog_entries entries WHERE entries.snapshot_id=snapshot.id)+
      (SELECT COUNT(*)::bigint FROM inserted_entries) AS entry_count
      FROM bound_snapshot snapshot`,[
    catalog.version,catalog.environmentId,catalog.databaseIdentity,catalog.workerGeneration,catalog.inventoryRevision,catalog.catalogRevision,
    catalog.measuredAt,catalog.catalogComplete,catalog.releaseLedgerComplete,catalog.authoritativeCurrentReleaseCount,
    catalog.leases.backupRestore,catalog.leases.build,catalog.leases.deploy,catalog.leases.rollback,catalog.backupCount,catalog.backupBytes,now,
    JSON.stringify(entries),
  ]);
  if(result.rowCount!==1||Number(result.rows[0].entry_count)!==catalog.backupCount)throw new Error("Backup catalog snapshot persistence is incomplete");
  return catalog;
}
...
export async function inspectAndHeartbeatHostMaintenanceWorker({executor,adapter=getHostMaintenanceAdapter(),workerId=`host-maintenance-${process.pid}`,now=new Date(),...bindingInput}={}){
  ...
  try{const inventory=await inspectFresh(adapter,bindings,now);const databaseBackupSupported=hasDatabaseBackupCapability(adapter);
    const backupCatalog=await persistBackupCatalogSnapshot(pool,inventory,now);
    await heartbeat(pool,inventory,workerId,now,databaseBackupSupported);return{status:"healthy",environmentId:bindings.environmentId,workerGeneration:inventory.workerGeneration,inventoryRevision:inventory.revision,
      backupCatalogRevision:backupCatalog.catalogRevision,databaseBackupSupported};}
...
async function processPreview(pool,adapter,intent,now,workerId,bindings){
  ...
  await heartbeat(pool,inventory,workerId,now,hasDatabaseBackupCapability(adapter));
...
async function processPrune(pool,adapter,intent,now,workerId,bindings){
  if(intent.category==="rollout-backups")throw new Error("Rollback-backup deletion is disabled until catalog-bound approval is implemented");
  ...
  await heartbeat(pool,inventory,workerId,now,hasDatabaseBackupCapability(adapter));const current=capPlan(planForCategory(buildHostMaintenancePlan(inventory,{retainedVerifiedCount:config.retained_verified_count,minimumAgeDays:config.minimum_age_days,now}),intent.category));
  ...
  const request={category:intent.category,environmentId:bindings.environmentId,databaseIdentity:bindings.databaseIdentity,workerGeneration:plan.workerGeneration,inventoryRevision:plan.inventoryRevision,candidateSetHash:candidateSetHash(plan),items:plan.items,
    candidateBytes:plan.candidateBytes,maxItems:plan.cap.maxItems,maxBytes:plan.cap.maxBytes,maxDurationSeconds:plan.cap.maxDurationSeconds,
    minimumAgeDays:Number(config.minimum_age_days),deadline:new Date(now.getTime()+plan.cap.maxDurationSeconds*1000).toISOString()};
...
async function writeFailure(client,intent,error,now){const destructive=["execute","scheduled"].includes(intent.intent_type);const detail=rawError(error);
  ...
  await client.query(`INSERT INTO public.audit_events(actor_user_id,source,event_type,resource_type,resource_id,outcome,reason,metadata)
    VALUES($1,'system',$2,'host-maintenance-intent',$3,'failed',$4,jsonb_build_object('category',$5::text,'breakerOpened',$6::boolean))`,
...
const DATABASE_BACKUP_MAX_DURATION_SECONDS=600;
const DATABASE_BACKUP_MAX_BYTES=50*1024**3;
const DATABASE_BACKUP_FILENAME=/^alpr-postgres-[0-9]{8}T[0-9]{6}Z-[0-9]+[.]dump$/;

async function claimDatabaseBackup(pool,workerId,bindings,now){const client=await pool.connect();try{await client.query("BEGIN");
  const result=await client.query(`WITH candidate AS(SELECT id FROM public.host_database_backup_requests
    WHERE status='pending' AND environment_id=$3 AND database_identity=$4 ORDER BY requested_at,id FOR UPDATE SKIP LOCKED LIMIT 1)
    UPDATE public.host_database_backup_requests r SET status='processing',locked_at=$1,locked_by=$2,started_at=$1,updated_at=$1
    FROM candidate WHERE r.id=candidate.id RETURNING r.*`,[now,String(workerId).slice(0,255),bindings.environmentId,bindings.databaseIdentity]);
  await client.query("COMMIT");return result.rows?.[0]||null;}catch(error){try{await client.query("ROLLBACK");}catch{}throw error;}finally{client.release();}}

export function validateDatabaseBackupReceipt(receipt,request){
  if(!receipt||receipt.hostLockHeld!==true||receipt.environmentId!==request.environmentId||receipt.databaseIdentity!==request.databaseIdentity||
    receipt.workerGeneration!==request.workerGeneration||Number(receipt.requestId)!==request.requestId||receipt.format!=="custom")throw new Error("Database-backup receipt binding mismatch");
  if(!DATABASE_BACKUP_FILENAME.test(String(receipt.filename||"")))throw new Error("Database-backup receipt filename is invalid");
  const sizeBytes=Number(receipt.sizeBytes);const durationMs=Number(receipt.durationMs);
  if(!Number.isSafeInteger(sizeBytes)||sizeBytes<=0||sizeBytes>request.maxBytes)throw new Error("Database-backup receipt size is invalid");
  if(receipt.verified!==true||!/^[0-9a-f]{64}$/.test(String(receipt.checksumSha256||"")))throw new Error("Database-backup receipt verification is invalid");
  if(!Number.isSafeInteger(durationMs)||durationMs<0||durationMs>DATABASE_BACKUP_MAX_DURATION_SECONDS*1000)throw new Error("Database-backup receipt duration is invalid");
  const createdAt=new Date(receipt.createdAt);if(Number.isNaN(createdAt.getTime()))throw new Error("Database-backup receipt time is invalid");
  return{filename:receipt.filename,sizeBytes,checksumSha256:receipt.checksumSha256,durationMs,createdAt};
}

async function processDatabaseBackup(pool,adapter,backup,now,workerId,bindings){
  if(!hasDatabaseBackupCapability(adapter))throw new Error("The fixed database-backup interface is not configured");
  const inventory=await inspectFresh(adapter,bindings,now);if(!hasDatabaseBackupCapability(adapter))throw new Error("The fixed database-backup prerequisites changed");
  if(inventory.hostLockAvailable!==true)throw new Error("Shared host maintenance lock is unavailable");
  await heartbeat(pool,inventory,workerId,now,true);
  const request={operation:"postgres-database-backup",format:"custom",requestId:Number(backup.id),environmentId:bindings.environmentId,databaseIdentity:bindings.databaseIdentity,
    workerGeneration:inventory.workerGeneration,maxBytes:DATABASE_BACKUP_MAX_BYTES,deadline:new Date(now.getTime()+DATABASE_BACKUP_MAX_DURATION_SECONDS*1000).toISOString()};
  const receipt=validateDatabaseBackupReceipt(await adapter.backup(request),request);const completedAt=new Date();const client=await pool.connect();try{await client.query("BEGIN");
    const completed=await client.query(`UPDATE public.host_database_backup_requests SET status='completed',completed_at=$3,worker_generation=$4,
      filename=$5,size_bytes=$6,checksum_sha256=$7,verified=TRUE,last_error=NULL,locked_at=NULL,locked_by=NULL,updated_at=$3
      WHERE id=$1 AND status='processing' AND locked_by=$2 RETURNING id`,[backup.id,backup.locked_by,completedAt,inventory.workerGeneration,receipt.filename,receipt.sizeBytes,receipt.checksumSha256]);assertOwned(completed,"database backup completion");
    await client.query(`INSERT INTO public.audit_events(actor_user_id,source,event_type,resource_type,resource_id,outcome,metadata)
      VALUES($1,'system','maintenance.database_backup_completed','database-backup',$2,'succeeded',jsonb_build_object('filename',$3::text,'sizeBytes',$4::bigint,'format','custom','verified',TRUE))`,
      [backup.actor_user_id,String(backup.id),receipt.filename,receipt.sizeBytes]);
    await client.query("COMMIT");return{status:"completed",requestId:Number(backup.id),filename:receipt.filename,sizeBytes:receipt.sizeBytes};
  }catch(error){try{await client.query("ROLLBACK");}catch{}throw error;}finally{client.release();}}

async function failDatabaseBackupAtomic(pool,backup,error,now){const client=await pool.connect();const detail=rawError(error);try{await client.query("BEGIN");await client.query("SET LOCAL lock_timeout='5s'");
  const failed=await client.query(`UPDATE public.host_database_backup_requests SET status='failed',completed_at=$3,last_error=$4,locked_at=NULL,locked_by=NULL,updated_at=$3
    WHERE id=$1 AND status='processing' AND locked_by=$2 RETURNING id`,[backup.id,backup.locked_by,now,detail]);assertOwned(failed,"database backup failure persistence");
  await client.query(`INSERT INTO public.audit_events(actor_user_id,source,event_type,resource_type,resource_id,outcome,reason,metadata)
    VALUES($1,'system','maintenance.database_backup_failed','database-backup',$2,'failed',$3,jsonb_build_object('format','custom'))`,[backup.actor_user_id,String(backup.id),detail]);
  await client.query("COMMIT");}catch(failure){try{await client.query("ROLLBACK");}catch{}throw new Error(`HOST_MAINTENANCE_FATAL_INVARIANT: unable to persist database-backup failure: ${rawError(failure)}`);}finally{client.release();}}

async function requeueDatabaseBackupAtomic(pool,backup,error,now,{increment=true}={}){const client=await pool.connect();const detail=rawError(error);try{await client.query("BEGIN");await client.query("SET LOCAL lock_timeout='5s'");
  const replayed=await client.query(`UPDATE public.host_database_backup_requests SET status='pending',started_at=NULL,completed_at=NULL,worker_generation=NULL,
    replay_count=replay_count+CASE WHEN $4::boolean THEN 1 ELSE 0 END,last_error=NULL,locked_at=NULL,locked_by=NULL,updated_at=$3
    WHERE id=$1 AND status='processing' AND locked_by=$2 AND replay_count+CASE WHEN $4::boolean THEN 1 ELSE 0 END<=2 RETURNING id,replay_count`,
    [backup.id,backup.locked_by,now,increment]);assertOwned(replayed,"database backup replay persistence");
  await client.query(`INSERT INTO public.audit_events(actor_user_id,source,event_type,resource_type,resource_id,outcome,reason,metadata)
    VALUES($1,'system','maintenance.database_backup_replayed','database-backup',$2,'succeeded',$3,jsonb_build_object('replayAttempt',$4::int,'format','custom'))`,
    [backup.actor_user_id,String(backup.id),detail,replayed.rows[0].replay_count]);await client.query("COMMIT");
  }catch(failure){try{await client.query("ROLLBACK");}catch{}throw new Error(`HOST_MAINTENANCE_FATAL_INVARIANT: unable to persist database-backup replay: ${rawError(failure)}`);}finally{client.release();}}

async function exhaustedDatabaseBackupCleanup(adapter,backup,bindings,now){if(typeof adapter?.cleanupDatabaseBackupRequest!=="function")throw new Error("Database-backup replay exhaustion cleanup is unavailable");
  return await adapter.cleanupDatabaseBackupRequest({requestId:Number(backup.id),environmentId:bindings.environmentId,databaseIdentity:bindings.databaseIdentity,
    deadline:new Date(now.getTime()+DATABASE_BACKUP_MAX_DURATION_SECONDS*1000).toISOString()});}

export async function recoverStaleHostMaintenanceLeases({executor,adapter,now=new Date(),leaseSeconds=600,databaseBackupSupported=false,...bindingInput}={}){const pool=executor||await defaultPool();const bindings=expectedBindings(bindingInput);const client=await pool.connect();let recovered=0;
  try{await assertDatabaseIdentity(client,bindings);await client.query("BEGIN");const stale=await client.query(`SELECT * FROM public.host_maintenance_intents WHERE status='processing' AND environment_id=$1 AND database_identity=$4
    ...
    if(databaseBackupSupported===true){const staleBackups=await client.query(`SELECT * FROM public.host_database_backup_requests WHERE status='processing' AND environment_id=$1 AND database_identity=$4
        AND locked_at<$2::timestamptz-make_interval(secs=>$3) ORDER BY id FOR UPDATE SKIP LOCKED`,[bindings.environmentId,now,leaseSeconds,bindings.databaseIdentity]);
      for(const backup of staleBackups.rows){const attempts=Number(backup.replay_count);const detail="Database-backup worker lease expired before an authoritative result";
        if(!Number.isInteger(attempts)||attempts<0||attempts>2)throw new Error("Database-backup replay count is invalid");
        if(attempts>=2){const cleanup=await exhaustedDatabaseBackupCleanup(adapter,backup,bindings,now);
          if(cleanup?.status==="cataloged"){await client.query(`UPDATE public.host_database_backup_requests SET status='pending',started_at=NULL,completed_at=NULL,worker_generation=NULL,last_error=NULL,locked_at=NULL,locked_by=NULL,updated_at=$2 WHERE id=$1`,[backup.id,now]);
            await client.query(`INSERT INTO public.audit_events(actor_user_id,source,event_type,resource_type,resource_id,outcome,reason,metadata)
              VALUES($1,'system','maintenance.database_backup_replayed','database-backup',$2,'succeeded',$3,jsonb_build_object('catalogedRecovery',TRUE,'replayAttempt',$4::int,'format','custom'))`,[backup.actor_user_id,String(backup.id),detail,attempts]);recovered++;continue;}
          if(cleanup?.status!=="cleaned")throw new Error("Database-backup replay exhaustion cleanup was not authoritative");
          await client.query(`UPDATE public.host_database_backup_requests SET status='failed',completed_at=$2,last_error=$3,locked_at=NULL,locked_by=NULL,updated_at=$2 WHERE id=$1`,[backup.id,now,detail]);
          await client.query(`INSERT INTO public.audit_events(actor_user_id,source,event_type,resource_type,resource_id,outcome,reason,metadata)
            VALUES($1,'system','maintenance.database_backup_failed','database-backup',$2,'failed',$3,jsonb_build_object('replayExhausted',TRUE,'replayAttempts',$4::int,'format','custom'))`,[backup.actor_user_id,String(backup.id),detail,attempts]);}
        else{await client.query(`UPDATE public.host_database_backup_requests SET status='pending',started_at=NULL,completed_at=NULL,worker_generation=NULL,replay_count=replay_count+1,last_error=NULL,locked_at=NULL,locked_by=NULL,updated_at=$2 WHERE id=$1`,[backup.id,now]);
          await client.query(`INSERT INTO public.audit_events(actor_user_id,source,event_type,resource_type,resource_id,outcome,reason,metadata)
            VALUES($1,'system','maintenance.database_backup_replayed','database-backup',$2,'succeeded',$3,jsonb_build_object('replayAttempt',$4::int,'format','custom'))`,[backup.actor_user_id,String(backup.id),detail,attempts+1]);}recovered++;}}
...
export async function processNextHostMaintenanceIntent({executor,adapter=getHostMaintenanceAdapter(),workerId=`host-maintenance-${process.pid}`,now=()=>new Date(),...bindingInput}={}){
  const pool=executor||await defaultPool();const bindings=expectedBindings(bindingInput);const databaseBackupSupported=hasDatabaseBackupCapability(adapter);
  await assertDatabaseIdentity(pool,bindings);await recoverStaleHostMaintenanceLeases({executor:pool,adapter,now:now(),databaseBackupSupported,...bindings});
  const backup=databaseBackupSupported?await claimDatabaseBackup(pool,workerId,bindings,now()):null;if(backup){try{return await processDatabaseBackup(pool,adapter,backup,now(),workerId,bindings);}
    catch(error){if(error?.databaseBackupReplayRequired===true){const attempts=Number(backup.replay_count||0);
        if(attempts<2)await requeueDatabaseBackupAtomic(pool,backup,error,now());
        else{const cleanup=await exhaustedDatabaseBackupCleanup(adapter,backup,bindings,now());if(cleanup?.status==="cataloged")await requeueDatabaseBackupAtomic(pool,backup,error,now(),{increment:false});
          else if(cleanup?.status==="cleaned")await failDatabaseBackupAtomic(pool,backup,error,now());else throw new Error("Database-backup replay exhaustion cleanup was not authoritative");}}
      else await failDatabaseBackupAtomic(pool,backup,error,now());throw error;}}
  ...
  try{switch(intent.intent_type){case'preview':return await processPreview(pool,adapter,intent,now(),workerId,bindings);case'execute':case'scheduled':return await processPrune(pool,adapter,intent,now(),workerId,bindings);default:throw new Error("Unknown host-maintenance intent type");}}
...
export const hostMaintenanceWorkerInternals=Object.freeze({assertDatabaseIdentity,capPlan,claimDatabaseBackup,claimIntent,failDatabaseBackupAtomic,failIntentAtomic,persistBackupCatalogSnapshot,processDatabaseBackup,processPreview,processPrune,requeueDatabaseBackupAtomic,validateDatabaseBackupReceipt,validateHostMaintenanceReceipt,writeFailure});
