...
export async function recoverStaleHostMaintenanceLeases({executor,now=new Date(),leaseSeconds=600,...bindingInput}={}){const pool=executor||await defaultPool();const bindings=expectedBindings(bindingInput);const client=await pool.connect();let recovered=0;
  try{await client.query("BEGIN");const stale=await client.query(`SELECT * FROM public.host_maintenance_intents WHERE status='processing' AND environment_id=$1 AND database_identity=$4
...
export async function processNextHostMaintenanceIntent({executor,adapter=getHostMaintenanceAdapter(),workerId=`host-maintenance-${process.pid}`,now=()=>new Date(),...bindingInput}={}){
  const pool=executor||await defaultPool();const bindings=expectedBindings(bindingInput);await recoverStaleHostMaintenanceLeases({executor:pool,now:now(),...bindings});
...
export const hostMaintenanceWorkerInternals=Object.freeze({capPlan,claimIntent,failIntentAtomic,processPreview,processPrune,validateHostMaintenanceReceipt,writeFailure});
