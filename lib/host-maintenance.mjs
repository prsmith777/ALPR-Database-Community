...
export async function inspectAndHeartbeatHostMaintenanceWorker({executor,adapter=getHostMaintenanceAdapter(),workerId=`host-maintenance-${process.pid}`,now=new Date(),...bindingInput}={}){
  ...
  try{const inventory=await inspectFresh(adapter,bindings,now);const databaseBackupSupported=hasDatabaseBackupCapability(adapter);
    await heartbeat(pool,inventory,workerId,now,databaseBackupSupported);return{status:"healthy",environmentId:bindings.environmentId,workerGeneration:inventory.workerGeneration,inventoryRevision:inventory.revision,databaseBackupSupported};}
...
export const hostMaintenanceWorkerInternals=Object.freeze({assertDatabaseIdentity,capPlan,claimDatabaseBackup,claimIntent,failDatabaseBackupAtomic,failIntentAtomic,processDatabaseBackup,processPreview,processPrune,requeueDatabaseBackupAtomic,validateDatabaseBackupReceipt,validateHostMaintenanceReceipt,writeFailure});
