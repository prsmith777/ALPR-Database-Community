...
async function processDatabaseBackup(pool,adapter,backup,now,workerId,bindings){
  ...
  const receipt=validateDatabaseBackupReceipt(await adapter.backup(request),request);const completedAt=new Date();const client=await pool.connect();try{await client.query("BEGIN");
    ...
    await client.query(`INSERT INTO public.audit_events(actor_user_id,source,event_type,resource_type,resource_id,outcome,metadata)
      VALUES($1,'host-worker','maintenance.database_backup_completed','database-backup',$2,'success',jsonb_build_object('filename',$3::text,'sizeBytes',$4::bigint,'format','custom','verified',TRUE))`,
...
async function failDatabaseBackupAtomic(pool,backup,error,now){const client=await pool.connect();const detail=rawError(error);try{await client.query("BEGIN");await client.query("SET LOCAL lock_timeout='5s'");
  ...
  await client.query(`INSERT INTO public.audit_events(actor_user_id,source,event_type,resource_type,resource_id,outcome,reason,metadata)
    VALUES($1,'host-worker','maintenance.database_backup_failed','database-backup',$2,'failed',$3,jsonb_build_object('format','custom'))`,[backup.actor_user_id,String(backup.id),detail]);
...
async function requeueDatabaseBackupAtomic(pool,backup,error,now,{increment=true}={}){const client=await pool.connect();const detail=rawError(error);try{await client.query("BEGIN");await client.query("SET LOCAL lock_timeout='5s'");
  ...
  await client.query(`INSERT INTO public.audit_events(actor_user_id,source,event_type,resource_type,resource_id,outcome,reason,metadata)
    VALUES($1,'host-worker','maintenance.database_backup_replayed','database-backup',$2,'success',$3,jsonb_build_object('replayAttempt',$4::int,'format','custom'))`,
      ...
      for(const backup of staleBackups.rows){const attempts=Number(backup.replay_count);const detail="Database-backup worker lease expired before an authoritative result";
        ...
        if(attempts>=2){const cleanup=await exhaustedDatabaseBackupCleanup(adapter,backup,bindings,now);
          if(cleanup?.status==="cataloged"){await client.query(`UPDATE public.host_database_backup_requests SET status='pending',started_at=NULL,completed_at=NULL,worker_generation=NULL,last_error=NULL,locked_at=NULL,locked_by=NULL,updated_at=$2 WHERE id=$1`,[backup.id,now]);
            await client.query(`INSERT INTO public.audit_events(actor_user_id,source,event_type,resource_type,resource_id,outcome,reason,metadata)
              VALUES($1,'host-worker','maintenance.database_backup_replayed','database-backup',$2,'success',$3,jsonb_build_object('catalogedRecovery',TRUE,'replayAttempt',$4::int,'format','custom'))`,[backup.actor_user_id,String(backup.id),detail,attempts]);recovered++;continue;}
          ...
          await client.query(`INSERT INTO public.audit_events(actor_user_id,source,event_type,resource_type,resource_id,outcome,reason,metadata)
            VALUES($1,'host-worker','maintenance.database_backup_failed','database-backup',$2,'failed',$3,jsonb_build_object('replayExhausted',TRUE,'replayAttempts',$4::int,'format','custom'))`,[backup.actor_user_id,String(backup.id),detail,attempts]);}
          ...
          await client.query(`INSERT INTO public.audit_events(actor_user_id,source,event_type,resource_type,resource_id,outcome,reason,metadata)
            VALUES($1,'host-worker','maintenance.database_backup_replayed','database-backup',$2,'success',$3,jsonb_build_object('replayAttempt',$4::int,'format','custom'))`,[backup.actor_user_id,String(backup.id),detail,attempts+1]);}recovered++;}}
