...
const BOOLEAN_DETAIL_KEYS = new Set([
  ...
  "legacyPushoverSent",
  "duplicateImageAttached",
  "duplicateAlertPointerAttached",
  "duplicateDirectionAttached",
  "duplicateRecognitionAttached",
  "duplicateOverviewQueued",
  "duplicateDirectionNotificationQueued",
]);
...
export function buildLateDuplicateReconciliationEvent({
  readId,
  requestId,
  ingressReceiptId,
  reconciliation = {},
  ...
  const read = reconciliation.read || {};
  return normalizedEvent({
    readId,
    requestId,
    ingressReceiptId,
    stage: "ingress",
    eventType: "read.duplicate_reconciled",
    status: "succeeded",
    component: "plate-read-ingress",
    details: {
      duplicateImageAttached: reconciliation.imageAttached === true,
      duplicateAlertPointerAttached: reconciliation.alertPointerAttached === true,
      duplicateDirectionAttached: reconciliation.directionAttached === true,
      duplicateRecognitionAttached: reconciliation.recognitionAttached === true,
      duplicateOverviewQueued: reconciliation.overviewQueued === true,
      duplicateDirectionNotificationQueued:
        Number(directionNotificationResult?.queued || 0) > 0,
      directionStatus: read.bi_trigger_direction_status,
      directionLabel: read.bi_trigger_direction_label,
      directionErrorCode: read.bi_trigger_direction_error_code,
      directionProfileVersion: read.bi_trigger_direction_profile_version,
      directionAlgorithm: read.bi_trigger_direction_algorithm,
      directionNotificationStatus: directionNotificationResult?.status,
      directionNotificationPlanned: directionNotificationResult?.planned,
      directionNotificationQueued: directionNotificationResult?.queued,
      directionNotificationDuplicates: directionNotificationResult?.duplicates,
      vehicleOverviewStatus: read.vehicle_image_status,
      vehicleOverviewQueueKind: read.vehicle_image_queue_kind,
      vehicleOverviewRetryable: read.vehicle_image_retryable,
      vehicleOverviewErrorCode: read.vehicle_image_error_code,
    },
  });
}
