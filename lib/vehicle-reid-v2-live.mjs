...
const LIVE_AUTHORITY_LOCK = "vehicle_reid_v2_authority_stage2_epoch_2";
const PREDECESSOR_AUTHORITY_LOCKS = Object.freeze([
  "vehicle_reid_v2_authority_stage2",
  LIVE_AUTHORITY_LOCK,
]);
const MAX_PREDECESSOR_AUTHORITY_SESSIONS = 4;
...
export class VehicleReidV2LiveRepository {
  ...
  async fencePredecessorAuthoritySessions() {
    const client = await this.pool.connect();
    let began = false;
    try {
      await client.query("BEGIN");
      ...
      // A prior app process can survive its container long enough to retain an
      // in-flight transaction and its authority/control locks.  Select only
      // older backends holding one of the exact ReID advisory locks; unrelated
      // database sessions are never candidates for termination.
      const holders = await client.query(
        `WITH current_backend AS (
           ...
           SELECT pg_catalog.hashtext(lock_name)::bigint AS lock_key
           FROM unnest($1::text[]) AS names(lock_name)
         )
         ...
         CROSS JOIN current_backend
         JOIN target_locks targets
           ON locks.classid::bigint = ((targets.lock_key >> 32) & 4294967295::bigint)
          AND locks.objid::bigint = (targets.lock_key & 4294967295::bigint)
         WHERE locks.locktype = 'advisory'
           ...
           AND activity.backend_start < current_backend.backend_start
           AND activity.usename = session_user
           AND activity.backend_type = 'client backend'
         ORDER BY locks.pid
         LIMIT $2::integer`,
        [PREDECESSOR_AUTHORITY_LOCKS, MAX_PREDECESSOR_AUTHORITY_SESSIONS + 1]
      );
      const candidatePids = holders.rows.map((row) => Number(row.pid));
      if (candidatePids.length > MAX_PREDECESSOR_AUTHORITY_SESSIONS
          || candidatePids.some((pid) => !Number.isSafeInteger(pid) || pid <= 0)) {
        throw codedError(
          "VEHICLE_REID_V2_AUTHORITY_FENCE_REFUSED",
          "Refusing to fence an unexpected number of ReID authority sessions.",
      ...
      if (candidatePids.length > 0) {
        const terminated = await client.query(
          `WITH current_backend AS (
             ...
             JOIN pg_catalog.pg_locks locks ON locks.pid = activity.pid
             CROSS JOIN current_backend
             JOIN target_locks targets
               ON locks.classid::bigint = ((targets.lock_key >> 32) & 4294967295::bigint)
              AND locks.objid::bigint = (targets.lock_key & 4294967295::bigint)
             WHERE activity.pid = ANY($2::integer[])
           ...
           SELECT COUNT(*)::integer AS candidate_count,
                  COUNT(*) FILTER (
                    WHERE pg_catalog.pg_terminate_backend(pid, 5000)
                  )::integer AS terminated_count
           FROM still_eligible`,
          [PREDECESSOR_AUTHORITY_LOCKS, candidatePids]
        );
        const candidateCount = Number(terminated.rows?.[0]?.candidate_count || 0);
        terminatedCount = Number(terminated.rows?.[0]?.terminated_count || 0);
        if (terminatedCount !== candidateCount) {
          throw codedError(
            "VEHICLE_REID_V2_AUTHORITY_FENCE_FAILED",
            "A predecessor ReID authority session did not terminate cleanly.",
            { candidateCount, terminatedCount }
          );
        }
      }
      ...
      const remaining = await client.query(
        `WITH target_locks AS (
         ...
         SELECT COUNT(DISTINCT locks.pid)::integer AS remaining_count
         FROM pg_catalog.pg_locks locks
         JOIN pg_catalog.pg_stat_activity activity ON activity.pid = locks.pid
         JOIN target_locks targets
           ON locks.classid::bigint = ((targets.lock_key >> 32) & 4294967295::bigint)
        ...
        [PREDECESSOR_AUTHORITY_LOCKS]
      );
      const remainingCount = Number(remaining.rows?.[0]?.remaining_count || 0);
      if (remainingCount !== 0) {
        throw codedError(
          "VEHICLE_REID_V2_AUTHORITY_FENCE_INCOMPLETE",
          "A predecessor ReID authority session remains after startup fencing.",
          { remainingCount }
        );
      }
      ...
      await client.query("COMMIT");
      began = false;
      return { candidateCount: candidatePids.length, terminatedCount, remainingCount };
    } catch (error) {
      if (began) await client.query("ROLLBACK");
      throw error;
    } finally {
  ...
  async loadExistingMember(client, derivativeId) {
    const result = await client.query(
      `SELECT members.*,
              ...
              (current_member.id IS NOT NULL) AS canonical_profile_current
       ...
       LEFT JOIN LATERAL (
         SELECT current_members.id
         FROM public.vehicle_reid_v2_current_profile_members current_members
         WHERE current_members.id = members.id
           AND current_members.canonical_profile_id =
             COALESCE(merges.target_profile_id, members.profile_id)
         OFFSET 0
       ) current_member ON TRUE
  ...
  async loadPlateAnchorProfiles(client, plates) {
    ...
    const result = await client.query(
      `WITH candidate_anchor_ids AS MATERIALIZED (
         SELECT historical.id
         FROM public.vehicle_reid_v2_profile_plate_anchors historical
         WHERE historical.status = 'current'
           AND historical.normalized_plate = ANY($1::varchar[])
       ), bounded_anchors AS MATERIALIZED (
         SELECT anchors.*
         FROM public.vehicle_reid_v2_current_plate_anchors anchors
         WHERE anchors.id = ANY(COALESCE(
           (SELECT ARRAY_AGG(candidate_anchor_ids.id) FROM candidate_anchor_ids),
           ARRAY[]::bigint[]
         ))
           AND anchors.normalized_plate = ANY($1::varchar[])
         OFFSET 0
       ), candidate_profiles AS MATERIALIZED (
         SELECT DISTINCT anchors.canonical_profile_id AS profile_id
         FROM bounded_anchors anchors
       ), candidate_merge_ids AS MATERIALIZED (
         SELECT merges.id
         FROM public.vehicle_reid_v2_profile_merges merges
         JOIN candidate_profiles candidates
           ON candidates.profile_id = merges.target_profile_id
         ...
         WHERE merges.id = ANY(COALESCE(
           (SELECT ARRAY_AGG(candidate_merge_ids.id) FROM candidate_merge_ids),
           ARRAY[]::bigint[]
         ))
           AND merges.target_profile_id = ANY(COALESCE(
             (SELECT ARRAY_AGG(candidate_profiles.profile_id) FROM candidate_profiles),
             ARRAY[]::bigint[]
           ))
         OFFSET 0
       ), candidate_source_profiles AS MATERIALIZED (
         SELECT candidates.profile_id
         FROM candidate_profiles candidates
         UNION
         SELECT merges.source_profile_id FROM exact_merges merges
       ), candidate_member_ids AS MATERIALIZED (
         SELECT members.id
         FROM public.vehicle_reid_v2_profile_members members
         JOIN candidate_source_profiles sources
           ON sources.profile_id = members.profile_id
         WHERE members.status = 'current'
       ), bounded_exact_members AS MATERIALIZED (
         ...
         WHERE exact_members.id = ANY(COALESCE(
           (SELECT ARRAY_AGG(candidate_member_ids.id) FROM candidate_member_ids),
           ARRAY[]::bigint[]
         ))
           AND COALESCE(exact_merges.target_profile_id, exact_members.profile_id)
                 = ANY(COALESCE(
                   (SELECT ARRAY_AGG(candidate_profiles.profile_id)
         ...
         FROM bounded_exact_members members
         ...
         JOIN bounded_exact_members low_members
           ON low_members.derivative_id = reviews.derivative_id_low
         JOIN bounded_exact_members high_members
       ...
       ), current_profiles AS MATERIALIZED (
         SELECT DISTINCT members.canonical_profile_id AS profile_id
         FROM current_members members
       )
       SELECT DISTINCT anchors.canonical_profile_id AS profile_id,
              profiles.status AS profile_status,
              ...
              anchors.normalized_plate
       FROM bounded_anchors anchors
       JOIN current_profiles current
         ON current.profile_id = anchors.canonical_profile_id
       JOIN public.vehicle_reid_v2_profiles profiles
  ...
  async loadCurrentProfileIds(client, profileIds) {
    const requestedIds = [...new Set(profileIds
      .map((profileId) => Number(profileId))
      .filter((profileId) => Number.isSafeInteger(profileId) && profileId > 0))]
      .sort((left, right) => left - right);
    ...
    return new Set((result.rows || []).map((row) => Number(row.profile_id)));
  }
  ...
  async loadHistoricalExactProfiles(client, readId, plate) {
    ...
    const currentProfileIds = await this.loadCurrentProfileIds(
      client,
      rows.map((row) => row.profile_id)
    );
    return rows.filter((row) => currentProfileIds.has(Number(row.profile_id)));
  }

  async loadPairReviewEvidence(client, evidence) {
    const result = await client.query(
      `SELECT reviews.id, reviews.label, reviews.revision,
              current_member.canonical_profile_id AS profile_id,
              profiles.status AS profile_status,
              profiles.revision AS profile_revision, profiles.provenance_basis,
       ...
       LEFT JOIN public.vehicle_reid_v2_current_profile_merges merges
         ON merges.source_profile_id = members.profile_id
       LEFT JOIN LATERAL (
         SELECT current_members.canonical_profile_id
         FROM public.vehicle_reid_v2_current_profile_members current_members
         WHERE current_members.id = members.id
           AND current_members.canonical_profile_id =
             COALESCE(merges.target_profile_id, members.profile_id)
       ...
       LEFT JOIN public.vehicle_reid_v2_profiles profiles
         ON profiles.id = current_member.canonical_profile_id
         AND profiles.status IN ('active','provisional')
...
export const vehicleReidV2LiveInternals = Object.freeze({
  LIVE_AUTHORITY_LOCK,
  PREDECESSOR_AUTHORITY_LOCKS,
  MAX_PREDECESSOR_AUTHORITY_SESSIONS,
  MAX_BATCH_SIZE,
  TRUSTED_PLATE_STATUSES,
  boundedLimit,
  codedError,
