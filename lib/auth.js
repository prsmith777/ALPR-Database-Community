...
function getAuthFilePath() {
  const configuredPath = process.env.ALPR_AUTH_FILE_PATH;
  ...
  if (process.env.NODE_ENV === "test") {
    throw new Error(
      "ALPR_AUTH_FILE_PATH must be set during tests to avoid touching auth/auth.json"
    );
  }

  return path.join(process.cwd(), "auth", "auth.json");
}
...
export async function getAuthConfig() {
  ...
  try {
    const data = await fs.readFile(getAuthFilePath(), "utf-8");
...
export async function requireApiKey(apiKey) {
  if (!apiKey) {
  ...
  try {
    ...
    console.error("API key authentication storage unavailable:", error);
    return {
      ok: false,
      status: 503,
