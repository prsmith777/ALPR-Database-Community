...
import { legacyAdministratorPrincipal } from "./identity-service.mjs";
...
async function resolveNamedSession(sessionId) {
  if (process.env.NEXT_PHASE === "build" || process.env.NODE_ENV === "test") {
    return { principal: null, bootstrapped: false, available: true };
  }
  try {
    const { getIdentityService } = await import("./identity-runtime.mjs");
    const identityService = getIdentityService();
    const principal = await identityService.getSessionPrincipal(sessionId);
    if (principal) return { principal, bootstrapped: true, available: true };
    ...
    console.error("Named session verification unavailable");
    return { principal: null, bootstrapped: false, available: false };
  }
}

export async function getSessionPrincipal(sessionId) {
  if (!isValidSessionId(sessionId)) return false;

  const namedSession = await resolveNamedSession(sessionId);
  if (namedSession.principal) return namedSession.principal;
  if (!namedSession.available || namedSession.bootstrapped) return null;
  ...
  return legacyAdministratorPrincipal({
    id: session.id,
...
export async function verifySession(sessionId) {
  return Boolean(await getSessionPrincipal(sessionId));
}
...
export async function getSessionInfo(sessionId) {
  ...
  const namedSession = await resolveNamedSession(sessionId);
  if (namedSession.principal) {
    const namedPrincipal = namedSession.principal;
    return {
      ...namedPrincipal.session,
      username: namedPrincipal.username,
      displayName: namedPrincipal.displayName,
      roles: namedPrincipal.roles,
      authMode: namedPrincipal.authMode,
    };
  }
  if (!namedSession.available || namedSession.bootstrapped) return null;
...
export async function invalidateSession(sessionId) {
  ...
  if (process.env.NODE_ENV !== "test") {
    try {
      const { getIdentityService } = await import("./identity-runtime.mjs");
      if (await getIdentityService().revokeSession(sessionId)) return true;
    } catch {
      console.error("Named session invalidation unavailable");
    }
  }
