...
export const HELP_MANUAL = Object.freeze({
  ...
  coverageBaseline: "Clean-history Community candidate with portable configuration and no private deployment controls.",
  ...
  sections: [
    ...
    },
    {
      ...
      blocks: [{
        ...
        items: [
          ...
          "For a new empty deployment on a Linux x86-64 host or Linux VM, check out an exact stable tag and run ./alpr-community install. It generates the database password, refuses existing data and port collisions, and completes only after health and empty-database checks pass.",
          "On a standard Linux host or Linux VM, run ./alpr-community from an exact stable release tag to check, install, validate, accept, or roll back an update. Compatibility depends on the Linux guest and Docker Compose, not the hypervisor.",
          ...
          "Rollback verifies its recorded backup, then recreates and restores the public schema inside one database transaction. Partitioned tables, standard schema access grants, extensions, and application objects are restored together or not at all.",
          ...
          "Use npm run migrate:guided -- start for existing-system migrations. The assistant resumes completed checkpoints, stores no passwords, and never switches traffic or deletes the retained source.",
          "Copy image storage separately, preserve relative paths, and do not blindly copy auth, config, or .env secrets.",
    ...
    },
    {
      ...
      blocks: [{
        ...
        items: [
          "The web application has no privileged host maintenance access; routine Linux updates and their one-generation rollback backup run only through the separate host-side updater.",
