...
export const HELP_MANUAL = Object.freeze({
  ...
  sections: [
    ...
    },
    {
      ...
      roles: allRoles(),
      keywords: ["visual search", "vehicle image similarity search", "vehicle crop", "dhash", "duplicate", "camera filter"],
      blocks: [
        {
          type: "steps",
          ...
          items: [
            ...
            "Review plate-confirmed identity separately from visual-only candidates. Visual scores combine structural distance with vehicle-focused color when daylight color is reliable; older assets are upgraded lazily and night images fall back conservatively.",
        ...
        },
        {
          ...
          items: [
            "An administrator uses Index next 20 to process the newest unindexed captures in small resumable batches; ingestion never waits for this work.",
            "In Camera crop setup, an administrator can choose Auto, Custom, or Full frame for each camera. Custom mode provides vehicle-context and vertical-position controls with a live preview from that camera's latest capture.",
            "Saving a camera profile creates a new profile revision and reindexes only that camera's next 20 captures. Tight LPR and wider overview cameras can therefore use different framing.",
            "The index stores a separate derived vehicle-region crop, a SHA-256 source hash for exact duplicates, a 64-bit perceptual hash, and a versioned vehicle-focused color signature.",
            "If an uploaded image exactly matches an indexed capture, its plate identity can confirm and group other captures of that vehicle. Exact duplicate and Plate-confirmed vehicle are identity signals; other percentages remain candidate-ranking aids, not identity proof.",
            "Search compares at most 5,000 recent filtered indexed captures in this foundation release. More advanced cross-angle learned embeddings remain planned.",
          ],
        },
        {
          type: "note",
      ...
      blocks: [
        {
          ...
          items: [
            ...
            "Cross-angle vehicle image similarity using learned embeddings, plus make, model, color, body type, year, and jurisdiction filters.",
