...
export const HELP_MANUAL = Object.freeze({
  ...
  sections: [
    ...
    },
    {
      ...
      blocks: [{
        ...
        items: [
          ...
          "A field marked Managed in .env is read-only in the browser because the host environment overrides stored Settings. Change that value in the private .env file and restart ALPR.",
          "The standard Docker installation keeps Database Password in .env. Installations using an external PostgreSQL service can also manage Host & Port, Name, and User there.",
          "When Database Password is not environment-managed, it is replacement-only. Leaving it blank preserves the saved password; entering a value replaces it.",
          "Change database connection values only when deliberately moving to another PostgreSQL service or following the migration guide.",
    ...
    },
    {
      ...
      blocks: [{
        ...
        items: [
          ...
          "A Blue Iris field marked Managed in .env is read-only in the browser. Change it in the host's private .env file and restart ALPR; unmarked fields remain editable in Settings.",
    ...
    },
    {
      ...
      summary: "Identify the installed build and use the guided check, install, Technical system checks, accept, rollback, and cleanup workflow.",
      roles: ["administrator"],
      keywords: ["release", "software updates", "accept update", "rollback", "Technical system checks", "validation", "update service"],
      ...
      blocks: [{
        ...
        items: [
          ...
          "Installation automatically runs Technical system checks for database readiness, the exact image, application health, row counts, and storage inventory. Run them again only after correcting a reported problem or when support asks you to do so.",
          "When the page says Update installed — acceptance required, manually confirm sign-in, Dashboard and Database search, a test plate read, plate and vehicle images, and health after a restart. Accept update becomes available only after every checkbox is selected and no operation is running.",
          "Technical system checks and acceptance are separate. Accepting completes the current update, permits future installations, and retains one rollback generation until its deadline. The page blocks a newer installation until the current release is accepted or rolled back.",
        ],
      }, {
        type: "note",
        tone: "warning",
    ...
    },
    {
      ...
      blocks: [{
        ...
        items: [
          "For a new empty deployment, use the verified release bootstrap on a listed x86-64 Ubuntu, Debian, RHEL, Rocky Linux, AlmaLinux, CentOS Stream, or Fedora release. Ubuntu Server 24.04 LTS remains the simplest recommended VM target. The bootstrap installs Git, Docker Engine, Compose, Buildx, and private Node.js 24, then checks out an exact stable tag and runs ./alpr-community install with health and empty-database proof.",
          "The exact automatic matrix is Ubuntu 22.04, 24.04, and 26.04; Debian 12 and 13; current RHEL, Rocky Linux, and AlmaLinux 8, 9, and 10; CentOS Stream 9 and 10; and Fedora 43 and 44. Fedora is usable but not preferred for a long-lived server because of its short support cycle. Other x86-64 Linux versions use the read-only compatibility preflight and manual package path. Native Windows Docker and ARM hosts are not currently supported installation targets.",
          ...
          "If the v0.1.36 bootstrap stopped immediately after cloning with Existing Community checkout is not clean, review and preserve the reported directory by moving it aside, then use the newest verified bootstrap. Git cannot distinguish that empty-index state from deliberate staged deletion of every tracked file, so current bootstraps refuse to overwrite or automatically repair it.",
          "Bootstrap release tests execute the real base-package adapter in disposable Ubuntu 24.04, Debian 12, Rocky Linux 9, and AlmaLinux 9 containers. Those checks do not reproduce systemd, enforcing SELinux, or the first Docker group login; Ubuntu Server 24.04 remains the most thoroughly exercised target.",
          "Docker must be a local Linux x86-64 daemon reached through a Unix socket. Remote SSH or TCP contexts cannot serve the required local bind mounts. Keep at least 20 GiB free at the checkout and 10 GiB free on Docker's data-root filesystem.",
        ...
        items: [
          ...
          "On a separate target from the supported Linux matrix, use bootstrap migration-preparation mode, then run ./alpr-community migrate wizard. That mode also installs PostgreSQL 17 clients, rsync, and OpenSSH from the appropriate APT or RPM repositories.",
