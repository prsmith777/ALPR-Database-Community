...
export const HELP_MANUAL = Object.freeze({
  ...
  coverageBaseline: "July 28, 2026 release line: complete Notification Rule lifecycle with independent tag and Known Plate context, tabbed integrations, durable delivery, completed legacy notification retirement, faster read review, read-only Storage Health with scheduled retention previews and bounded filesystem reconciliation, read-only installed release information, Monitored Plates, Vehicle ReID, configurable single-frame vehicle direction calibration with paced and explicitly repeatable historical evaluation, per-read color and automatic coarse vehicle-type observations, independently paginated Vehicle Intelligence review queues, reviewable vehicle profiles, explicit plate associations, and read-only Blue Iris correlation with bounded best-vehicle-frame selection",
    ...
    },
    {
      ...
      blocks: [
        ...
        },
        {
          ...
          text: "ALPR never mounts DrivePool volumes, parses BVR files, changes retention, or deletes recordings. Best-frame selection uses Blue Iris's read-only timeline JPEG endpoint and stores only one derived JPEG per matched read. If the recording has expired, the read receives a terminal unavailable result instead of repeated retrieval attempts. Multiframe direction analysis remains a later phase.",
        },
      ],
    },
    {
