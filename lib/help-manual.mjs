...
export const HELP_MANUAL = Object.freeze({
  ...
  coverageBaseline: "July 24, 2026 release line: unified MQTT, Pushover, SMTP email, and signed webhook notifications with durable delivery and finalizable legacy notification migration; faster read review; read-only Storage Health; Monitored Plates; and Vehicle ReID",
  ...
  sections: [
    ...
    },
    {
      ...
      summary: "Build, preview, and activate unified MQTT, Pushover, email, or signed webhook rules without bypassing migration safeguards.",
      roles: ["administrator"],
      ...
      blocks: [
        {
          ...
          items: [
            "Configure the required delivery channels in Settings > Notifications before testing. SMTP supports authenticated relays or no authentication when both username and password are blank. Webhooks require a signing secret. Never include credentials or secrets in screenshots or support messages.",
            "For email, enter the SMTP host, port, sender, TLS mode, and optional username/password. Keep certificate verification enabled unless you fully control and trust the relay. Save settings, then use Test notification channels on Notifications to send a direct test email.",
            "For webhooks, prefer HTTPS. HTTP and private-network targets are disabled unless an administrator explicitly permits them; loopback, link-local, special-use targets, URL credentials, redirects, and fragments remain blocked. Save settings, then send a direct test to the intended receiver.",
            "Review Monthly message allowance in Settings > Push Notifications or at the top of Notification operations. It shows messages sent, remaining, the current account-wide limit, and the next reset reported by Pushover. Select Refresh usage after changing the application token or sending a test.",
            "Open Notifications. In Unified notification rules, enter a name, optional description, cooldown in seconds, and the explicit rule time zone.",
            ...
            "There is no separate legacy Pushover or MQTT rule editor. Review Unified rules migration preview to see how any existing Pushover and MQTT rules translate into the shared model, then create the disabled copies from that workflow.",
            "Migrated copies remain disabled until their shadow evidence is approved and guarded cutover is explicitly performed. Do not create a separate active unified rule that duplicates an active legacy source, because both paths would deliver.",
            "Use the no-delivery simulator and shadow review before cutover. A matching real accepted read is required before positive rule evidence can be approved.",
            "After cutover, confirm Delivery Activity identifies the unified rule and succeeds. Use the per-rule rollback control if delivery does not behave as approved. When every MQTT and Pushover replacement has a successful post-cutover delivery, Finalize legacy notification migration archives credential-free snapshots, records audit evidence, and permanently removes the disabled legacy rules in one batch.",
            "Review Notification operations for each recent decision, its event-time trace, suppression reason, channel delivery state, retry attempts, and dead-letter error. MQTT, Pushover, email, and webhook delivery failures are handled asynchronously without holding open plate ingestion.",
            "If an unneeded legacy source is deleted before cutover, use Retire orphaned migration only when the unified rule, channel, and actions are all disabled. Retirement preserves the disabled rule, configuration, and audit evidence; it changes no delivery path.",
          ],
        },
        {
          type: "note",
          tone: "warning",
          title: "Unified rules use guarded activation and cutover",
          text: "New builder rules use disabled drafts, no-delivery preview, separate audited activation, atomic deactivation, and protected credential references. Migrated copies still require their existing shadow approval and cutover workflow; the general builder cannot edit or activate them. Unified MQTT, Pushover, email, and webhook actions use durable, deduplicated delivery ledgers with bounded retries and dead-letter state. Webhook receivers should verify the HMAC-SHA256 X-ALPR-Signature against the raw JSON body and deduplicate with Idempotency-Key. Legacy finalization is permanent and becomes available only after verified unified delivery for every remaining migrated MQTT and Pushover source.",
        },
      ],
    },
    {
      ...
      summary: "Connect brokers, map cameras and topics, and test MQTT delivery while managing all rules from Notifications.",
      roles: ["administrator"],
      ...
      blocks: [
        {
          ...
          items: [
            ...
            "Test the broker connection before creating a rule on Notifications. A successful broker test proves connectivity, not that a rule is correct.",
            "Review detected camera mappings and the per-camera topic before enabling automation.",
            "Open Notifications and create a disabled unified rule. Add an MQTT action, select its broker and destination, and configure the trigger and conditions there.",
      ...
      blocks: [
        {
          ...
          items: [
            ...
            "Index-confirmed missing sources counts current failed visual assets with SOURCE_IMAGE_MISSING. Reads without an image path are reported separately; a full orphan-file reconciliation is not performed.",
            "Use Refresh to take a new measurement. Reconciliation, retention, pruning, vacuum, and backup jobs remain future work and require separate approval.",
          ],
        },
        {
          type: "bullets",
