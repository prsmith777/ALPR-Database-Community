...
export const HELP_MANUAL = Object.freeze({
  ...
  coverageBaseline: "July 24, 2026 release line: focused Notification Rules plus dedicated MQTT, Pushover, SMTP email, and signed webhook integration pages; durable delivery; completed legacy notification retirement; faster read review; read-only Storage Health; Monitored Plates; and Vehicle ReID",
      ...
      blocks: [
        {
          ...
          items: [
            "Open Settings > Integrations. MQTT, Pushover, Email, and Webhook each have a dedicated setup page with status, configuration, and the relevant test or activity tools. Never include credentials or secrets in screenshots or support messages.",
            "For email, open Settings > Integrations > Email and enter the SMTP host, port, sender, TLS mode, and optional username/password. Keep certificate verification enabled unless you fully control and trust the relay. Save settings, then send a direct test from the same page.",
            "For webhooks, open Settings > Integrations > Webhook and prefer HTTPS. HTTP and private-network targets are disabled unless an administrator explicitly permits them; loopback, link-local, special-use targets, URL credentials, redirects, and fragments remain blocked. Save settings, then send a direct test from the same page.",
            "For Pushover, open Settings > Integrations > Pushover. The page separates enablement and credentials from delivery defaults, monthly account usage, and a direct test alert. Select Refresh usage after changing the application token or sending a test.",
            "Open Notification Rules from the left sidebar. On the Rules tab, select Create rule and enter a name, optional description, cooldown in seconds, and the explicit rule time zone.",
