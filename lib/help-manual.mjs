...
export const HELP_MANUAL = Object.freeze({
  ...
  coverageBaseline: "July 24, 2026 release line: unified MQTT, Pushover, SMTP email, and signed webhook notifications with durable delivery and finalizable legacy MQTT migration; faster read review; read-only Storage Health; Monitored Plates; and Vehicle ReID",
      ...
      blocks: [
        {
          ...
          items: [
            ...
            "Legacy exact-plate Pushover rows remain available below the builder during transition. Avoid creating an active unified rule that intentionally duplicates an active legacy row unless two messages are desired.",
            "Review Unified rules migration preview to see how current Pushover and MQTT rules translate into the shared model. Copied rules remain disabled until their shadow evidence is approved and guarded cutover is explicitly performed.",
            ...
            "After cutover, confirm Delivery Activity identifies the unified rule and succeeds. Use the per-rule rollback control if delivery does not behave as approved. When every MQTT replacement has a successful post-cutover delivery, Finalize MQTT migration archives a credential-free snapshot, records audit evidence, and permanently removes the disabled legacy MQTT rules.",
