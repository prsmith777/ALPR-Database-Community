...
export class PlateReviewRepository {
  ...
  async getHistory(readId) {
    ...
    const result = await pool.query(
      "SELECT review.id, review.read_id, review.action, review.previous_plate, review.new_plate, " +
        "review.previous_status, review.new_status, review.reason, review.notes, review.actor_user_id, " +
        "review.actor_username, review.actor_display_name, review.alias_id, review.batch_id, " +
        "review.reverses_review_id, review.created_at, " +
        "EXISTS (SELECT 1 FROM public.plate_read_reviews reversal WHERE reversal.reverses_review_id = review.id) AS reversed, " +
        "COALESCE(linked_alias.id, inferred_alias.id) AS related_alias_id, " +
        "COALESCE(linked_alias.source_plate, inferred_alias.source_plate) AS related_alias_source_plate, " +
        "COALESCE(linked_alias.target_plate, inferred_alias.target_plate) AS related_alias_target_plate, " +
        "COALESCE(linked_alias.camera_name, inferred_alias.camera_name) AS related_alias_camera_name, " +
        "COALESCE(linked_alias.enabled, inferred_alias.enabled, FALSE) AS related_alias_enabled " +
        "FROM public.plate_read_reviews review " +
        "JOIN public.plate_reads read ON read.id = review.read_id " +
        "LEFT JOIN public.plate_aliases linked_alias ON linked_alias.id = review.alias_id " +
        "LEFT JOIN LATERAL (" +
          "SELECT alias.id, alias.source_plate, alias.target_plate, alias.camera_name, alias.enabled " +
          "FROM public.plate_aliases alias WHERE review.alias_id IS NULL AND alias.enabled = TRUE " +
          "AND alias.source_plate = COALESCE(read.observed_plate, review.previous_plate) " +
          "AND alias.target_plate = review.new_plate " +
          "AND (alias.camera_name IS NULL OR LOWER(alias.camera_name) = LOWER(read.camera_name)) " +
          "ORDER BY (alias.camera_name IS NOT NULL) DESC, alias.id DESC LIMIT 1" +
        ") inferred_alias ON TRUE " +
        "WHERE review.read_id = $1 ORDER BY review.created_at DESC, review.id DESC",
      [Number(readId)]
    );
    return result.rows;
  }
  ...
  async reverseLatestReview({ readId, reason, actor, disableAliasId = null }) {
    const reviewer = actorSnapshot(actor);
    const reversalReason = requireReason(reason);
    return await this.transaction(async (client) => {
      const selected = await client.query(
        "SELECT id, event_identity, COALESCE(observed_plate, plate_number) AS observed_plate, " +
          "plate_number, camera_name, COALESCE(review_status, 'unreviewed') AS review_status " +
      ...
      let disabledAlias = null;
      if (disableAliasId != null && String(disableAliasId).trim() !== "") {
        const aliasResult = await client.query(
          "SELECT id, source_plate, target_plate, camera_name FROM public.plate_aliases " +
            "WHERE id = $1 AND enabled = TRUE FOR UPDATE",
          [Number(disableAliasId)]
        );
        const alias = aliasResult.rows[0];
        const sameSource = alias?.source_plate === read.observed_plate;
        const sameTarget = alias?.target_plate === prior.new_plate;
        const sameScope = !alias?.camera_name ||
          String(alias.camera_name).trim().toLowerCase() === String(read.camera_name || "").trim().toLowerCase();
        if (!alias || !sameSource || !sameTarget || !sameScope) {
          throw new PlateReviewError(
            "ALIAS_REVIEW_MISMATCH",
            "The selected alias is not the active alias associated with this review."
          );
        }
        await client.query(
          "UPDATE public.plate_aliases SET enabled = FALSE, disabled_at = CURRENT_TIMESTAMP, " +
            "disabled_by_user_id = $2 WHERE id = $1",
          [alias.id, reviewer.id]
        );
        await audit(client, {
          actor: reviewer,
          eventType: "plate.alias_disabled_on_review_reversal",
      ...
      return {
        effectivePlate: restoredPlate,
        reviewStatus: restoredStatus,
        disabledAliasId: disabledAlias?.id || null,
      };
    });
  }
  ...
  async getEnabledAlias({ sourcePlate, cameraName = null }) {
    const source = normalizePlateValue(sourcePlate, "Observed plate");
    const camera = normalizedText(cameraName, 30);
    const pool = await this.getPool();
    const result = await pool.query(
      "SELECT id, source_plate, target_plate, camera_name, enabled, reason, created_at, use_count " +
        "FROM public.plate_aliases WHERE enabled = TRUE AND source_plate = $1 " +
  ...
  async createAlias({ sourcePlate, targetPlate, cameraName = null, reason, actor }) {
    const result = await this.createOrReplaceAlias({
      sourcePlate,
      targetPlate,
      cameraName,
      reason,
  ...
  async createOrReplaceAlias({
    ...
    return await this.transaction(async (client) => {
      const existing = await client.query(
        "SELECT id, source_plate, target_plate, camera_name, reason FROM public.plate_aliases " +
          "WHERE enabled = TRUE AND source_plate = $1 " +
          "AND COALESCE(LOWER(camera_name), '') = COALESCE(LOWER($2), '') LIMIT 1 FOR UPDATE",
        [source, camera]
      );
      if (existing.rowCount) {
        if (existing.rows[0].target_plate === target) {
          return { alias: existing.rows[0], replacedAlias: null, unchanged: true };
        }
        if (!replaceExisting) {
          throw new PlateReviewError(
            "ALIAS_EXISTS",
        ...
        await client.query(
          "UPDATE public.plate_aliases SET enabled = FALSE, disabled_at = CURRENT_TIMESTAMP, " +
            "disabled_by_user_id = $2 WHERE id = $1",
          [existing.rows[0].id, reviewer.id]
        );
        await audit(client, {
          actor: reviewer,
          eventType: "plate.alias_replaced",
          resourceType: "plate_alias",
          resourceId: existing.rows[0].id,
          reason: aliasReason,
          metadata: {
            sourcePlate: source,
            previousTargetPlate: existing.rows[0].target_plate,
            replacementTargetPlate: target,
            cameraName: camera,
          },
        });
      }
      ...
      await audit(client, {
        ...
        metadata: {
          ...
          targetPlate: target,
          cameraName: camera,
          replacesAliasId: existing.rows[0]?.id || null,
        },
      });
      return {
        alias: inserted.rows[0],
        replacedAlias: existing.rows[0] || null,
        unchanged: false,
      };
    });
  }
