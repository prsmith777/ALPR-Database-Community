...
export async function getVehicleReidV2LiveRuntime({
  ...
  if (!state.runtimePromise) {
    state.runtimePromise = (async () => {
      ...
      const startupFence = await repository.fencePredecessorAuthoritySessions();
      logger?.info?.("Authoritative ReID startup authority fence complete", startupFence);
      const service = new VehicleReidV2LiveService({ repository, logger });
      const worker = new VehicleReidV2LiveWorker({ service, logger });
      state.worker = worker;
      return { repository, service, worker, startupFence };
    })().catch((error) => {
