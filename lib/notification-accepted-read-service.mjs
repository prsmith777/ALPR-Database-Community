...
export class NotificationAcceptedReadService {
  ...
  async processAcceptedRead(read = {}) {
    ...
    try {
      ...
      if (groups.size === 0 && pushoverPlans.length === 0) {
        return {
          status: "no-match",
          readId,
          eventId,
      ...
      const status = failures.length === 0
        ? groups.size > 0 ? "queued" : "planned"
        : accepted.length || pushoverPlans.length ? "partial" : "error";
      if (failures.length) {
      ...
      return {
        ...
        readId,
        eventId,
        planned: groups.size + pushoverPlans.length,
        queued,
