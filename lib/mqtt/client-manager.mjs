...
export class MqttClientManager {
  ...
  async discardClient(client) {
    for (const [identity, entry] of this.entries) {
      if (entry.client === client) this.entries.delete(identity);
    }
  ...
  async getClient(broker) {
    ...
    if (existing) {
      ...
      if (client.connected && !client.ended) return client;
      await this.discardClient(client);
    }

    const { options } = this.buildConnection(config);
    safelyLog(this.logger, "info", "Opening MQTT broker connection", {
      broker: redactBrokerConfig(config),
    });

    // Passing an options object avoids mqtt.js's deprecated legacy url.parse()
    // compatibility path on Node 24.
  ...
  async publish({ broker, topic, payload, qos = 1, retain = false } = {}) {
    ...
    try {
      ...
      // mqtt.js queues publishes while offline by default. Once the outbox
      // records a retry, force-close and evict this client so a timed-out
      // packet cannot later escape during an automatic reconnect.
      await this.discardClient(client);
      throw error;
    }
