...
const PROFILE_METHOD_RANK = Object.freeze({
  ocr: 0,
  transposition: 1,
  ordinary: 2,
});

function findBestProfilePlateMatch({
  observedPlate,
  ...
  const settings = normalizePlateMatchingSettings(matchingSettings);
  const resolvedMode = resolvePlateMatchMode(matchMode);

  if (resolvedMode === "off") {
    return noMatch(observedPlate, "fuzzy-disabled");
  }
  ...
  const ranked = normalizedCandidates
    .map((candidate) => {
      ...
      return {
        ...
        methodRank: PROFILE_METHOD_RANK[evaluation.method] ?? 99,
      };
    })
  ...
  if (ranked.length === 0) {
    return noMatch(observedPlate, "outside-tolerance");
  }
  ...
  return {
    ...
    distance: best.distance,
    quality: resolvedMode === "broad" ? "moderate" : "strong",
...
export function findBestPlateMatch(
  ...
  if (matchMode !== undefined && matchMode !== null) {
    return findBestProfilePlateMatch({
      observedPlate,
      normalizedCandidates,
      matchMode,
      matchingSettings,
