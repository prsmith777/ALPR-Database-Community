...
function fuzzyOptionsFromRule(rule) {
  return {
...
function resolveKnownCandidate(rule, observedPlate, knownCandidates) {
  return findBestPlateMatch(observedPlate, knownCandidates, fuzzyOptionsFromRule(rule));
}
...
export function evaluateMqttRule(
  rule,
  { observedPlate: observedValue, camera = null, knownPlates = [] } = {}
) {
  ...
  const match = resolveKnownCandidate(rule, observedPlate, knownCandidates);
  if (match.status !== "exact" && match.status !== "fuzzy") {
    return { ...base, reason: match.reason };
  }
...
export function planMqttPublications({
  ...
  const publications = [...groups.values()].map((group) => {
    const canonicalPlates = uniqueNonEmpty(
      group.matches.map((match) => match.matchedPlateNumber)
    );
    ...
    return {
      ...
      matchMethods: uniqueNonEmpty(
        group.matches.map((match) => match.matchMethod)
      ),
