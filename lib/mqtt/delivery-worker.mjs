...
export class MqttDeliveryWorker {
  ...
  async runBatch() {
    ...
    // Start every claimed camera delivery independently. Promise.all
    // preserves result ordering while preventing one slow publish from
    // delaying unrelated camera observations in the same batch.
    ...
    if (summary.releasedLeases > 0 || summary.claimed > 0) {
