...
export const CURRENT_RELEASE_NOTES = Object.freeze({
  title: "August 21, 2026 reversible ReID v1 producer-stop candidate",
  publishedAt: "2026-08-21",
  items: Object.freeze([
    "Added a default-active, audited Stage 3 control that can stop the retained ReID v1 producer only while authoritative ReID v2 remains primary. Deploying this source leaves production behavior unchanged until an Administrator types the exact stop phrase and confirms the separate operation.",
    "The stop transition serializes current legacy derived-file writers, makes the visual-index worker inert, and blocks new legacy asset, match-review, cluster, assignment, and plate-association writes in both the application and PostgreSQL. Existing rows remain readable and counted for the preservation window.",
    "Stopping or restoring the producer deletes no database row or file. Original LPR images, thumbnails, Overview images, canonical v2 crops and embeddings, plate reads, authoritative profiles and assignments, audit history, and every retained v1 row and derived file remain unchanged.",
    "A v1 consumer rollback is database-blocked while the producer is stopped. An Administrator must type the separate restore phrase, verify the producer is active, and then confirm the existing consumer rollback. Historical export, derived-file cleanup, and table retirement remain later separately approved work.",
    "Added a controlled ReID v2 Stage 2 lifecycle over the immutable Stage 1 preview. Accept verified preview records approval without authority writes; Materialize authoritative ReID revalidates the exact live fingerprint and atomically writes only the projected stable profiles, crop members, reviewed plate anchors, and assigned reads. Evidence drift marks the run stale with zero authority writes.",
    "Added database-enforced, bidirectional reconciliation before a run can complete or the singleton control can enter v2_primary. Profile/member/anchor/assignment counts and provenance must exactly reproduce the accepted projection, and conversion-origin authority is sealed after completion.",
    "Added compatibility routing for v2_primary: Live Feed Vehicle numbers, Find Similar, Vehicle Search, Profiles, Review, and legacy route redirects use current-contract ReID v2 views. Rollback changes consumers to v1_rollback without rewriting plate reads or deleting v1 clusters, assignments, reviews, files, or workers.",
    "Added bounded observable live ReID processing that runs only while v2 is primary. Exact current canonical images, shared assets, trusted reviewed plates, and audited Same decisions can create authority; Different, Unsure, ambiguous multi-plate, stale, missing, rejected, or conflicting evidence remains unassigned with one bounded operator retry after correction. Cosine similarity never establishes identity.",
    "Added audited current profile merge and split history. An exact-current Same review can merge compatible stable profiles, including expansion through a member of an existing merge group; revising that pair to Different or Unsure withdraws its merge. Current consumers fail closed when a source link, embedding, plate revision, anchor, or negative review contract changes.",
    "Expanded the disposable PostgreSQL 17 gate to commit the complete Stage 2 lifecycle: exact accept/materialize/cutover, source-link and plate-revision replacement, plate-only history, provisional live profiles, merge/split/remerge, v1 retention, and rollback. The gate still replays migrations twice in a uniquely named disposable database.",
    "This source candidate extends the deployed Stage 2 baseline with the first Stage 3 producer-stop control, but does not itself merge, deploy to staging or production, or change a production producer state. Historical export, ReID v1 retirement, and deletion remain later separately approved work.",
    "The additive Stage 1 foundation remains intact: snapshot candidate IDs stay evidence references rather than authoritative profile IDs, immutable preview batches remain bounded at 1, 5, 25, or 250 reads, and v1 comparison remains observation only.",
    "Corrected the ReID v2 ungrouped-to-profile suggestion query to read the persisted shadow candidate status and order conflicts by their real deterministic key, fixing the production-schema query mismatch without changing suggestion or identity rules.",
    "ReID v2 Shadow now compares at most the newest 48 currently ungrouped canonical crops with exact-current multi-member candidates from the latest immutable snapshot and displays at most 12 one-profile review suggestions.",
    "Embedding similarity averages the closest two or three current candidate members and never assigns a crop or creates a threshold. Exact plate and human Same evidence waits for a refreshed snapshot; incompatible or ambiguous plates, snapshot conflicts, stale members, and prior Different or Unsure evidence fail closed.",
    "ReID v2 Shadow can now create an immutable evidence-backed profile candidate snapshot from the current canonical crop inventory. Only exact non-rejected effective/corrected plate agreement and audited exact-current human Same-vehicle labels can connect crops; cosine similarity and display-only companion plates never add a member.",
    "Human Different or Unsure labels, incompatible effective plates, ambiguous multi-plate evidence, stale pair-review contracts, rejected plates, stale links, and a truncated bounded scan fail closed. Excluded conflicts remain visible, identical evidence reuses the prior fingerprinted snapshot, and every newly created snapshot is audited.",
    "Shadow profile candidates are separate from current v1 vehicle profiles and create no threshold, cluster, vehicle assignment, notification, or external-provider result.",
    "ReID v2 Shadow now offers one explicit 500-human-review diversity campaign over a frozen current-crop inventory. It excludes crop pairs and effective-plate groups used by earlier reviews, including Balanced-close OCR variants, limits repeated evidence inside each recommendation set, includes Entry Overview crops with their linked LPR and conservative companion evidence, and stops at the real unresolved inventory instead of recycling familiar vehicles.",
    "Exact normalized effective plates, including reviewed corrections, are automatically marked Same vehicle. Clearly dissimilar effective plates outside the existing Balanced fuzzy-match tolerance are automatically marked Different vehicle. Only close OCR variants, missing plates, or incomplete plate evidence consume a human review; automatic outcomes remain visibly distinguished and do not alter cosine ranking.",
    "Campaign progress is durable and labels are bound to its frozen embedding contract and maximum crop ID. The server revalidates every submitted pair against the current diversity queue before counting it. No ReID threshold, profile, cluster, assignment, notification, or external-provider result is created.",
    "ReID v2 Shadow now turns its stratified coverage gaps into a bounded guided review queue. Review targeted pairs opens one exact unlabeled recommendation, keeps the candidate's real embedding rank visible, and advances after each Same, Different, or Unsure label.",
    "The queue is deterministic, excludes previously reviewed and stale crop pairs, and uses effective-plate agreement only to prioritize a stated coverage aim. The aim is not a predicted label; no threshold, profile, cluster, assignment, notification, or external-provider result is created.",
