...
export const CURRENT_RELEASE_NOTES = Object.freeze({
  title: "August 16, 2026 ReID v2 authoritative Stage 1 foundation",
  publishedAt: "2026-08-16",
  items: Object.freeze([
    "Added an additive authoritative ReID v2 ownership foundation with deliberately empty stable profile, canonical-crop member, and plate-read assignment tables. Snapshot candidate IDs remain evidence references and cannot become authoritative profile IDs.",
    "Added a transition control whose single initial row is v2_shadow. Legacy ReID v1 remains the primary identity source: no consumer switches, no v1 producer is stopped, no existing assignment changes, and no v1 data rewrite or deletion occurs in Stage 1.",
    "Added immutable fingerprinted conversion evidence, projected profile/member/read-disposition, visible conflict, bounded job-state, and observation-only v1 comparison contracts plus a deterministic preview projector. Administrators can process preview-only batches of 1, 5, 25, or 250 reads, pause or resume, cancel, retry one listed failure once, and verify frozen fingerprints; no action, worker, or UI accepts or materializes an authoritative profile or assignment.",
    "Conversion projection joins canonical crops only through exact normalized effective-plate agreement or audited exact-current human Same evidence. Human Different or Unsure evidence, clearly different or ambiguous plates, conflicts, stale or replaced links, incomplete evidence, and display-only Entry-to-Street fallbacks fail closed; cosine similarity never establishes identity.",
    "Current read-owned plate-image Visual Search, Profiles, Needs Review, Live Feed Vehicle numbers, Find Similar, and the retained v1 review/calibration controls on those routes are now explicitly labeled legacy so they cannot be mistaken for authoritative ReID v2.",
    ...
    "This source candidate stops before Stage 2. It does not cut over identity, merge a branch, deploy to staging or production, or remove rollback support.",
