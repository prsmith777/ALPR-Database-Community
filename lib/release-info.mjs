...
export const CURRENT_RELEASE_NOTES = Object.freeze({
  title: "August 15, 2026 guarded Overview framing repair",
  publishedAt: "2026-08-15",
  items: Object.freeze([
    "Operator-selected Overview framing repair now binds the six-second search to local Vehicle ReID evidence from the job's frozen prior image, detector box, and dimensions. It can choose a complete frame of that reviewed vehicle while ignoring an unrelated later vehicle; the selected frame and every recovery candidate must satisfy the target-relative identity threshold, and missing or ambiguous evidence fails closed. Normal live Overview ambiguity handling is unchanged.",
    "Framing-repair jobs now use a distinct stable Blue Iris timeline-export identity and may acquire that ledger while the claim-owned original Vehicle View remains Ready. This prevents collision with the original live export, keeps repair retries idempotent, and preserves the current image on every failure.",
    "Added operator-selected repair for direct Overview images with confirmed edge contact or overly tight framing. A saved-pixel preview revalidates the exact current image and frozen acquisition profile; explicit 1, 5, or 25-read batches run one at a time behind live Vehicle View work.",
    "The existing Vehicle View remains current during processing and every failure. A replacement is accepted only with no edge contact, at least a 1.5% margin, and a higher completeness tier; otherwise the attempted file is discarded and the result is Preserved. Blur, difficult exposure, stationary workers, multiple-vehicle scenes, and shared/fallback copies remain review-only and usable when they are the best available evidence.",
