  ...
  try {
    ...
    `WITH interrupted AS (
       ...
       WHERE job_name = $2 AND mode = 'execute' AND status = 'running'
         AND started_at < $1 - make_interval(mins => $4::integer)
       RETURNING id, actor_user_id
     ), marked_items AS (
       UPDATE public.maintenance_cleanup_items item SET
         status = 'failed', error = $3, completed_at = $1
...
export function runScheduledStorageCleanup() {
  return {
    status: "disabled",
    automatic: true,
    categories: [],
    reason: "No automatic cleanup categories are approved in this release.",
  };
}
