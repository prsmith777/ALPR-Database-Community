...
const REFERENCE_CHECK_SQL = `
  SELECT EXISTS (
    ...
    UNION ALL
    SELECT 1 FROM public.vehicle_image_assets WHERE REPLACE(storage_path, E'\\\\', '/') = $1
    UNION ALL
    SELECT 1 FROM public.vehicle_image_derivatives WHERE REPLACE(storage_path, E'\\\\', '/') = $1
  ) AS referenced`;
...
export async function processCleanupCandidateTransaction({
  ...
  try {
    ...
    await applyTimeouts();
    await client.query(
      "LOCK TABLE public.plate_reads, public.capture_assets, public.vehicle_image_assets, public.vehicle_image_derivatives IN SHARE MODE"
    );
