...
import { enqueueDueScheduledHostMaintenance, getHostMaintenanceOverview } from "./host-maintenance-control.mjs";
...
export class StorageMaintenanceMonitor {
  ...
  async runBatch() {
    ...
    await enqueueDueScheduledHostMaintenance({ executor: pool, now: this.now() });
    const hostMaintenance = await getHostMaintenanceOverview({ executor: pool });
    const workerRequired = (hostMaintenance.configs || []).some((config) => config.scheduledEnabled);
    const workerStale = workerRequired && hostMaintenance.worker?.status !== "healthy";
    await alerts.observe({
      eventKey: "maintenance.worker-stale:host-maintenance",
      severity: workerStale ? "critical" : "ok",
      message: workerStale ? "Scheduled host maintenance worker is missing or stale." : "Host maintenance worker liveness is acceptable.",
      details: { required: workerRequired, workerStatus: hostMaintenance.worker?.status || "unavailable", heartbeatAt: hostMaintenance.worker?.heartbeat_at || null },
      settings,
      now: this.now(),
    });
    for (const config of hostMaintenance.configs || []) {
      const latest = (hostMaintenance.runs || []).find((run) => run.configuration?.category === config.category || run.job_name === `host-maintenance:${config.category}`);
      const failed = config.circuitBreakerOpen || latest?.status === "failed";
      await alerts.observe({
        eventKey: `maintenance.job-failure:host-maintenance:${config.category}`,
        severity: failed ? "critical" : "ok",
        message: failed
          ? `Host maintenance ${config.category} is fail-closed: ${config.circuitBreakerReason || latest?.last_error || "unknown failure"}`
          : `Host maintenance ${config.category} is not failed.`,
        details: { category: config.category, status: failed ? "failed" : "ok", nextRunAt: config.nextRunAt },
        settings,
        now: this.now(),
      });
    }
