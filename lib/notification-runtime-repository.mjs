...
export class NotificationRuntimeRepository {
  ...
  async claimDuePushoverDeliveries({ workerId, limit = 10, now = new Date() } = {}) {
    const result = await this.executor.query(`
      WITH due AS (
        SELECT d.id
        FROM public.notification_deliveries d
      ...
      SELECT claimed.*, r.name AS rule_name
      FROM claimed
  ...
  async releaseExpiredPushoverLeases({ leaseMs = 60_000, now = new Date() } = {}) {
    ...
    const result = await this.executor.query(`
      ...
      SET status = 'retry', next_attempt_at = $1, locked_at = NULL, locked_by = NULL,
          last_error = COALESCE(last_error, 'Pushover worker lease expired')
      FROM public.notification_channels ch
      WHERE d.channel_id = ch.id AND ch.channel_type = 'pushover'
  ...
  async recordPushoverSuccess({ deliveryId, workerId, response = {}, now = new Date() } = {}) {
    const result = await this.executor.query(`
      WITH updated AS (
    ...
    if (!result.rows[0]) throw new Error("Pushover delivery success could not be recorded because its worker lease was lost");
    return result.rows[0];
  }

  async recordPushoverFailure({ deliveryId, workerId, error, now = new Date() } = {}) {
    const message = String(error?.message ?? error ?? "Pushover delivery failed").slice(0, 4000);
    const result = await this.executor.query(`
      WITH updated AS (
        UPDATE public.notification_deliveries
        SET attempt_count = attempt_count + 1,
            status = CASE WHEN attempt_count + 1 >= max_attempts THEN 'dead' ELSE 'retry' END,
            next_attempt_at = CASE WHEN attempt_count + 1 >= max_attempts THEN next_attempt_at
      ...
      SELECT updated.* FROM updated JOIN attempted ON attempted.delivery_id = updated.id
    `, [deliveryId, String(workerId), now, message]);
    if (!result.rows[0]) throw new Error("Pushover delivery failure could not be recorded because its worker lease was lost");
    return result.rows[0];
  }
