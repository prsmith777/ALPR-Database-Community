...
function buildWhere(query) {
  ...
  if (query.readId) {
    conditions.push(`${addValue(query.readId)}::bigint = ANY(processed_read_ids)`);
  }
  if (query.cameraName) {
...
export async function queryIntegrationIngressReceipts(queryDatabase, input = {}) {
  ...
  const rowResult = await queryDatabase(
    `SELECT
       ...
       trigger_value_state, trigger_type, heavy_fields, state, received_at,
       completed_at, duration_ms, http_status, outcome, error_code,
       processed_read_ids, processed_count, duplicate_count, ignored_count,
       overview_work_queued, updated_at
     FROM public.integration_ingress_receipts
     ${where.sql}
