...
export class BlueIrisExportDiagnosticService {
  ...
  async check({ actor, token }) {
    ...
    // Deliberately one exact-path status request per explicit administrator click.
    let remote = null;
    try {
      remote = await this.client.getTimelineExport(record.remotePath);
    } catch (error) {
      if (error?.code !== "EXPORT_UNAVAILABLE") throw error;
    }
    record.listed = Boolean(remote);
    record.checkedAt = new Date(this.now()).toISOString();
    if (remote) {
      record.remoteUri = remote.uri || record.remoteUri;
      record.remoteUtc = remote.utc ?? record.remoteUtc;
      record.remoteDurationMs = remote.durationMs ?? record.remoteDurationMs;
  ...
  async download({ actor, token }) {
    const record = this.ownedRecord({ actor, token });
    if (!record.checkedAt) throw new Error("Check the export status once before downloading it.");
