...
export async function processUnifiedPushoverPlans({
  plans = [],
  imageData = null,
  sendPushover,
  logger = console,
  ...
  for (const plan of Array.isArray(plans) ? plans : []) {
    try {
      const plateNumber = normalizePlateNumber(plan?.plateNumber);
      const message = String(plan?.message ?? "").trim() || null;
      const result = await sendPushover(plateNumber, message, imageData, {
        priority: Number(plan?.priority ?? 1),
        title: `${plateNumber} · ${String(plan?.ruleName || "Notification")}`,
      });
      if (result?.success === false) throw new Error(result.error || "Pushover notification failed");
      outcomes.push({ ok: true, ruleId: Number(plan?.ruleId), actionId: Number(plan?.actionId) });
    } catch (error) {
      const normalized = safeError(error);
      safelyLog(logger, "warn", "Unified Pushover notification failed", {
        ruleId: Number(plan?.ruleId) || null,
        actionId: Number(plan?.actionId) || null,
        error: normalized,
      });
      outcomes.push({ ok: false, ruleId: Number(plan?.ruleId), actionId: Number(plan?.actionId), error: normalized });
    }
  }
  return outcomes;
}
