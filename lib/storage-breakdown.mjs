...
export const DEFAULT_HOST_SNAPSHOT_MAX_BYTES = 64 * 1024;
...
export async function readHostStorageSnapshot({
  ...
  maxAgeSeconds = DEFAULT_HOST_SNAPSHOT_MAX_AGE_SECONDS,
  futureToleranceSeconds = DEFAULT_HOST_SNAPSHOT_FUTURE_TOLERANCE_SECONDS,
  maxBytes = DEFAULT_HOST_SNAPSHOT_MAX_BYTES,
} = {}) {
  ...
  try {
    ...
    const size = nonNegativeInteger(info.size, "Host storage snapshot size");
    if (size > Math.max(1024, Number(maxBytes) || DEFAULT_HOST_SNAPSHOT_MAX_BYTES)) {
