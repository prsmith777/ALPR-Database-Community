const STARTUP_STATE = Symbol.for("alpr.vehicle-image-crop.startup.v1");
...
function safeError(error) {
  return {
    ...
    message: String(error?.message || error || "Vehicle crop startup failed").trim().slice(0, 500),
  };
}

async function defaultStarter() {
  const runtime = await import("./vehicle-image-crop-runtime.mjs");
  return runtime.startVehicleImageCropRuntime();
}

export async function startVehicleImageCropRuntimeWithRetry({
    ...
    try {
      ...
      if (!state.retryTimer) {
        state.retryTimer = schedule(() => {
          ...
          return startVehicleImageCropRuntimeWithRetry({
            stateHost, logger, retryDelayMs: delay, startRuntime, schedule,
          });
        }, delay);
        state.retryTimer?.unref?.();
      }
      logger?.error?.("Canonical Overview vehicle crop startup failed; retry scheduled", {
        retryDelayMs: delay,
...
export const vehicleImageCropStartupInternals = Object.freeze({ STARTUP_STATE, safeError, stateFor });
