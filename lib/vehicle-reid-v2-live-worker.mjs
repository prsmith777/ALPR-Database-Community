...
export class VehicleReidV2LiveWorker {
  ...
  async runOnce() {
    // One authority transaction per duty window keeps the rollback lock and
    // interactive consumers responsive while a pre-cutover backlog drains.
    const result = await this.service.processBatch({ limit: 1 });
    this.lastBatch = {
    ...
    if (result.processed > 0) return 2_000;
