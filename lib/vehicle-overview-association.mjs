...
export const OVERVIEW_CONTEXTS = Object.freeze(["street", "entry"]);

export function normalizedOverviewContext(value) {
  return String(value || "street").trim().toLowerCase() === "entry"
    ? "entry"
    : "street";
}

export function primaryOverviewSourceKind(profile = {}) {
  const explicitSourceKind = String(profile.source_kind ?? profile.sourceKind ?? "").trim();
  if (explicitSourceKind === "entry_overview_history") return explicitSourceKind;
  return normalizedOverviewContext(profile.overview_context ?? profile.overviewContext) === "entry"
