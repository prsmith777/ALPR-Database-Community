...
export function failureDisposition(error, item) {
  ...
  if (details.code === "VEHICLE_IMAGE_ASSET_SOURCE_MISSING") {
    const retryLimit = Math.max(1, Number(item?.retry_limit || 3));
    const exhausted = Number(item?.attempt_count || 0) >= retryLimit;
    return {
...
export class VehicleImageAssetCatalogCampaignService {
  constructor({ repository, catalog, liveCatalog = null, logger = console } = {}) {
  ...
  async createPreview({ actorUserId }) {
    const live = await this.liveCatalog?.getOverview?.();
    if (live?.enabled === true) {
      throw new Error(
        "Disable automatic canonical Overview cataloging before creating an operator preview."
      );
    }
  ...
  async getOverview() {
    const [overview, live] = await Promise.all([
      ...
      this.liveCatalog?.getOverview?.() ?? null,
    ]);
    return { ...overview, live };
  }

  async setLiveEnabled(input) {
    if (!this.liveCatalog?.setEnabled) {
      throw new Error("Automatic canonical Overview catalog is unavailable.");
    }
    return this.liveCatalog.setEnabled(input);
  }

  async retryLiveJob(input) {
    if (!this.liveCatalog?.retryJob) {
      throw new Error("Automatic canonical Overview catalog is unavailable.");
    }
    return this.liveCatalog.retryJob(input);
  }
