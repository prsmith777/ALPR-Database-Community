...
export class VehicleReidV2ShadowRepository {
      ...
      this.query(
        `SELECT members.profile_candidate_id, members.derivative_id,
         ...
         JOIN public.vehicle_reid_v2_profile_candidates candidates
          ...
          AND candidates.status = 'candidate'
  ...
  async createProfileCandidateSnapshot({ actor } = {}) {
    ...
    try {
      if (connected) await client.query("BEGIN ISOLATION LEVEL REPEATABLE READ");
      await client.query(
        "SELECT pg_advisory_xact_lock(hashtext('vehicle_reid_v2_profile_candidate_snapshot'))"
      );
