...
export class NotificationOperationsWorker {
  constructor({
    ...
    this.now = now;
    this.logger = logger;
    this.maintenanceAlertWorker = maintenanceAlertWorker;
    this.stopped = false;
  ...
  async runBatch() {
    ...
    const maintenanceAlerts = this.maintenanceAlertWorker
      ? await this.maintenanceAlertWorker.runBatch()
      : { claimed: 0, succeeded: 0, retry: 0, dead: 0 };
    return {
      schedule,
