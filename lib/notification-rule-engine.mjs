...
function wildcardExpression(pattern) {
  const normalized = String(pattern ?? "").toUpperCase().replace(/[^A-Z0-9*?]/g, "");
  ...
  return { normalized, expression: new RegExp(`^${source}$`) };
}

function evaluateAdvancedPlateMatch(actualValue, candidateValue, value, context) {
  const actual = normalizePlateText(actualValue);
  const candidate = normalizePlateText(candidateValue);
  const strategy = normalizeText(value?.strategy || "profile");
  if (!actual || !candidate) {
    return { matched: false, strategy, method: "none", distance: null, reason: "Plate values are required." };
  }
  if (strategy === "exact") {
    return { matched: actual === candidate, strategy, method: actual === candidate ? "exact" : "none", distance: actual === candidate ? 0 : null, reason: actual === candidate ? "Normalized exact match." : "Not an exact match." };
  }
  if (strategy === "contains") {
    const matched = actual.includes(candidate);
    return { matched, strategy, method: matched ? "contains" : "none", distance: null, reason: matched ? "Observed plate contains the configured text." : "Configured text was not found." };
  }
  if (strategy === "wildcard") {
    const wildcard = wildcardExpression(candidateValue);
    const matched = Boolean(wildcard?.expression.test(actual));
    return { matched, strategy, method: matched ? "wildcard" : "none", distance: null, pattern: wildcard?.normalized || "", reason: matched ? "Wildcard pattern matched." : "Wildcard pattern did not match." };
  }
  if (strategy === "ocr_confusion") {
    const result = evaluatePlateIdentityMatch(actual, candidate, "strict", context?.matchingSettings ?? context?.matching_settings ?? {});
    const matched = result.method === "exact" || result.method === "ocr";
    return { ...result, matched, strategy, reason: matched ? result.reason : "Not an exact or OCR-confusion match." };
  }
  if (strategy === "edit_distance") {
    const maximumDistance = Number(value?.maximumDistance ?? value?.maximum_distance ?? 1);
    const distance = editDistance(actual, candidate);
    const matched = Number.isInteger(maximumDistance) && maximumDistance >= 0 && maximumDistance <= 3 && distance <= maximumDistance;
    return { matched, strategy, method: matched ? "edit_distance" : "none", distance, maximumDistance, reason: matched ? `Edit distance ${distance} is within ${maximumDistance}.` : `Edit distance ${distance} exceeds ${maximumDistance}.` };
  }
  return evaluatePlateIdentityMatch(
    actual,
    candidate,
    value?.mode ?? "off",
...
function evaluateTimeWindow(condition, context, value) {
  ...
  const timeZone = String(
    value?.timeZone ?? value?.time_zone ?? context?.ruleTimeZone ?? context?.rule_time_zone ?? "UTC"
  ).trim();
...
export function evaluateNotificationCondition(condition = {}, context = {}) {
  ...
  if (conditionType === "plate_match") {
    ...
    const evaluation = evaluateAdvancedPlateMatch(eventPlate(event), candidate, value, context);
  ...
  if (conditionType === "direction") {
    const actual = firstDefined(event.directionLabel, event.direction_label, "");
    const expected = value?.labels ?? value;
    const matched = compareText(actual, operator || "in", expected);
    if (matched === null) return invalidCondition(condition, "invalid-direction-condition");
    return conditionResult(condition, matched, matched ? "direction-matched" : "direction-mismatch", {
      actual,
      expected,
    });
  }
  ...
  if (conditionType === "known_name") {
    const actual = firstDefined(event.knownName, event.known_name, "");
    const expected = value?.names ?? value?.name ?? value;
    const matched = compareText(actual, operator || "in", expected);
    if (matched === null) return invalidCondition(condition, "invalid-known-name-condition");
    return conditionResult(condition, matched, matched ? "known-name-matched" : "known-name-mismatch", {
      actual,
      expected,
    });
  }
...
export function evaluateNotificationRule(rule = {}, context = {}) {
  ...
  const ruleTimeZone = String(ruleValue(rule, "timeZone", "time_zone", "UTC") || "UTC");
  const evaluationContext = { ...context, ruleTimeZone };
  const trace = evaluateGroup(root, evaluationContext, { nodes: 0 });
  if (trace.invalid) {
  ...
  const quietHours = ruleValue(rule, "quietHours", "quiet_hours", { enabled: false }) || {};
  if (quietHours.enabled) {
    const quietTrace = evaluateTimeWindow(
      { conditionType: "local_time_window", operator: "within" },
      evaluationContext,
      { ...quietHours, timeZone: quietHours.timeZone || quietHours.time_zone || ruleTimeZone }
    );
    if (quietTrace.invalid) {
      return {
        ...base,
        outcome: "invalid",
        matched: false,
        shouldDeliver: false,
        reason: `quiet-hours-${quietTrace.reason}`,
        trace,
        quietHours: quietTrace,
        timeZone: ruleTimeZone,
      };
    }
    if (quietTrace.matched) {
      return {
        ...base,
        outcome: "suppressed",
        matched: true,
  ...
  return {
    ...
    trace,
    timeZone: ruleTimeZone,
    eventTime: safeDate(eventTimestamp(event, context))?.toISOString() || null,
  };
}
