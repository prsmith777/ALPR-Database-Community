...
export async function getStorageMaintenanceOverview() {
  ...
  const [settings, health, runs, alerts, runtime, automatic, postgresMaintenance, applicationConfig] = await Promise.all([
  ...
  return {
    ...
    postgresMaintenance,
    integrations: {
      email: emailConfigurationState(applicationConfig?.notifications?.email || {}),
      webhook: webhookConfigurationState(applicationConfig?.notifications?.webhook || {}),
    },
