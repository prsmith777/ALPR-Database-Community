...
export function createIntegrationRouteHandler(
  ...
  return async function authenticatedIntegrationRoute(request) {
    ...
    if (!authorization.ok) return authenticationErrorResponse(authorization, requestId);
    ...
    if (typeof logger.info === "function") {
      logger.info("integration_request_received", {
        requestId,
        integration,
        routeName,
      });
    } else {
      logger.log?.("integration_request_received", {
        requestId,
        integration,
        routeName,
      });
