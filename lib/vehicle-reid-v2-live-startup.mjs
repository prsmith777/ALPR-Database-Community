const STARTUP_STATE = Symbol.for("alpr.vehicle-reid-v2-live.startup.v1");
...
function safeError(error) {
  return {
    ...
    message: String(error?.message || error || "Authoritative ReID startup failed")
      .trim().slice(0, 500),
  };
}

async function defaultStarter() {
  const runtime = await import("./vehicle-reid-v2-live-runtime.mjs");
  return runtime.startVehicleReidV2LiveRuntime();
}

export async function startVehicleReidV2LiveRuntimeWithRetry({
    ...
    try {
      ...
      if (!state.retryTimer) {
        state.retryTimer = schedule(() => {
          ...
          return startVehicleReidV2LiveRuntimeWithRetry({
            stateHost, logger, retryDelayMs: delay, startRuntime, schedule,
          });
        }, delay);
        state.retryTimer?.unref?.();
      }
      logger?.error?.("Authoritative ReID startup failed; retry scheduled", {
        retryDelayMs: delay,
...
export const vehicleReidV2LiveStartupInternals = Object.freeze({
  STARTUP_STATE,
  safeError,
  stateFor,
});
