...
export function createColorSignature(rgbPixels) {
  ...
  for (let offset = 0; offset < rgbPixels.length; offset += 3) {
    ...
    hue[hueBin] += 0.2 + 0.8 * hsv.saturation;
    saturation[saturationBin] += 1;
    value[valueBin] += 1;
  }
...
export function colorSignatureDistance(leftSignature, rightSignature) {
  ...
  return Number((hueDistance * 0.6 + saturationDistance * 0.25 + valueDistance * 0.15).toFixed(4));
}

export function explainSimilarity({ sourceSha256, candidateSha256, distance, colorDistance = null }) {
  ...
  const score = exact
    ...
    : colorScore === null
      ? structuralScore
      : Number((structuralScore * 0.75 + colorScore * 0.25).toFixed(1));
  return {
    exact,
    ...
    rankingVersion: colorScore === null ? "structure-v1" : "multisignal-v1",
    label: exact
      ? "Exact duplicate"
      : colorScore !== null && score >= 88 && normalizedDistance <= 8
        ? "Very strong visual match"
        : colorScore !== null && score >= 78
          ? "Strong visual match"
