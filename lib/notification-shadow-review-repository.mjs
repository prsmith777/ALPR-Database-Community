...
const GROUPS_QUERY = `
  ...
  JOIN public.notification_rule_migrations m
    ON m.target_rule_id = g.rule_id AND m.retired_at IS NULL AND m.finalized_at IS NULL
...
const ACTIONS_QUERY = `
  ...
  JOIN public.notification_rule_migrations m
    ON m.target_rule_id = a.rule_id AND m.retired_at IS NULL AND m.finalized_at IS NULL
...
const RECENT_READS_QUERY = `
  ...
  LEFT JOIN public.known_plates kp
    ON kp.plate_number = pr.plate_number
   AND kp.created_at <= pr.timestamp
  LEFT JOIN public.plate_tags pt
    ON pt.plate_number = pr.plate_number
   AND pt.created_at <= pr.timestamp
...
const REVIEWS_QUERY = `
  SELECT review.id, review.rule_id, review.rule_version, review.sample_count,
         ...
         review.approval_mode, review.expansion_count, review.regression_count,
...
export class NotificationShadowReviewRepository {
  ...
  async approve({ ruleId, approvalMode = "parity", actor = null, matchingSettings = {} } = {}) {
    ...
    if (!new Set(["parity", "intentional_expansion"]).has(approvalMode)) {
      throw new Error("Select a valid unified rule approval mode");
    }
    ...
    try {
      ...
      if (approvalMode === "parity") {
        ...
        if (rule.regressionCount > 0) {
          throw new Error("Intentional expansion cannot approve a legacy match that unified logic would lose");
        }
        if (rule.expansionCount === 0 || rule.unifiedPositiveMatchCount === 0) {
          throw new Error("Intentional expansion requires at least one real read matched only by unified logic");
        }
      }
        ...
        `
          INSERT INTO public.notification_rule_shadow_reviews
            (rule_id, rule_version, reviewer_user_id, sample_count,
             ...
             approval_mode, expansion_count, regression_count)
          VALUES ($1::bigint, $2, $3::bigint, $4, $5, $6, $7, $8, $9, $10)
      ...
      await client.query(
        ...
        [
          ...
          JSON.stringify({
            ...
            unifiedPositiveMatchCount: rule.unifiedPositiveMatchCount,
            mismatchCount: rule.mismatchCount,
            expansionCount: rule.expansionCount,
