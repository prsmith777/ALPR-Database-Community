...
export async function getPlateReads({
  ...
  return withClient(async (client) => {
    ...
    if (filters.timestampRange?.from && filters.timestampRange?.to) {
      ...
      conditions.push(
        `pr.timestamp::date BETWEEN ${addValue(filters.dateRange.from)} AND ${addValue(filters.dateRange.to)}`
      );
    }
    ...
    const pagedGroupBy = sortsByTags
      ? "GROUP BY pr.id, p.occurrence_count, direction.direction_label"
      : "";
    ...
    const dataQuery = `
      ...
      SELECT 
        ...
        COALESCE(overview_playback.source_camera, overview_candidate.source_camera_name) AS vehicle_bi_camera,
        pr.thumbnail_path,
        pr.timestamp,
        pr.camera_name,
        p.occurrence_count,
