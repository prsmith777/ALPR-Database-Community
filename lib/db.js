...
export async function getPlateReads({
  ...
  return withClient(async (client) => {
    ...
    const requestedReadIds = [...new Set((Array.isArray(filters.readIds)
      ? filters.readIds
      : [])
    ...
    if (requestedReadIds.length > 0) {
      conditions.push(`pr.id = ANY(${addValue(requestedReadIds)}::bigint[])`);
    } else if (Number.isSafeInteger(requestedReadId) && requestedReadId > 0) {
    ...
    const vehicleIdentityResult = await client.query(
      "SELECT mode FROM public.vehicle_reid_control WHERE singleton = TRUE"
    );
    const vehicleIdentityMode = normalizeVehicleIdentityMode(
      vehicleIdentityResult.rows[0]?.mode
    );
    const vehicleIdentitySql = buildPlateReadVehicleIdentitySql(
      vehicleIdentityMode,
      `$${paramIndex + 2}`
    );
