...
export class VehicleReidV2AuthorityRepository {
  ...
  async listProfiles({ page = 1, pageSize = 24, search = "" } = {}) {
      ...
      term
        ? this.query(
          `SELECT DISTINCT anchors.canonical_profile_id
           ...
           WHERE anchors.canonical_profile_id = ANY($1::bigint[])
    ...
    // to an explicit id array before it is expanded.  The base list is small,
    // and resolving merge/search visibility separately prevents PostgreSQL
    // from expanding those views as part of a paginated profile scan.
    const [membersResult, assignmentsResult, anchorsResult] = await Promise.all([
      this.query(
        `WITH page_members AS MATERIALIZED (
           ...
           WHERE members.canonical_profile_id = ANY($1::bigint[])
      ...
      this.query(
        `SELECT assignments.canonical_profile_id,
         ...
         WHERE assignments.canonical_profile_id = ANY($1::bigint[])
