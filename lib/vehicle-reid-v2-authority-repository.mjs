...
import { STORAGE_CLEANUP_LOCK_NAME } from "./storage-maintenance-lock.mjs";
...
async function audit(client, {
  ...
  eventType,
  resourceId,
  resourceType = "vehicle_reid_v2_conversion_run",
  outcome = "succeeded",
  metadata = {},
}) {
  await client.query(
    `INSERT INTO public.audit_events (
       ...
       $1::bigint, 'browser', $2, $3,
       $4, $5, $6::jsonb
     )`,
    [actor?.id || null, eventType, resourceType, String(resourceId), outcome, JSON.stringify(metadata)]
  );
}
...
export class VehicleReidV2AuthorityRepository {
  ...
  async transitionV1Producer({ state, confirmation, actor, reason } = {}) {
    const operator = actorSnapshot(actor);
    const requested = String(state || "").trim();
    if (!["active", "stopped"].includes(requested)) {
      throw codedError(
        "VEHICLE_REID_V1_PRODUCER_STATE",
        "Unsupported ReID v1 producer state."
      );
    }
    const requiredConfirmation = requested === "stopped"
      ? "STOP REID V1 PRODUCER"
      : "RESTORE REID V1 PRODUCER";
    if (String(confirmation || "").trim() !== requiredConfirmation) {
      throw codedError(
        "VEHICLE_REID_V1_PRODUCER_CONFIRMATION",
        `Type ${requiredConfirmation} to confirm this ReID v1 producer transition.`
      );
    }
    return this.transaction(async (client) => {
      await client.query("SET LOCAL lock_timeout = '15s'");
      await client.query("SET LOCAL statement_timeout = '30s'");
      // Wait for every current legacy derived-file writer to finish before the
      // control row changes. New writers acquire the same shared lock and then
      // recheck this state before saving any file or database row.
      await client.query(
      ...
      if (!control) {
        throw codedError(
          "VEHICLE_REID_V2_AUTHORITY_CONTROL",
          "ReID authority control is unavailable."
        );
      }
      if (control.mode !== "v2_primary") {
        throw codedError(
          "VEHICLE_REID_V1_PRODUCER_REQUIRES_V2",
          "ReID v2 must remain primary while changing the retained v1 producer."
        );
      }
      if (control.v1_producer_state === requested) return control;
      const result = await client.query(
        `UPDATE public.vehicle_reid_control
         SET v1_producer_state = $1,
             v1_producer_revision = v1_producer_revision + 1,
             v1_producer_actor_user_id = $2,
             v1_producer_actor_username = $3,
             v1_producer_actor_display_name = $4,
             v1_producer_reason = $5,
             v1_producer_changed_at = CURRENT_TIMESTAMP,
             updated_at = CURRENT_TIMESTAMP
         WHERE singleton = TRUE
         RETURNING *`,
        [
          ...
          normalizedReason(
            reason,
            requested === "stopped"
              ? "Explicit Stage 3 stop of the retained ReID v1 producer."
              : "Explicit Stage 3 restore of the retained ReID v1 producer for rollback readiness."
          ),
        ]
      );
      ...
      await audit(client, {
        actor: operator,
        eventType: requested === "stopped"
          ? "vehicle.reid_v1_producer_stopped"
          : "vehicle.reid_v1_producer_restored",
        resourceType: "vehicle_reid_control",
        resourceId: "singleton",
        metadata: {
          mode: saved.mode,
          priorState: control.v1_producer_state,
          state: saved.v1_producer_state,
          revision: Number(saved.v1_producer_revision),
  ...
  async getOverview() {
    const [control, counts, liveJobs, transitionRun, v1Counts] = await Promise.all([
      this.getControl(),
      this.query(
      ...
      this.query(
        `SELECT
           ...
           (SELECT COUNT(*)::integer FROM public.capture_assets
             WHERE status = 'ready') AS ready_assets,
           (SELECT COUNT(*)::integer FROM public.capture_assets
             WHERE derived_path IS NOT NULL) AS derived_files,
           ...
           GREATEST(
             (SELECT MAX(updated_at) FROM public.capture_assets),
             (SELECT MAX(updated_at) FROM public.vehicle_match_feedback),
             (SELECT MAX(updated_at) FROM public.vehicle_clusters),
             (SELECT MAX(updated_at) FROM public.vehicle_cluster_assignments),
             (SELECT MAX(updated_at) FROM public.vehicle_plate_associations)
           ) AS last_write_at`
      ),
    ]);
    return {
    ...
    const [profile, evidence] = await Promise.all([
      ...
      this.query(
        `WITH expected_merges AS MATERIALIZED (
           ...
           FROM merge_candidates candidates
           JOIN LATERAL (
             SELECT merges.id, merges.source_profile_id,
                    merges.target_profile_id
             FROM public.vehicle_reid_v2_current_profile_merges merges
             WHERE merges.id = candidates.id
               AND (
                 merges.target_profile_id = $2
                 OR merges.source_profile_id = $8
               )
