...
export class VehicleReidV2AuthorityRepository {
  ...
  async listProfiles({ page = 1, pageSize = 24, search = "" } = {}) {
    ...
    const searchClause = term
      ...
      : "";
    const limitParameter = term ? "$2" : "$1";
    const offsetParameter = term ? "$3" : "$2";
    const result = await this.query(
      `WITH paged_profiles AS MATERIALIZED (
         SELECT profiles.*,
         ...
         JOIN paged_profiles profiles ON profiles.id = members.canonical_profile_id
       ), member_counts AS MATERIALIZED (
         ...
         JOIN paged_profiles profiles ON profiles.id = assignments.canonical_profile_id
         ...
         JOIN paged_profiles profiles ON profiles.id = anchors.canonical_profile_id
         GROUP BY anchors.canonical_profile_id
       )
       SELECT profiles.id,
               CASE WHEN profiles.status = 'provisional'
                        AND (COALESCE(member_counts.member_count, 0) > 1
                          OR COALESCE(anchor_counts.anchor_count, 0) > 0)
               ...
               COALESCE(assignment_counts.read_count, 0)::integer AS read_count,
               COALESCE(anchor_counts.anchor_plates, '[]'::jsonb) AS anchor_plates,
               profiles.total_count
       FROM paged_profiles profiles
       JOIN representatives representative
         ON representative.canonical_profile_id = profiles.id
       JOIN member_counts ON member_counts.canonical_profile_id = profiles.id
       LEFT JOIN assignment_counts
         ON assignment_counts.canonical_profile_id = profiles.id
       LEFT JOIN anchor_counts ON anchor_counts.canonical_profile_id = profiles.id
       ORDER BY profiles.updated_at DESC, profiles.id DESC
      `,
      term ? [term, size, offset] : [size, offset]
    );
    return {
      rows: result.rows || [],
