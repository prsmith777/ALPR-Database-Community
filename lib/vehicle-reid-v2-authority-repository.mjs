    ...
    const [control, counts, liveJobs, transitionRun] = await Promise.all([
      ...
      this.query(
         `WITH valid_members AS MATERIALIZED (
           SELECT members.id, members.canonical_profile_id
           ...
           FROM valid_members members
           GROUP BY members.canonical_profile_id
         ), valid_anchors AS MATERIALIZED (
           SELECT anchors.id, anchors.canonical_profile_id
           ...
           SELECT COUNT(*)::integer AS assignments,
                  COUNT(*) FILTER (
                    WHERE assignment_basis = 'exact_effective_plate'
                  )::integer AS exact_plate_assignments,
                  COUNT(*) FILTER (
                    WHERE assignment_basis = 'shared_asset'
                  )::integer AS shared_asset_assignments,
                  COUNT(*) FILTER (
                    WHERE assignment_basis IN ('canonical_image','human_same')
         ...
         SELECT
           ...
           (SELECT COUNT(*) FROM valid_members)::integer
             AS members,
           (SELECT COUNT(*) FROM valid_anchors)::integer
             AS plate_anchors,
           assignment_counts.assignments,
           ((SELECT COUNT(*) FROM public.plate_reads)
             - assignment_counts.assignments)::integer AS unassigned_reads,
           ...
           ((SELECT COUNT(*) FROM public.vehicle_reid_v2_read_assignments
             WHERE status = 'active') - assignment_counts.assignments)::integer
