...
export class VehicleReidV2AuthorityRepository {
    ...
    const [membersResult, anchorsResult] = await Promise.all([
      memberIds.length ? this.query(
        `WITH exact_merges AS MATERIALIZED (
           ...
           WHERE COALESCE(exact_merges.target_profile_id, exact_members.profile_id)
                   = ANY($2::bigint[])
           ...
           SELECT DISTINCT COALESCE(
                    low_merges.target_profile_id, low_members.profile_id
                  ) AS canonical_profile_id
           FROM public.vehicle_reid_v2_pair_reviews reviews
           JOIN page_exact_members page_review_members
             ON page_review_members.derivative_id IN (
               reviews.derivative_id_low, reviews.derivative_id_high
             )
           JOIN public.vehicle_reid_v2_exact_profile_members low_members
             ON low_members.derivative_id = reviews.derivative_id_low
           JOIN public.vehicle_reid_v2_exact_profile_members high_members
             ON high_members.derivative_id = reviews.derivative_id_high
           LEFT JOIN exact_merges low_merges
             ON low_merges.source_profile_id = low_members.profile_id
           LEFT JOIN exact_merges high_merges
             ON high_merges.source_profile_id = high_members.profile_id
           WHERE reviews.label IN ('different_vehicle','unsure')
             ...
             AND COALESCE(low_merges.target_profile_id, low_members.profile_id)
                   = COALESCE(high_merges.target_profile_id, high_members.profile_id)
             AND COALESCE(low_merges.target_profile_id, low_members.profile_id)
                   = ANY($2::bigint[])
  ...
  async getProfile(profileId) {
    ...
    // The physical primary-key lists bound each exact-current view to one set
    // evaluation for this profile.  Keep the computed canonical guard as well
    // so a concurrent merge change fails closed rather than returning another
    // profile's evidence.
    const [profile, members, reads, anchors] = await Promise.all([
      ...
      assignmentIds.length ? this.query(
        `WITH scoped_assignments AS MATERIALIZED (
           ...
           WHERE assignments.id = ANY($1::bigint[])
             AND assignments.canonical_profile_id = $2
           OFFSET 0
         )
      ...
      anchorIds.length ? this.query(
        `WITH scoped_anchors AS MATERIALIZED (
           ...
           FROM public.vehicle_reid_v2_current_plate_anchors anchors
           WHERE anchors.id = ANY($1::bigint[])
             AND anchors.canonical_profile_id = $2
           OFFSET 0
         )
