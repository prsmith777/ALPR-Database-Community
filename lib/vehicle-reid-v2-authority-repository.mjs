...
export class VehicleReidV2AuthorityRepository {
  ...
  async listProfiles({ page = 1, pageSize = 24, search = "" } = {}) {
    ...
    const [mergesResult, searchAnchorsResult] = await Promise.all([
      this.query(
        `SELECT merges.source_profile_id, merges.target_profile_id
         FROM public.vehicle_reid_v2_current_profile_merges merges
         WHERE merges.source_profile_id = ANY($1::bigint[])`,
        [allProfileIds]
      ),
      term
        ? this.query(
    ...
    const hiddenProfileIds = new Set((mergesResult.rows || []).map((row) => (
      Number(row.source_profile_id)
    )));
    const plateMatchedProfileIds = new Set((searchAnchorsResult.rows || []).map((row) => (
      Number(row.canonical_profile_id)
    )));
    const visibleProfileRows = allProfileRows.filter((profile) => {
      const id = Number(profile.id);
      if (hiddenProfileIds.has(id)) return false;
      return !term || String(id) === term || plateMatchedProfileIds.has(id);
    });
    ...
    // Keep the exact-current consumer contract, but bind every expensive view
    // to indexed physical source-profile ids before it is expanded.  Filtering
    // on the views' computed canonical_profile_id prevented PostgreSQL from
    // pushing the page boundary through the merge-alias expression.
  ...
  async getProfile(profileId) {
    ...
    const mergeSources = await this.query(
      `SELECT merges.source_profile_id
       FROM public.vehicle_reid_v2_current_profile_merges merges
       WHERE merges.target_profile_id = $1`,
      [id]
    );
    const sourceProfileIds = [...new Set([
      id,
      ...(mergeSources.rows || []).map((row) => Number(row.source_profile_id)),
    ])];

    // Materialize the indexed physical source scope before applying the
    // computed canonical guard.  The guard keeps a concurrent merge change
    // fail-closed without expanding an exact-current view globally.
