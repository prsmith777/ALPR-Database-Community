...
export class VehicleReidV2ShadowService {
  ...
  async getPrimaryOverview(input = {}) {
    ...
    const rows = await this.repository.listPrimaryCurrentSourceCatalog({
      limit: MAX_SCAN_SOURCES,
    });
    const totalSources = Number(rows[0]?.total_sources || rows.length);
    const catalogSources = rows.map(mapSource);
    ...
    const detailedRows = await this.repository.listPrimaryCurrentSourceDetails(hydrationIds);
    ...
    const unreviewedMatches = selected ? scored.flatMap((item) => {
      ...
      const candidate = mapSource(detailedRow);
      const match = {
        ...candidate,
        rank: item.rank,
        ...
        reviewEvidence: {
          ...
          currentProfileAgreement: intersects(
            selected.currentProfileIds,
            candidate.currentProfileIds
          ),
    ...
    const pairReviews = selectedRow && unreviewedMatches.length
