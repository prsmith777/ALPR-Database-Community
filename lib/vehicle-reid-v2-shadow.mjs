...
const MAX_PRIMARY_HYDRATION_SOURCES = MAX_PAGE_SIZE + MAX_RESULT_LIMIT + 1;
...
function mapPrimarySource(row) {
  const source = mapSource(row);
  return source ? { ...source, currentProfileIds: [] } : null;
}
...
function primarySourceChanged() {
  const error = new Error(
    "The requested Vehicle View changed or is no longer exact-current. Return to Vehicle Search and choose it again."
  );
  error.code = "VEHICLE_REID_V2_SEARCH_SOURCE_CHANGED";
  return error;
}
...
function samePrimarySourceContract(catalog, detailed) {
  if (!catalog || !detailed) return false;
  const fixedValuesMatch = (
    Number(catalog.derivative_id) === Number(detailed.derivative_id)
    && Number(catalog.asset_id) === Number(detailed.asset_id)
    && normalizedHash(catalog.content_sha256)
      === normalizedHash(detailed.content_sha256)
    && normalizedHash(catalog.content_sha256).length === 64
    && Number(catalog.embedding_id) === Number(detailed.embedding_id)
    && String(catalog.model_name || "") === String(detailed.model_name || "")
    && String(catalog.embedding_algorithm_version || "")
      === String(detailed.embedding_algorithm_version || "")
  );
  if (!fixedValuesMatch) return false;
  const catalogEmbeddingHash = normalizedHash(catalog.embedding_sha256);
  const detailedEmbeddingHash = normalizedHash(detailed.embedding_sha256);
  if (catalogEmbeddingHash && detailedEmbeddingHash) {
    return catalogEmbeddingHash === detailedEmbeddingHash;
  }
  const catalogEmbedding = byteBuffer(catalog.embedding);
  const detailedEmbedding = byteBuffer(detailed.embedding);
  return Boolean(catalogEmbedding && detailedEmbedding
    && catalogEmbedding.equals(detailedEmbedding));
}

function pairReviewMatchesPrimarySources(review, source, candidate) {
  if (!review || !source || !candidate) return false;
  const low = Number(source.derivative_id) < Number(candidate.derivative_id)
    ? source : candidate;
  const high = low === source ? candidate : source;
  return (
    Number(review.derivative_id_low) === Number(low.derivative_id)
    && Number(review.derivative_id_high) === Number(high.derivative_id)
    && normalizedHash(review.source_sha256_low) === normalizedHash(low.content_sha256)
    && normalizedHash(review.source_sha256_high) === normalizedHash(high.content_sha256)
    && Number(review.embedding_id_low) === Number(low.embedding_id)
    && Number(review.embedding_id_high) === Number(high.embedding_id)
    && String(review.embedding_model || "") === String(low.model_name || "")
    && String(review.embedding_model || "") === String(high.model_name || "")
    && String(review.algorithm_version || "")
      === String(low.embedding_algorithm_version || "")
    && String(review.algorithm_version || "")
      === String(high.embedding_algorithm_version || "")
  );
}

export class VehicleReidV2ShadowService {
  constructor({ repository, logger = null, now = () => Date.now() } = {}) {
  ...
  async getPrimaryOverview(input = {}) {
    ...
    let outcome = "failed";
    try {
      const overview = await this.loadPrimaryOverview(input, timing);
      outcome = "succeeded";
      return overview;
    } finally {
      const totalMs = Math.max(0, Math.round(this.now() - startedAt));
      try {
        this.logger?.info?.("vehicle_reid_v2_primary_browse", {
          outcome,
          ...timing,
          totalMs,
        });
      } catch {
        // Search behavior must not depend on operational logging availability.
      }
    }
  }

  async loadPrimaryOverview(input = {}, timing = {}) {
    if (
      typeof this.repository.listPrimaryCurrentSourceCatalog !== "function"
      || typeof this.repository.listPrimaryCurrentSourceDetails !== "function"
    ) {
      throw new Error("Primary ReID v2 Vehicle Search storage is unavailable.");
    }
    ...
    const catalogStartedAt = this.now();
    let rows;
    try {
      rows = await this.repository.listPrimaryCurrentSourceCatalog({
        limit: MAX_SCAN_SOURCES,
      });
    } finally {
      timing.catalogMs = Math.max(0, Math.round(this.now() - catalogStartedAt));
    }
    timing.catalogRowCount = rows.length;
    const totalSources = Number(rows[0]?.total_sources || rows.length);
    const catalogSources = rows.map(mapPrimarySource);
    const filtered = catalogSources.filter((source) => matchesSearch(source, search));
    const pageCount = Math.max(1, Math.ceil(filtered.length / pageSize));
    const page = Math.min(requestedPage, pageCount);
    const pageCatalogSources = filtered.slice((page - 1) * pageSize, page * pageSize);
    const rowsById = new Map(rows.map((row) => [Number(row.derivative_id), row]));

    const requestedSourceId = Number(input.sourceDerivativeId);
    const sourceParameterPresent = input.sourceDerivativeId !== null
      && input.sourceDerivativeId !== undefined
      && String(input.sourceDerivativeId).trim() !== "";
    const explicitSourceRequested = Number.isSafeInteger(requestedSourceId)
      && requestedSourceId > 0;
    const suppressDefaultSelection = input.suppressDefaultSelection === true
      || (sourceParameterPresent && !explicitSourceRequested);
    let selectedCatalogRow = explicitSourceRequested
      ? rowsById.get(requestedSourceId) || null
      : suppressDefaultSelection
        ? null
        : rowsById.get(pageCatalogSources[0]?.derivativeId) || null;
    if (
      explicitSourceRequested
      && !selectedCatalogRow
      && typeof this.repository.getPrimaryCurrentSourceCatalog === "function"
    ) {
      const exactSourceStartedAt = this.now();
      try {
        selectedCatalogRow = await this.repository.getPrimaryCurrentSourceCatalog(
          requestedSourceId
        );
      } finally {
        timing.exactSourceMs = Math.max(
          0,
          Math.round(this.now() - exactSourceStartedAt)
        );
      }
    }
    if (explicitSourceRequested && !selectedCatalogRow) throw primarySourceChanged();
    if (selectedCatalogRow) {
      rowsById.set(Number(selectedCatalogRow.derivative_id), selectedCatalogRow);
    }

    const allScored = selectedCatalogRow ? rows
      .filter((row) => Number(row.derivative_id) !== Number(selectedCatalogRow.derivative_id))
      .map((row) => ({
        row,
        similarity: cosineSimilarityFromBytes(selectedCatalogRow.embedding, row.embedding),
      }))
    ...
    const scored = ranked.slice(0, resultLimit);
    const hydrationIds = [...new Set([
      ...pageCatalogSources.map((source) => source.derivativeId),
      Number(selectedCatalogRow?.derivative_id) || null,
      ...scored.map((item) => Number(item.row.derivative_id)),
    ].filter((value) => Number.isSafeInteger(value) && value > 0))];
    if (hydrationIds.length > MAX_PRIMARY_HYDRATION_SOURCES) {
      throw new Error("Primary ReID v2 Vehicle Search exceeded its hydration bound.");
    }
    timing.hydrationCount = hydrationIds.length;
    const hydrationStartedAt = this.now();
    let detailedRows;
    try {
      detailedRows = await this.repository.listPrimaryCurrentSourceDetails(hydrationIds);
    } finally {
      timing.hydrationMs = Math.max(0, Math.round(this.now() - hydrationStartedAt));
    }
    const detailsById = new Map(detailedRows.flatMap((row) => {
      const derivativeId = Number(row.derivative_id);
      const catalogRow = rowsById.get(derivativeId);
      return samePrimarySourceContract(catalogRow, row) ? [[derivativeId, row]] : [];
    }));
    const selectedRow = selectedCatalogRow
      ? detailsById.get(Number(selectedCatalogRow.derivative_id)) || null
      : null;
    if (explicitSourceRequested && !selectedRow) throw primarySourceChanged();
    const selected = mapPrimarySource(selectedRow);
    const sources = pageCatalogSources
      .map((source) => detailsById.get(source.derivativeId))
      .filter(Boolean)
      .map(mapPrimarySource);

    // A candidate can turn stale between the catalog and hydration queries. Drop
    // it without renumbering later candidates: rank remains global to the exact
    // bounded catalog that was compared.
    const unreviewedMatches = selected ? scored.flatMap((item) => {
      const detailedRow = detailsById.get(Number(item.row.derivative_id));
      if (!detailedRow) return [];
      const candidate = mapPrimarySource(detailedRow);
      const match = {
        ...candidate,
        rank: item.rank,
        ...
        marginToNext: Number.isFinite(item.nextSimilarity)
          ...
          : null,
        reviewEvidence: {
          plateAgreement: intersects(selected.plateNumbers, candidate.plateNumbers),
          currentProfileAgreement: "unavailable",
    ...
    let pairReviews = [];
    if (
      selectedRow && unreviewedMatches.length
      && typeof this.repository.listPairReviewsForSource === "function"
    ) {
      const pairReviewStartedAt = this.now();
      try {
        pairReviews = await this.repository.listPairReviewsForSource({
        ...
        timing.pairReviewMs = Math.max(0, Math.round(this.now() - pairReviewStartedAt));
      }
    }
    const reviewsByCandidate = new Map(pairReviews.flatMap((review) => {
      const candidateId = Number(review.candidate_derivative_id);
      const candidateRow = detailsById.get(candidateId);
      return pairReviewMatchesPrimarySources(review, selectedRow, candidateRow)
        ? [[candidateId, publicVehicleReidV2Review(review, candidateId)]]
        : [];
    }));
    ...
    const reviewSummaries = this.summarizeReviewRows([]);
    ...
    return {
      ...
      stats: {
        ...
        fullyAttributed: rows.filter((row) => row.fully_attributed === true).length,
      ...
      selected,
      matches,
      winnerMargin: !selectedRow
        || ranked[0]?.nextSimilarity == null
        || !ranked.slice(0, 2).every((item) => (
          detailsById.has(Number(item.row.derivative_id))
        ))
        ? null
      ...
      reviewCampaign: {
        campaign: null,
        active: false,
        browseMode: input.browseMode === true,
        available: 0,
        current: null,
        next: null,
  ...
  async getOverview(input = {}) {
    if (input.primaryBrowse === true) return this.getPrimaryOverview(input);
    const pageSize = boundedInteger(input.pageSize, DEFAULT_PAGE_SIZE, 1, MAX_PAGE_SIZE);
    const requestedPage = boundedInteger(input.page, 1, 1, Number.MAX_SAFE_INTEGER);
    const resultLimit = boundedInteger(
      input.resultLimit,
