...
export class VehicleReidV2ShadowService {
  ...
  async getOverview(input = {}) {
    ...
    let selectedRow = Number.isSafeInteger(requestedSourceId) && requestedSourceId > 0
      ? rows.find((row) => Number(row.derivative_id) === requestedSourceId)
      : null;
    if (!selectedRow && Number.isSafeInteger(requestedSourceId) && requestedSourceId > 0) {
      selectedRow = await this.repository.getCurrentSource(requestedSourceId);
    }
    ...
    const unreviewedMatches = scored.map((item, index) => {
      const candidate = mapSource(item.row);
      const nextSimilarity = scored[index + 1]?.similarity;
      return {
        ...candidate,
        rank: index + 1,
        similarity: Number(item.similarity.toFixed(6)),
        marginToNext: Number.isFinite(nextSimilarity)
          ? Number((item.similarity - nextSimilarity).toFixed(6))
          : null,
        reviewEvidence: {
    ...
    const [pairReviews, calibration] = await Promise.all([
      selected && unreviewedMatches.length
        && typeof this.repository.listPairReviewsForSource === "function"
        ? this.repository.listPairReviewsForSource({
    ...
    return {
      ...
      selected,
      matches,
      winnerMargin: matches[0]?.marginToNext ?? null,
      calibration,
    };
  }

  async getCalibrationSummary() {
    if (typeof this.repository.listPairReviewCalibration !== "function") {
      return summarizeVehicleReidV2Reviews([]);
    }
    return summarizeVehicleReidV2Reviews(await this.repository.listPairReviewCalibration({
      modelName: VEHICLE_ASSET_EMBEDDING_MODEL,
      algorithmVersion: VEHICLE_ASSET_EMBEDDING_ALGORITHM,
    }));
  }
