...
export class VehicleReidV2ShadowService {
  ...
  async getOverview(input = {}) {
    ...
    const campaignRow = typeof this.repository.getLatestReviewCampaign === "function"
      ? await this.repository.getLatestReviewCampaign()
      : null;
    const campaign = publicCampaign(campaignRow);
    const campaignActive = input.campaignReview === true && campaign?.status === "active";
  ...
  async recordPairReview({
    ...
    if (normalizedCampaignId) {
      ...
      if (!campaignQueue.some((item) => item.pairIdentity === requestedIdentity)) {
        throw new VehicleReidV2ReviewError(
          "VEHICLE_REID_V2_REVIEW_CAMPAIGN_CHANGED",
          "This pair is no longer a current new unresolved campaign recommendation. Refresh before continuing."
        );
      }
    }
