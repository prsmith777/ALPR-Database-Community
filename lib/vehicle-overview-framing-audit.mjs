...
export class VehicleOverviewFramingAuditService {
  ...
  async auditRead(read) {
    try {
      ...
      const repairReason = reasons.find((reason) => [
        "VEHICLE_TOUCHES_IMAGE_EDGE",
        "VEHICLE_FRAMING_TOO_TIGHT",
      ].includes(reason)) || null;
      const repairEligible = Boolean(repairReason) && [
        "overview_primary",
        "entry_overview_primary",
      ].includes(String(read.vehicle_image_source_kind || ""));
      return this.result(read, {
        width,
        height,
        classification: repairEligible ? "repair_candidate" : reasons.length ? "review" : "acceptable",
  ...
  async auditBatch({ afterReadId = 0, maxReadId = null, limit = 10 } = {}) {
    ...
    return {
      ...
      repairCandidates: items.filter((item) => item.repairEligible === true).length,
      // Kept as a response alias for one release so an older loaded client can
      // finish an audit across a rolling deployment without losing its count.
      unacceptable: items.filter((item) => item.repairEligible === true).length,
