...
export class CaptureAssetRepository {
  ...
  async recordReady({ read, derivedPath, sourceSha256, perceptualHash, colorSignature, colorSignatureVersion, crop, imageWidth, imageHeight, profileVersion }) {
    const result = await this.query(
      `INSERT INTO public.capture_assets (
         read_id, asset_type, algorithm_version, crop_profile_version, status, source_image_path,
         derived_path, source_sha256, perceptual_hash, color_signature, color_signature_version, crop_box,
         image_width, image_height, crop_width, crop_height, indexed_at
       ) VALUES ($1, $2, $3, $4, 'ready', $5, $6, $7, $8, $9, $10, $11::jsonb,
      ...
      [read.id, ASSET_TYPE, CAPTURE_ASSET_ALGORITHM, profileVersion, read.image_path,
        derivedPath, sourceSha256, perceptualHash, colorSignature, colorSignatureVersion, JSON.stringify(crop), imageWidth,
    ...
    await this.query(
      `INSERT INTO public.capture_assets (
       ...
       ON CONFLICT (read_id, asset_type, algorithm_version) DO UPDATE SET
         ...
         color_signature_version = NULL, crop_box = NULL,
  ...
  async listSearchCandidates({ readId, cameraNames = [], startDate, endDate }) {
    ...
    const result = await this.query(
      `SELECT ca.read_id, ca.derived_path, ca.source_sha256, ca.perceptual_hash,
              ca.color_signature, ca.color_signature_version,
