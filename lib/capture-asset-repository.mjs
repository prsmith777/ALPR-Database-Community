...
import {
  VEHICLE_DISTINCTIVE_FEATURE_MODEL,
  VEHICLE_DISTINCTIVE_FEATURE_PROVIDER,
  VEHICLE_DISTINCTIVE_FEATURES,
} from "./vehicle-distinctive-features.mjs";
...
export class CaptureAssetRepository {
  ...
  async reviewVehicleDistinctiveFeatures({ readId, features, actor }) {
    const normalizedReadId = Number(readId);
    if (!Number.isSafeInteger(normalizedReadId) || normalizedReadId < 1) {
      const error = new Error("Vehicle capture was not found.");
      error.code = "VEHICLE_CAPTURE_NOT_FOUND";
      throw error;
    }
    const connected = Boolean(this.pool?.connect);
    const client = connected ? await this.pool.connect() : this.executor;
    if (!client?.query) throw new Error("Vehicle feature review requires a database client");
    const actorId = Number.isSafeInteger(Number(actor?.id)) && Number(actor.id) > 0 ? Number(actor.id) : null;
    const actorUsername = String(actor?.username || "legacy-admin").slice(0, 64);
    const actorDisplayName = String(actor?.displayName || "Legacy Administrator").slice(0, 120);
    const catalogKeys = VEHICLE_DISTINCTIVE_FEATURES.map((feature) => feature.key);
    try {
      if (connected) await client.query("BEGIN");
      const readResult = await client.query(
        `SELECT id FROM public.plate_reads WHERE id = $1::integer FOR UPDATE`,
        [normalizedReadId]
      );
      ...
      const previousResult = await client.query(
        `SELECT SUBSTRING(attribute_key FROM 9) AS feature_key
         FROM public.vehicle_attribute_observations
         WHERE read_id = $1::integer
           AND attribute_key LIKE 'feature:%'
           AND provider = $2::varchar AND model_version = $3::varchar
           AND status = 'ready' AND attribute_value = 'present'
         ORDER BY feature_key`,
        [normalizedReadId, VEHICLE_DISTINCTIVE_FEATURE_PROVIDER, VEHICLE_DISTINCTIVE_FEATURE_MODEL]
      );
      await client.query(
        `INSERT INTO public.vehicle_attribute_observations (
           read_id, attribute_key, status, attribute_value, confidence,
         ...
         SELECT $1::integer, 'feature:' || feature_key, 'ready',
                CASE WHEN feature_key = ANY($2::text[]) THEN 'present' ELSE 'absent' END,
                1::real, $3::varchar, $4::varchar,
                jsonb_build_object(
                  'reviewed', true,
                  'actorUsername', $5::varchar,
                  'actorDisplayName', $6::varchar
                ), NULL
         FROM UNNEST($7::text[]) AS feature_key
         ON CONFLICT (read_id, attribute_key, provider, model_version) DO UPDATE SET
           status = EXCLUDED.status, attribute_value = EXCLUDED.attribute_value,
           confidence = EXCLUDED.confidence, raw_result = EXCLUDED.raw_result,
           error_code = NULL, evaluated_at = CURRENT_TIMESTAMP`,
        [normalizedReadId, features, VEHICLE_DISTINCTIVE_FEATURE_PROVIDER,
          VEHICLE_DISTINCTIVE_FEATURE_MODEL, actorUsername, actorDisplayName, catalogKeys]
      );
      const previousFeatures = previousResult.rows.map((row) => row.feature_key);
      await client.query(
        `INSERT INTO public.audit_events (
           actor_user_id, source, event_type, resource_type, resource_id, outcome, metadata
         ) VALUES ($1::bigint, 'browser', 'vehicle.distinctive_features_review',
                   'plate_read', $2, 'succeeded', $3::jsonb)`,
        [actorId, String(normalizedReadId), JSON.stringify({
          readId: normalizedReadId,
          previousFeatures,
          features,
          provider: VEHICLE_DISTINCTIVE_FEATURE_PROVIDER,
          modelVersion: VEHICLE_DISTINCTIVE_FEATURE_MODEL,
        })]
      );
      if (connected) await client.query("COMMIT");
      return { readId: normalizedReadId, features, reviewed: true };
    } catch (error) {
      if (connected) await client.query("ROLLBACK");
      throw error;
    } finally {
  ...
  async listVehicleClusterOverview({ clusterLimit = 100, reviewLimit = 40, featureKey = null } = {}) {
    const [clusters, suggestions, stats] = await Promise.all([
      this.query(
        `SELECT clusters.id, clusters.status, clusters.embedding_model,
                ...
                COALESCE((
                  SELECT jsonb_agg(jsonb_build_object(
                    ...
                    'captureCount', features.capture_count,
                    'confidence', features.confidence,
                    'humanReviewed', features.human_reviewed
                  ) ORDER BY features.capture_count DESC, features.feature_key)
                  FROM (
                    SELECT SUBSTRING(observation.attribute_key FROM 9) AS feature_key,
                           COUNT(DISTINCT feature_assignment.read_id)::integer AS capture_count,
                           MAX(observation.confidence)::real AS confidence,
                           BOOL_OR(observation.provider = 'human-review') AS human_reviewed
                    FROM public.vehicle_cluster_assignments feature_assignment
                    JOIN public.vehicle_attribute_observations observation
                      ON observation.read_id = feature_assignment.read_id
                    WHERE feature_assignment.cluster_id = clusters.id
                      AND feature_assignment.assignment_status IN ('seed', 'confirmed')
                      AND observation.attribute_key LIKE 'feature:%'
                      AND observation.status = 'ready'
                      AND observation.attribute_value = 'present'
                    GROUP BY SUBSTRING(observation.attribute_key FROM 9)
                  ) features
                ), '[]'::jsonb) AS distinctive_features
         FROM public.vehicle_clusters clusters
         JOIN LATERAL (
         ...
         WHERE clusters.status <> 'retired'
           AND ($4::varchar IS NULL OR EXISTS (
             SELECT 1
             FROM public.vehicle_cluster_assignments filter_assignment
             JOIN public.vehicle_attribute_observations filter_observation
               ON filter_observation.read_id = filter_assignment.read_id
             WHERE filter_assignment.cluster_id = clusters.id
               AND filter_assignment.assignment_status IN ('seed', 'confirmed')
               AND filter_observation.attribute_key = 'feature:' || $4::varchar
               AND filter_observation.status = 'ready'
               AND filter_observation.attribute_value = 'present'
           ))
        ...
        [clusterLimit, ASSET_TYPE, CAPTURE_ASSET_ALGORITHM, featureKey]
      ),
      this.query(
  ...
  async getVehicleClusterProfile(clusterId, captureLimit = 60) {
    ...
    const [profileResult, associationResult, captureResult, featureSummaryResult] = await Promise.all([
      this.query(
        `SELECT clusters.id, clusters.status, clusters.embedding_model,
                clusters.algorithm_version, clusters.representative_read_id,
                representative.derived_path AS representative_path,
      ...
      this.query(
        `SELECT assignments.read_id, assignments.assignment_status,
                ...
                COALESCE(distinctive.distinctive_features, '[]'::jsonb) AS distinctive_features
         FROM public.vehicle_cluster_assignments assignments
         JOIN public.plate_reads reads ON reads.id = assignments.read_id
         JOIN LATERAL (
           SELECT derived_path FROM public.capture_assets
         ...
         LEFT JOIN LATERAL (
           SELECT attribute_value, confidence FROM public.vehicle_attribute_observations
           WHERE read_id = assignments.read_id AND attribute_key = 'color' AND status = 'ready'
           ORDER BY evaluated_at DESC, id DESC LIMIT 1
         ) color ON TRUE
         LEFT JOIN public.vehicle_direction_observations direction ON direction.read_id = assignments.read_id
         LEFT JOIN LATERAL (
           SELECT jsonb_agg(jsonb_build_object(
             'key', SUBSTRING(observation.attribute_key FROM 9),
             'confidence', observation.confidence,
             'provider', observation.provider,
             'reviewed', COALESCE((observation.raw_result ->> 'reviewed')::boolean, false)
           ) ORDER BY observation.attribute_key) AS distinctive_features
           FROM public.vehicle_attribute_observations observation
           WHERE observation.read_id = assignments.read_id
             AND observation.attribute_key LIKE 'feature:%'
             AND observation.status = 'ready'
             AND observation.attribute_value = 'present'
         ) distinctive ON TRUE
      ...
      this.query(
        `SELECT SUBSTRING(observation.attribute_key FROM 9) AS feature_key,
                COUNT(DISTINCT assignments.read_id)::integer AS capture_count,
                MAX(observation.confidence)::real AS confidence,
                BOOL_OR(observation.provider = 'human-review') AS human_reviewed
         FROM public.vehicle_cluster_assignments assignments
         JOIN public.vehicle_attribute_observations observation
           ON observation.read_id = assignments.read_id
         WHERE assignments.cluster_id = $1
           AND assignments.assignment_status IN ('seed', 'confirmed')
           AND observation.attribute_key LIKE 'feature:%'
           AND observation.status = 'ready'
           AND observation.attribute_value = 'present'
         GROUP BY SUBSTRING(observation.attribute_key FROM 9)
         ORDER BY capture_count DESC, feature_key`,
        [normalizedClusterId]
      ),
    ]);
    ...
    return {
      ...
      distinctiveFeatures: featureSummaryResult.rows,
    };
  }
