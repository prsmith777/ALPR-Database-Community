    ...
    try {
      ...
      const removed = await client.query(
        `WITH target_reads AS (${targetReads})
         DELETE FROM public.vehicle_direction_observations observations
         USING target_reads
         WHERE observations.read_id = target_reads.read_id
         RETURNING observations.status`,
        values
      );
      ...
      const previousReady = removed.rows.filter((row) => row.status === "ready").length;
      const previousUnknown = removed.rows.length - previousReady;
      const actorId = Number.isSafeInteger(Number(actor?.id)) && Number(actor.id) > 0 ? Number(actor.id) : null;
      await client.query(
        `INSERT INTO public.audit_events (
           actor_user_id, source, event_type, resource_type, resource_id, outcome, metadata
