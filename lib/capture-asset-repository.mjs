...
export class CaptureAssetRepository {
  ...
  async recordReady({ read, derivedPath, sourceSha256, perceptualHash, crop, imageWidth, imageHeight, profileVersion }) {
    const result = await this.query(
      `INSERT INTO public.capture_assets (
       ...
       ) VALUES ($1, $2, $3, $4, 'ready', $5, $6, $7, $8, $9::jsonb,
      ...
      [read.id, ASSET_TYPE, CAPTURE_ASSET_ALGORITHM, profileVersion, read.image_path,
        derivedPath, sourceSha256, perceptualHash, JSON.stringify(crop), imageWidth,
    ...
    await this.query(
      `INSERT INTO public.capture_assets (
       ...
       ON CONFLICT (read_id, asset_type, algorithm_version) DO UPDATE SET
         ...
         source_sha256 = NULL, perceptual_hash = NULL, crop_box = NULL,
  ...
  async listSearchCandidates({ readId, cameraNames = [], startDate, endDate }) {
    ...
    const conditions = ["ca.asset_type = $1", "ca.algorithm_version = $2", "ca.status = 'ready'", "ca.read_id <> $3", CURRENT_PROFILE];
