...
export class CaptureAssetRepository {
  ...
  async listOrientationSamples(cameraName, embeddingModel) {
    const result = await this.query(
      `SELECT labels.orientation, assets.vehicle_embedding
  ...
  async listUnassignedVehicleAssets(embeddingModel, limit = 100) {
    const result = await this.query(
      `SELECT assets.read_id, assets.vehicle_embedding, assets.embedding_model
       FROM public.capture_assets assets
       ...
       ORDER BY assets.indexed_at ASC, assets.read_id ASC LIMIT $2`,
      [embeddingModel, limit]
    );
    return result.rows;
  }
  ...
  async listPendingVehicleColorAssets(provider, modelVersion, limit = 100) {
    const result = await this.query(
      `SELECT assets.read_id, assets.derived_path
       ...
       ORDER BY assets.indexed_at DESC, assets.read_id DESC LIMIT $3`,
      [provider, modelVersion, limit]
    );
    return result.rows;
  }
    ...
    const [clusters, suggestions, stats] = await Promise.all([
      this.query(
        `SELECT clusters.id, clusters.status, clusters.embedding_model,
         ...
         FROM public.vehicle_clusters clusters
         JOIN public.capture_assets representative ON representative.read_id = clusters.representative_read_id
         ...
         ORDER BY last_seen DESC NULLS LAST, clusters.id DESC LIMIT $1`,
        [clusterLimit]
      ),
      this.query(
        `SELECT assignment.read_id, assignment.cluster_id, assignment.similarity,
         ...
         JOIN public.capture_assets candidate ON candidate.read_id = assignment.read_id
    ...
    try {
      ...
      const savedResult = await client.query(
        `UPDATE public.vehicle_cluster_assignments SET
           cluster_id = $2, assignment_status = $3, similarity = $4,
           similarity_margin = CASE WHEN $3 = 'seed' THEN NULL ELSE similarity_margin END,
