...
export class VehicleImageAssetCatalogWorker {
  constructor({
    ...
    if (liveCatalog && typeof liveCatalog.processBatch !== "function") {
      throw new Error("Automatic canonical Overview catalog worker dependency is invalid");
    }
  ...
  async runOnce() {
    ...
    const useLiveCatalog = !campaignActive
      && campaignResult.status === "idle"
      && this.liveCatalog;
    const result = useLiveCatalog
      ? await this.liveCatalog.processBatch({ limit: 5 })
      : campaignResult;
    const mode = useLiveCatalog ? "automatic" : "campaign";
    this.lastBatch = {
    ...
    this.phase = result.processed > 0
      ...
      : (result.activation || "idle");
    return {
      delayMs: result.processed > 0 || result.discovered > 0
        ? 100
        : campaignActive || result.status !== "idle"
          ? 2_000
          : 30_000,
    };
  }
