...
export function normalizeVehicleDetections(detections, { imageWidth, imageHeight, plateBox = null } = {}) {
...
export function selectVehicleDetection(detections, dimensions = {}) {
  return normalizeVehicleDetections(detections, dimensions)[0] || null;
}
...
export class VehicleReidEngine {
  ...
  async detectAll(buffer, dimensions) {
    return normalizeVehicleDetections(await this.inferDetections(buffer), dimensions);
  }

  async detect(buffer, dimensions) {
    return (await this.detectAll(buffer, dimensions))[0] || null;
  }
  ...
  async embedDetection(buffer, detection, { imageWidth, imageHeight } = {}) {
    const width = Number(imageWidth);
    const height = Number(imageHeight);
    if (!detection || !Number.isFinite(width) || !Number.isFinite(height) || width <= 0 || height <= 0) {
      return null;
    }
    const crop = paddedPixelCrop(detection, width, height);
    crop.width = Math.min(crop.width, width - crop.left);
    crop.height = Math.min(crop.height, height - crop.top);
    const cropBuffer = await this.imageProcessor(buffer)
