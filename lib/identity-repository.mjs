...
export class PostgresIdentityRepository {
  ...
  async getLoginThrottle(subjectHash) {
    const pool = await this.getPool();
    const result = await pool.query(
      `
  ...
  async recordFailedLogin({
    ...
    return await this.transaction(async (client) => {
      ...
      `);
      await client.query("SELECT pg_advisory_xact_lock(hashtext($1))", [subjectHash]);
      const result = await client.query(
        `
          INSERT INTO public.login_attempt_limits
            (subject_hash, failed_attempts, window_started_at, blocked_until, updated_at)
          VALUES ($1, 1, CURRENT_TIMESTAMP, NULL, CURRENT_TIMESTAMP)
          ON CONFLICT (subject_hash) DO UPDATE
          SET failed_attempts = CASE
                WHEN login_attempt_limits.window_started_at <
                     CURRENT_TIMESTAMP - make_interval(secs => $2::double precision)
                  THEN 1
                ELSE login_attempt_limits.failed_attempts + 1
              END,
              ...
              END,
              blocked_until = CASE
                WHEN login_attempt_limits.blocked_until > CURRENT_TIMESTAMP
                  THEN login_attempt_limits.blocked_until
                WHEN CASE
                       WHEN login_attempt_limits.window_started_at <
                            CURRENT_TIMESTAMP - make_interval(secs => $2::double precision)
                         THEN 1
        ...
        `,
        [subjectHash, windowSeconds, failureLimit, blockSeconds]
      );
      ...
      `
        ...
        VALUES ($1::bigint, 'browser', 'auth.login', 'user', $2::text, 'denied',
                 $3::text, $4::jsonb)
      `,
        [
          userId || null,
          userId ? String(userId) : null,
          blocked ? "rate_limited" : "invalid_credentials",
          JSON.stringify({
            username,
            userAgent,
            failedAttempts: Number(throttle.failed_attempts),
            rateLimited: blocked,
          }),
        ]
      );
      return {
        failedAttempts: Number(throttle.failed_attempts),
        blocked,
        blockedUntil: throttle.blocked_until,
      };
    });
  }

  async clearLoginThrottle(subjectHash) {
    const pool = await this.getPool();
    await pool.query(
      "DELETE FROM public.login_attempt_limits WHERE subject_hash = $1",
      [subjectHash]
    );
  }
  ...
  async createUser({
    ...
    return await this.transaction(async (client) => {
      const inserted = await client.query(
        ...
        `,
        [username, displayName, passwordHash, mustChangePassword]
      );
      const user = inserted.rows[0];
      ...
      await client.query(
        `
          INSERT INTO public.audit_events
            (actor_user_id, source, event_type, resource_type, resource_id,
             outcome, metadata)
          VALUES ($1::bigint, 'browser', 'identity.user_created', 'user', $2::text,
                  'succeeded', $3::jsonb)
        `,
  ...
  async setUserStatus({ actorUserId, targetUserId, status }) {
    return await this.transaction(async (client) => {
      ...
      const target = await client.query(
        "SELECT id, username FROM public.users WHERE id = $1 AND deleted_at IS NULL FOR UPDATE",
        [targetUserId]
      );
      ...
      await client.query("UPDATE public.users SET status = $2 WHERE id = $1 AND deleted_at IS NULL", [
        targetUserId,
        status,
      ]);
      if (status === "disabled") {
      ...
      await client.query(
        `
          INSERT INTO public.audit_events
            (actor_user_id, source, event_type, resource_type, resource_id,
             outcome, metadata)
          VALUES ($1::bigint, 'browser', 'identity.user_status_changed', 'user', $2::text,
                  'succeeded', $3::jsonb)
        `,
  ...
  async setUserRole({ actorUserId, targetUserId, roleName }) {
    return await this.transaction(async (client) => {
      ...
      const current = await client.query(
        "SELECT id, status FROM public.users WHERE id = $1 AND deleted_at IS NULL FOR UPDATE",
        [targetUserId]
      );
      ...
      await client.query(
        `
          INSERT INTO public.audit_events
            (actor_user_id, source, event_type, resource_type, resource_id,
             outcome, metadata)
          VALUES ($1::bigint, 'browser', 'identity.user_role_changed', 'user', $2::text,
                  'succeeded', $3::jsonb)
        `,
    ...
    return await this.transaction(async (client) => {
      const result = await client.query(
        `
          ...
          RETURNING id
        `,
        [targetUserId, passwordHash, mustChangePassword]
      );
      ...
      await client.query(
        `
          INSERT INTO public.audit_events
            (actor_user_id, source, event_type, resource_type, resource_id, outcome)
          VALUES ($1::bigint, 'browser', $3, 'user', $2::text, 'succeeded')
        `,
  ...
  async deleteUser({
    ...
    return await this.transaction(async (client) => {
      ...
      if (actorUserId === targetUserId) {
        throw repositoryError("CANNOT_DELETE_SELF", "You cannot delete your own account.");
      }
      ...
      if (!target.rows[0]) throw repositoryError("USER_NOT_FOUND", "User not found.");
      if (target.rows[0].username !== confirmUsername) {
        throw repositoryError(
          "INVALID_DELETE_CONFIRMATION",
          "Type the account username exactly to confirm deletion."
        );
      }
      ...
      if (target.rows[0].status === "active" && targetRole.rows[0].is_administrator) {
        const administrators = await client.query(`
          SELECT COUNT(DISTINCT app_user.id)::integer AS count
          FROM public.users AS app_user
          JOIN public.user_roles AS user_role ON user_role.user_id = app_user.id
      ...
      await client.query(
        `
          INSERT INTO public.audit_events
            (actor_user_id, source, event_type, resource_type, resource_id,
             outcome, metadata)
          VALUES ($1::bigint, 'browser', 'identity.user_deleted', 'user', $2::text,
                  'succeeded', $3::jsonb)
        `,
        [
          actorUserId,
