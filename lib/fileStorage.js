...
const canonicalPublicationTails = new Map();

function storageErrorCode(error, fallback) {
  return String(error?.code || fallback).slice(0, 80);
}

async function safeDerivedPublicationPath(baseDir, derivedDir, fullPath) {
  const [baseInfo, derivedInfo] = await Promise.all([
    fs.lstat(baseDir),
    fs.lstat(derivedDir),
  ]);
  if (baseInfo.isSymbolicLink() || derivedInfo.isSymbolicLink() || !derivedInfo.isDirectory()) {
    throw new Error("Derived storage roots must be real directories");
  }
  const [realBaseDir, realDerivedDir] = await Promise.all([
    fs.realpath(baseDir),
    fs.realpath(derivedDir),
  ]);
  if (!isPathInside(realBaseDir, realDerivedDir)) {
    throw new Error("Derived storage root is outside storage");
  }

  const parentPath = path.dirname(fullPath);
  const relativeParent = path.relative(derivedDir, parentPath);
  let current = derivedDir;
  for (const segment of relativeParent.split(path.sep).filter(Boolean)) {
    ...
    if (info.isSymbolicLink() || !info.isDirectory()) {
      throw new Error("Derived image paths cannot contain symbolic links");
    }
  }

  const realParent = await fs.realpath(parentPath);
  if (!isPathInside(realDerivedDir, realParent)) {
    throw new Error("Derived image path is outside derived storage");
  }
  return path.join(realParent, path.basename(fullPath));
}

async function assertOwnedCanonicalFile(fullPath) {
  const info = await fs.lstat(fullPath);
  if (info.isSymbolicLink() || !info.isFile() || Number(info.nlink || 1) !== 1) {
    const unsafe = new Error("Canonical derived image path is not a regular owned file");
    unsafe.code = "CANONICAL_DERIVED_IMAGE_UNSAFE";
    throw unsafe;
  }
}

async function withCanonicalPublicationTurn(key, operation) {
  const prior = canonicalPublicationTails.get(key) || Promise.resolve();
  let releaseTurn;
  const turn = new Promise((resolve) => { releaseTurn = resolve; });
  const tail = prior.catch(() => {}).then(() => turn);
  canonicalPublicationTails.set(key, tail);
  await prior.catch(() => {});
  try {
    return await operation();
  } finally {
    releaseTurn();
    if (canonicalPublicationTails.get(key) === tail) {
      canonicalPublicationTails.delete(key);
    }
  }
}
...
export class FileStorage {
  ...
  async saveDerivedImageIfAbsent(relativePath, imageData) {
    const requestedPath = resolveStoragePath(this.baseDir, relativePath, ["derived"]);
    return withCanonicalPublicationTurn(requestedPath, async () => {
      const fullPath = await safeDerivedPublicationPath(
        this.baseDir,
        this.derivedDir,
        requestedPath
      );
      const temporaryPath = `${fullPath}.${crypto.randomUUID()}.tmp`;
      const expected = Buffer.isBuffer(imageData) ? imageData : Buffer.from(imageData);
      try {
        await fs.writeFile(temporaryPath, expected, { flag: "wx" });
        try {
          // Publishing through a hard link is atomic and never replaces a
          // canonical file another writer has already installed.
          await fs.link(temporaryPath, fullPath);
          return { relativePath, created: true };
        } catch (error) {
          if (error?.code !== "EEXIST") throw error;
          await assertOwnedCanonicalFile(fullPath);
          ...
          if (existingHash !== expectedHash) {
            const mismatch = new Error("Canonical derived image does not match its content identity");
            mismatch.code = "CANONICAL_DERIVED_IMAGE_MISMATCH";
            throw mismatch;
          }
