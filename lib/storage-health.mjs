...
export async function collectStorageHealth({
  ...
  try {
    ...
    [activeCatalogCampaign, liveCatalog] = await Promise.all([
  ...
  if (result.filesystem && result.database && result.assets) {
    ...
    const estimatedBytesPerDay = baseEstimatedBytesPerDay + canonicalEstimatedBytesPerDay;
    ...
    result.growth = {
      ...
      basis: liveCatalogGrowthEnabled
        ? "Seven-day read rate × recent bounded source sample and database bytes/read, plus eligible Overview rate × observed canonical bytes/current link"
