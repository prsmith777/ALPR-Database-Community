...
export const VEHICLE_MOTION_DIRECTION_ALGORITHM = "plate-anchored-motion-v2-shadow";

const MOTION_FRAME_WIDTH = 160;
const NIGHT_MONOCHROME_RATIO = 0.9;
const DAY_MONOCHROME_RATIO = 0.82;
const MAX_ANCHOR_FRAME_OFFSET_MS = 750;
...
function summarizeTrajectory(points, {
  ...
  const anchorDiagnostics = { monochromeRatio, anchorOffsetMs, anchorSource, anchorDistance };
...
export function analyzeDayDetectionMotion({
  ...
  if (!anchorCandidate) {
    return unknownObservation({
      ...
      diagnostics: { monochromeRatio, anchorOffsetMs, anchorSource, anchorDistance },
    });
  }
  const points = track
    .filter((candidate) => (
...
export async function analyzeVehicleMotionDirection({
  ...
  const requestedAnchorOffset = Number(anchorOffsetMs);
  const exactAnchorFrame = Number.isFinite(requestedAnchorOffset)
  ...
  const anchorFrame = exactAnchorFrame
    || (nearestAnchorFrame && Math.abs(Number(nearestAnchorFrame.offsetMs)) <= MAX_ANCHOR_FRAME_OFFSET_MS
      ? nearestAnchorFrame
      : null);
  if (!anchorFrame) {
    return unknownObservation({
      ...
      diagnostics: {
        requestedAnchorOffsetMs: Number.isFinite(requestedAnchorOffset) ? requestedAnchorOffset : null,
