...
async function loadDraft(client, ruleId, { lock = false } = {}) {
  ...
  if (
    groupResult.rows.length !== 1 ||
    groupResult.rows[0].parent_group_id != null ||
    groupResult.rows[0].combinator !== "all" ||
    groupResult.rows[0].negated === true
  ) {
    throw new Error("This rule uses a condition structure that is not editable here");
  }
  const conditions = conditionResult.rows;
  if (
    conditions.length === 0 ||
    conditions.some((condition) => !EDITABLE_TYPES.has(condition.condition_type)) ||
    !conditions.some((condition) => condition.condition_type === "tag") ||
    !conditions.some((condition) => condition.condition_type === "camera")
  ) {
    throw new Error("Only migrated tag-and-camera rules can be edited here");
  }
  return { rule, rootGroup: groupResult.rows[0], conditions };
}
...
function conditionTree(rootGroup, conditions) {
  return {
    id: rootGroup.id,
    kind: "group",
    combinator: rootGroup.combinator,
    negated: Boolean(rootGroup.negated),
...
export class NotificationRuleDraftRepository {
  ...
  async updateTagCameraRule({ ruleId, requireKnownPlate = false, tags = [], cameras = [], actor = null } = {}) {
    ...
    try {
      ...
      await client.query(
        "DELETE FROM public.notification_conditions WHERE group_id = $1::bigint",
        [locked.rootGroup.id]
      );
      let position = 0;
      ...
      const decision = evaluateNotificationRule(
        {
          ...
          conditionTree: conditionTree(locked.rootGroup, locked.conditions),
          actions: [],
        },
