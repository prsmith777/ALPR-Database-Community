...
export class BlueIrisTimelineExportService {
  constructor({
    ...
    this.pollIntervalMs = Math.min(5_000, Math.max(250, Number(pollIntervalMs) || DEFAULT_POLL_INTERVAL_MS));
  ...
  async reconcileRemoteExports({ limit = 20 } = {}) {
    if (typeof this.repository.claimTimelineExportsNeedingCleanup !== "function") {
      return { examined: 0, deleted: 0, failed: 0 };
    }
    const exports = await this.repository.claimTimelineExportsNeedingCleanup({ limit });
    let deleted = 0;
    let failed = 0;
    for (const record of exports) {
      try {
        await this.client.deleteTimelineExport(record.remote_path);
        await this.repository.markTimelineExportDeleted(record.export_token);
        deleted += 1;
      } catch (error) {
        await this.repository.markTimelineExportFailed(record.export_token, {
          errorCode: "EXPORT_DELETE_FAILED",
    ...
    return { examined: exports.length, deleted, failed };
  }
  ...
  async acquire({
    ...
    try {
      ...
      while (!remote.complete) {
        ...
        await ensureActive();
        try {
          remote = await this.client.getTimelineExport(remote.remotePath);
        } catch (error) {
          const hasVerifiableReservedExport = error?.code === "EXPORT_UNAVAILABLE"
            && remote?.uri
            && normalizedEpochMs(remote.utc) !== null
            && Number.isFinite(Number(remote.durationMs));
          if (!hasVerifiableReservedExport) throw error;
          remote = {
            ...remote,
            complete: true,
            status: "disappeared_after_export",
          };
        }
        await this.repository.recordTimelineExportRemote(exportToken, remote);
      }
      if (!remote.uri) {
        throw new BlueIrisError("EXPORT_URI_MISSING", "Blue Iris completed the export without a download URI.");
      }
      ...
      try {
        await this.client.deleteTimelineExport(remote.remotePath);
        await this.repository.markTimelineExportDeleted(exportToken);
        deletedRemotely = true;
      } catch (deleteError) {
        await this.repository.markTimelineExportFailed(exportToken, {
          errorCode: "EXPORT_DELETE_FAILED",
          errorDetails: boundedErrorDetails(deleteError),
          deletePending: true,
        });
        this.logger?.warn?.("Blue Iris timeline export cleanup was deferred", {
          exportToken,
          code: String(deleteError?.code || "EXPORT_DELETE_FAILED"),
        });
      }
      await ensureActive();
      ...
      if (remote?.remotePath && !deletedRemotely && !deleteAttempted) {
        deleteAttempted = true;
        try {
          await this.client.deleteTimelineExport(remote.remotePath);
          await this.repository.markTimelineExportDeleted(exportToken);
      ...
      if (remote?.remotePath && !deletedRemotely && deleteAttempted) {
        await this.repository.markTimelineExportFailed(exportToken, {
          errorCode: "EXPORT_DELETE_FAILED",
          errorDetails: boundedErrorDetails(cleanupError),
          deletePending: true,
        }).catch(() => {});
      }
      if (!remote?.remotePath || deletedRemotely) {
        await this.repository.markTimelineExportFailed(exportToken, {
