...
import {
  overviewFramingRepairLifecycle,
  overviewFramingRepairProfileFromClaim,
} from "./vehicle-overview-framing-repair.mjs";
...
export class BlueIrisVehicleFrameQueue {
  ...
  async processBatch({ limit = 1 } = {}) {
    ...
    try {
      ...
      if (typeof this.repository.terminalizeExpiredOverviewFramingRepairJobs === "function") {
        await this.repository.terminalizeExpiredOverviewFramingRepairJobs();
      }
      ...
      for (let index = 0; index < boundedLimit; index += 1) {
        ...
        const repairRead = typeof this.repository.claimNextOverviewFramingRepairJob === "function"
          ? await this.repository.claimNextOverviewFramingRepairJob({ requireNoLiveWork: true })
          : null;
        if (repairRead) {
          const repairProfile = overviewFramingRepairProfileFromClaim(repairRead);
          const repairJobId = Number(repairRead.framing_repair_job_id);
          const repairClaimToken = String(repairRead.vehicle_image_claim_token || "").trim();
          const failRepair = async ({ errorCode, unavailable = false, retryable = false, details = null }) => {
            await this.repository.markOverviewFramingRepairFailed(repairJobId, {
            ...
            return {
              ...
              status: retryable ? "retry_scheduled" : unavailable ? "unavailable" : "failed",
              readId: Number(repairRead.id),
              errorCode,
            };
          };
          if (!repairProfile) {
            results.push(await failRepair({
              errorCode: "OVERVIEW_REPAIR_PROFILE_INVALID",
              unavailable: true,
            }));
            continue;
          }
          ...
          try {
            ...
            const retryable = Number(repairRead.vehicle_image_attempt_count || 0) < 2;
            results.push(await failRepair({
          ...
          const cameraByName = uniqueCamera(context.cameras, repairProfile.source_camera_name);
          const cameraByShortName = repairProfile.source_camera_short_name
            ? uniqueCamera(context.cameras, repairProfile.source_camera_short_name)
            : null;
          const isEntry = repairProfile.overview_context === "entry";
          const bindingMismatch = isEntry
            ? !cameraByName || !cameraByShortName || cameraByName.id !== cameraByShortName.id
            : Boolean(repairProfile.source_camera_short_name
              && (!cameraByShortName || (cameraByName && cameraByName.id !== cameraByShortName.id)));
          const repairCamera = bindingMismatch ? null : cameraByShortName || cameraByName;
          if (!repairCamera) {
            results.push(await failRepair({
          ...
          results.push(await context.overviewService.processOverviewRead({
            ...
            camera: repairCamera.id,
            alreadyClaimed: true,
            lifecycle: overviewFramingRepairLifecycle(this.repository, repairRead),
          }));
          continue;
        }
