...
const BLUE_IRIS_CAMERA_INVENTORY_REFRESH_MS = 5 * 60_000;
...
export class BlueIrisVehicleFrameQueue {
  constructor({
    ...
    this.cameraInventoryRefreshAttemptAt = 0;
  ...
  async loadCameras(client, { force = false } = {}) {
    ...
    if (typeof this.repository.syncBlueIrisCameraInventory === "function") {
      try {
        await this.repository.syncBlueIrisCameraInventory(connection.cameras || []);
      } catch (error) {
        this.logger.warn("blue_iris_camera_inventory_sync_failed", {
  ...
  async refreshCameraInventory({ force = false } = {}) {
    ...
    if (
      !force
      && this.cameraInventoryRefreshAttemptAt
      && now - this.cameraInventoryRefreshAttemptAt < BLUE_IRIS_CAMERA_INVENTORY_REFRESH_MS
    ) {
      return { configured: true, refreshed: false };
    }

    this.cameraInventoryRefreshAttemptAt = now;
    const cameras = await this.loadCameras(this.clientFactory(config.blueiris), { force });
    return { configured: true, refreshed: true, aliases: cameras.size };
  }
      ...
      for (let index = 0; index < boundedLimit; index += 1) {
        ...
        if (entryHistoryRead) {
          ...
          try {
            ...
            this.logger?.error?.("Blue Iris initialization failed for Gate Overview history", {
              readId: Number(entryHistoryRead.id),
...
export const blueIrisVehicleFrameQueueInternals = Object.freeze({
  BLUE_IRIS_CAMERA_INVENTORY_REFRESH_MS,
