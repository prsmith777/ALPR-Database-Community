...
export class BlueIrisClient {
  ...
  async getTimelineExport(remotePath) {
    ...
    const records = await this.listTimelineExports();
    const matched = records.find((record) => record.remotePath === exportPath);
    if (matched) return matched;
    const summary = await this.listTimelineExports({ summary: true });
    const summarized = summary.find((record) => record.remotePath === exportPath);
    if (summarized) return summarized;
    throw new BlueIrisError(
      "EXPORT_UNAVAILABLE",
      "Blue Iris no longer lists the generated timeline export."
    );
  }
  ...
  async deleteTimelineExport(remotePath) {
    const exportPath = normalizedExportPath(remotePath);
    try {
      await this.command({ cmd: "export", path: exportPath, delete: true });
      return { remotePath: exportPath, deleted: true, alreadyMissing: false };
    } catch (error) {
      if (error?.code !== "COMMAND_FAILED") throw error;
      const records = await this.listTimelineExports();
      if (records.some((record) => record.remotePath === exportPath)) throw error;
      return { remotePath: exportPath, deleted: true, alreadyMissing: true };
    }
  }
