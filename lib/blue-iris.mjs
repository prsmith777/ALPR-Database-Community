...
export class BlueIrisClient {
  ...
  async checkTimelineExportDownloadAvailability(uri, { retryAuthentication = true } = {}) {
    ...
    try {
      ...
      throw new BlueIrisError(
        "HTTP_ERROR",
        `Blue Iris returned HTTP ${response?.status || "error"} while checking export availability.`
      );
    } catch (error) {
      if (error instanceof BlueIrisError) throw error;
      if (error?.name === "AbortError") {
        throw new BlueIrisError(
          "TIMEOUT",
          "Blue Iris did not confirm export availability before the timeout."
        );
      }
      throw new BlueIrisError(
        "CONNECTION_FAILED",
        "Unable to check Blue Iris export availability.",
        { cause: error }
      );
    } finally {
      clearTimeout(timeout);
