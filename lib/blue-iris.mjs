...
function normalizedClipKey(value) {
...
export class BlueIrisClient {
  ...
  async findMatchingAlert({
    ...
    const targetClip = normalizedClipKey(clip);
    const targetOffset = offsetMs === null || offsetMs === undefined || offsetMs === ""
      ? null
      : Number(offsetMs);
    const candidates = records
      ...
      .map((candidate) => ({
        ...
        clipMatch: Boolean(targetClip && normalizedClipKey(candidate.record?.clip) === targetClip),
        offsetDeltaMs: Number.isFinite(targetOffset) && Number.isFinite(Number(candidate.record?.offset))
          ? Math.abs(Number(candidate.record.offset) - targetOffset)
          : null,
      }))
      .sort((left, right) => (
        Number(right.clipMatch) - Number(left.clipMatch)
        || (left.offsetDeltaMs ?? Number.POSITIVE_INFINITY)
          - (right.offsetDeltaMs ?? Number.POSITIVE_INFINITY)
        || left.deltaSeconds - right.deltaSeconds
      ));
  ...
  async findNearestAlert({ camera, timestamp, toleranceSeconds = 90 }) {
    return this.findMatchingAlert({ camera, timestamp, toleranceSeconds });
  }
}
