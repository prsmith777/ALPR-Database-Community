      ...
      const inserted = await client.query(`
        INSERT INTO public.notification_rules
          (name, description, enabled, event_type, cooldown_seconds, time_zone,
           ...
           created_by_user_id, updated_by_user_id)
        VALUES ($1, NULLIF($2, ''), FALSE, $3, $4, $5, $6::jsonb, $7,
                CASE WHEN $3 = 'camera.activity_check' THEN CURRENT_TIMESTAMP ELSE NULL END,
                $8::bigint, $8::bigint)
  ...
  async updateDraft({ id, draft, actor = null } = {}) {
    ...
    try {
      ...
      const updated = await client.query(`
        ...
        SET name = $2, description = NULLIF($3, ''), event_type = $4,
            cooldown_seconds = $5, time_zone = $6, quiet_hours = $7::jsonb,
            evaluation_interval_seconds = $8,
            next_evaluation_at = CASE WHEN $4 = 'camera.activity_check' THEN CURRENT_TIMESTAMP ELSE NULL END,
