...
async function loadRules(executor, { onlyRuleId = null, lock = false } = {}) {
  const where = onlyRuleId
    ? "WHERE r.id = $1::bigint AND r.deleted_at IS NULL"
  ...
  const [rules, groups, conditions, actions] = await Promise.all([
    executor.query(`
      SELECT r.*,
             EXISTS (
               ...
               WHERE m.target_rule_id = r.id
                 AND m.retired_at IS NULL
                 AND m.finalized_at IS NULL
             ) AS managed_by_migration,
             EXISTS (
               SELECT 1
               FROM public.notification_rule_migrations history
               WHERE history.target_rule_id = r.id
             ) AS migrated_from_legacy
    ...
    executor.query(`
      ...
      ${onlyRuleId ? "AND a.rule_id = $1::bigint" : ""}
...
function assertDeletable(rule, expectedName) {
  assertEditable(rule);
  if (String(expectedName || "") !== rule.name) {
    throw new Error("Rule name confirmation does not match");
  }
}

function readEvent(row, eventType = "plate_read.accepted") {
  return {
    id: Number(row.id),
    type: eventType,
    ...
    vehicleOrientation: row.vehicle_orientation || "unknown",
    directionConfidence: row.orientation_confidence == null ? null : Number(row.orientation_confidence),
  };
}
...
export class NotificationRuleBuilderRepository {
  ...
  async overview({ channels = {}, pushover = {} } = {}) {
    const pushoverState = channels.pushover || pushover;
    const emailState = channels.email || {};
    const webhookState = channels.webhook || {};
    const [rules, tags, cameras, directions, brokers, mqttSettings] = await Promise.all([
      loadRules(this.pool),
      ...
      this.pool.query(`
        SELECT MIN(direction_label) AS direction_label FROM (
          SELECT front_direction_label AS direction_label FROM public.camera_direction_profiles WHERE enabled = TRUE
          UNION
          SELECT rear_direction_label AS direction_label FROM public.camera_direction_profiles WHERE enabled = TRUE
          UNION
          SELECT direction_label FROM public.vehicle_direction_observations WHERE status = 'ready'
        ) configured WHERE NULLIF(BTRIM(direction_label), '') IS NOT NULL
    ...
    return {
      ...
      options: {
        ...
        directions: directions.rows.map((row) => row.direction_label),
        ...
        pushoverEnabled: Boolean(pushoverState.enabled),
        pushoverConfigured: Boolean(pushoverState.configured),
        emailEnabled: Boolean(emailState.enabled),
        emailConfigured: Boolean(emailState.configured),
        webhookEnabled: Boolean(webhookState.enabled),
        webhookConfigured: Boolean(webhookState.configured),
        localTimeZone: String(mqttSettings.rows[0]?.local_timezone || pushoverState.localTimeZone || "America/Denver"),
      },
    };
  }

  async loadOperationsHistory({ limit = 50 } = {}) {
    return new NotificationRuntimeRepository({ executor: this.pool }).loadOperationsHistory({ limit });
  }
    ...
    try {
      ...
      const inserted = await client.query(`
        INSERT INTO public.notification_rules
          (name, description, enabled, event_type, cooldown_seconds, time_zone,
           quiet_hours, evaluation_interval_seconds, next_evaluation_at,
           created_by_user_id, updated_by_user_id)
        VALUES ($1, NULLIF($2, ''), FALSE, $3::text, $4, $5, $6::jsonb, $7,
                CASE WHEN $3::text = 'camera.activity_check' THEN CURRENT_TIMESTAMP ELSE NULL END,
                $8::bigint, $8::bigint)
      ...
      `, [
        ...
        normalized.timeZone,
        JSON.stringify(normalized.quietHours),
        normalized.evaluationIntervalSeconds,
        userId,
      ]);
  ...
  async updateDraft({ id, draft, actor = null } = {}) {
    ...
    try {
      ...
      if (channelIds.length) {
        ...
        `, [channelIds, actorId(actor)]);
      }
      // Delivery history retains a restrictive foreign key to the exact action
      // that produced it. Retire the prior draft actions instead of deleting
      // them, then create a fresh active action set for the new rule version.
      await client.query(`
        UPDATE public.notification_actions
        SET enabled = FALSE,
            retired_at = CURRENT_TIMESTAMP,
            retired_by_user_id = $2::bigint
        WHERE rule_id = $1::bigint AND retired_at IS NULL
      `, [parsedId, actorId(actor)]);
      await client.query("DELETE FROM public.notification_condition_groups WHERE rule_id = $1::bigint", [parsedId]);
      const updated = await client.query(`
        UPDATE public.notification_rules
        SET name = $2, description = NULLIF($3, ''), event_type = $4::text,
            cooldown_seconds = $5, time_zone = $6, quiet_hours = $7::jsonb,
            evaluation_interval_seconds = $8,
            next_evaluation_at = CASE WHEN $4::text = 'camera.activity_check' THEN CURRENT_TIMESTAMP ELSE NULL END,
            evaluation_locked_at = NULL, evaluation_locked_by = NULL,
            version = version + 1, updated_by_user_id = COALESCE($9::bigint, updated_by_user_id)
        WHERE id = $1::bigint
        RETURNING version
      `, [
        parsedId,
    ...
    try {
      ...
      if (enabled) {
        const available = (channelType) => {
          const state = channelAvailability[channelType];
          if (state) return Boolean(state.enabled && state.configured);
          return channelType === "pushover" ? Boolean(pushoverAvailable) : false;
        };
        for (const [channelType, label] of [["pushover", "Pushover"], ["email", "Email"], ["webhook", "Webhooks"]]) {
          if (rule.actions.some((action) => action.channelType === channelType) && !available(channelType)) {
            throw new Error(`Enable and configure ${label} before activating this rule`);
          }
        }
      ...
      await client.query(`
        ...
        SET enabled = $2,
            next_evaluation_at = CASE
              WHEN $2 = TRUE AND event_type = 'camera.activity_check'
                THEN COALESCE(next_evaluation_at, CURRENT_TIMESTAMP)
              ELSE next_evaluation_at
            END,
            evaluation_locked_at = CASE WHEN $2 = FALSE THEN NULL ELSE evaluation_locked_at END,
            evaluation_locked_by = CASE WHEN $2 = FALSE THEN NULL ELSE evaluation_locked_by END,
            updated_by_user_id = COALESCE($3::bigint, updated_by_user_id)
        WHERE id = $1::bigint
  ...
  async deleteRule({ id, expectedName, actor = null } = {}) {
    const parsedId = ruleId(id);
    if (typeof this.pool.connect !== "function") throw new Error("Rule deletion requires a transactional pool");
    const client = await this.pool.connect();
    try {
      await client.query("BEGIN");
      await client.query("SELECT pg_advisory_xact_lock(hashtext('alpr_notification_rule_transition'), $1::integer)", [parsedId]);
      const rule = (await loadRules(client, { onlyRuleId: parsedId, lock: true }))[0];
      assertDeletable(rule, expectedName);

      const inFlight = await client.query(`
        SELECT COUNT(*)::integer AS count
        FROM public.notification_deliveries d
        JOIN public.notification_executions e ON e.id = d.execution_id
        WHERE e.rule_id = $1::bigint AND d.status = 'processing'
      `, [parsedId]);
      if (Number(inFlight.rows[0]?.count) > 0) {
        throw new Error("Wait for the in-progress delivery before deleting this rule");
      }

      const cancelled = await client.query(`
        UPDATE public.notification_deliveries d
        SET status = 'cancelled', next_attempt_at = CURRENT_TIMESTAMP,
            locked_at = NULL, locked_by = NULL, updated_at = CURRENT_TIMESTAMP
        FROM public.notification_executions e
        WHERE d.execution_id = e.id AND e.rule_id = $1::bigint
          AND d.status IN ('pending', 'retry')
      `, [parsedId]);
      await client.query("UPDATE public.notification_actions SET enabled = FALSE WHERE rule_id = $1::bigint", [parsedId]);
      await client.query(`
        UPDATE public.notification_channels ch
        SET enabled = FALSE, updated_by_user_id = COALESCE($2::bigint, updated_by_user_id)
        FROM public.notification_actions a
        WHERE a.channel_id = ch.id AND a.rule_id = $1::bigint
      `, [parsedId, actorId(actor)]);
      const deleted = await client.query(`
        UPDATE public.notification_rules
        SET enabled = FALSE, deleted_at = CURRENT_TIMESTAMP,
            deleted_by_user_id = $2::bigint, deletion_reason = 'administrator_deleted',
            next_evaluation_at = NULL, evaluation_locked_at = NULL, evaluation_locked_by = NULL,
            updated_by_user_id = COALESCE($2::bigint, updated_by_user_id)
        WHERE id = $1::bigint AND deleted_at IS NULL
        RETURNING id, name, version
      `, [parsedId, actorId(actor)]);
      if (!deleted.rows[0]) throw new Error("The notification rule was not found");
      await audit(client, {
        actor,
        eventType: "notification.rule_deleted",
        id: parsedId,
        metadata: {
          name: rule.name,
          version: rule.version,
          deletedFromWorkspace: true,
          cancelledDeliveries: Number(cancelled.rowCount || 0),
        },
      });
      await client.query("COMMIT");
      return {
        ...
        version: Number(deleted.rows[0].version),
        deleted: true,
        cancelledDeliveries: Number(cancelled.rowCount || 0),
      };
    } catch (error) {
  ...
  async preview({ id, limit = 25, matchingSettings = {} } = {}) {
    ...
    const reads = await this.pool.query(`
      ...
      LEFT JOIN public.vehicle_direction_observations direction ON direction.read_id = pr.id
      GROUP BY pr.id, kp.plate_number, kp.name, p.flagged,
               direction.orientation, direction.orientation_confidence, direction.direction_label
    ...
    if (rule.eventType === "camera.activity_check") {
      const cameraNames = [];
      const visit = (node) => {
        if (node?.kind === "condition" && node.conditionType === "camera") {
          cameraNames.push(...(node.value?.names || []));
        } else for (const child of node?.children || []) visit(child);
      };
      visit(rule.conditionTree);
      const timestamp = new Date();
      const samples = [];
      for (const cameraName of [...new Set(cameraNames)]) {
        const event = {
          id: `preview-${cameraName}`,
          type: "camera.activity_check",
          timestamp,
          cameraName,
          plateNumber: "",
        };
        const metrics = await new NotificationRuntimeRepository({ executor: this.pool })
          .loadReadCountMetrics({ rules: [previewRule], event });
        const decision = evaluateNotificationRule(previewRule, {
          event,
          now: timestamp,
          matchingSettings,
          metrics,
        ...
        samples.push({
          readId: `camera-${cameraName}`,
          plateNumber: "Camera activity",
          cameraName,
          timestamp,
    ...
    for (const row of reads.rows) {
      const event = readEvent(row, rule.eventType);
