...
function sourceSemantics(entry, read, knownPlates, matchingSettings) {
  ...
  const knownPlatesAtRead = typeof read.known_plate === "boolean"
    ? read.known_plate
      ? [{
          plate_number: read.plate_number,
          name: read.known_name || "",
...
export function buildNotificationShadowReview({
  ...
  const rules = entries.map((entry) => {
    ...
    const expansionCount = decisions.filter(
      (decision) => !decision.legacyMatched && decision.unifiedMatched
    ).length;
    const regressionCount = decisions.filter(
      (decision) => decision.legacyMatched && !decision.unifiedMatched
    ).length;
    const positiveMatchCount = decisions.filter(
      (decision) => decision.legacyMatched && decision.unifiedMatched
    ).length;
    const unifiedPositiveMatchCount = decisions.filter(
      (decision) => decision.unifiedMatched
    ).length;
    const allDisabled =
    ...
    else if (regressionCount > 0) status = "needs_review";
    else if (expansionCount > 0) {
      status =
        currentApproval && latestReview?.approval_mode === "intentional_expansion"
          ? "approved_intentional"
    ...
    return {
      ...
      latestReview: latestReview
        ? {
            ...
            approvalMode: latestReview.approval_mode || "parity",
            expansionCount: Number(latestReview.expansion_count || 0),
            regressionCount: Number(latestReview.regression_count || 0),
  ...
  return {
    ...
    expansionCount: rules.reduce((sum, rule) => sum + rule.expansionCount, 0),
    regressionCount: rules.reduce((sum, rule) => sum + rule.regressionCount, 0),
    deliveryAttempts: 0,
    rules,
  };
}
