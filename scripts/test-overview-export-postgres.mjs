...
try {
  ...
  const migrationCompatibilityProfile = await pool.query(
    `INSERT INTO public.vehicle_overview_pair_profiles (
     ...
     ) VALUES ($1, 'Overview01', $2, 'Entering', 'primary', 'entry', 0, 1500, 90, TRUE)
     RETURNING id`,
    [`Migration Gate Overview ${suffix}`, `Migration Gate LPR ${suffix}`]
  );
  ...
  try {
    const legacyRoute = await legacyFallbackClient.query(
      ...
      [
        `Legacy route ${suffix}`,
        plateCamera,
        `Legacy Gate LPR 1 ${suffix}`,
        `Legacy Gate LPR 2 ${suffix}`,
      ],
    );
  ...
  const insertHistoryRead = async ({
    ...
    const inserted = await pool.query(
      `INSERT INTO public.plate_reads (
         ...
         $1, 'Gate LPR 1', $2::timestamptz, $3, $4, $5, 0, TRUE,
  ...
  assert.equal(historyClaim.overview_source_camera_short_name, "Overview01");
  ...
  const historyReady = await repositoryA.markEntryOverviewBackfillReady(
    ...
    {
      framePath: `derived/history-${suffix}-overview01.jpg`,
  ...
  assert.equal(historyReadyRead.rows[0].vehicle_image_path, `derived/history-${suffix}-overview01.jpg`);
