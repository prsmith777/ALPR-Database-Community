...
export async function readBuildMetadata(gitDirectory = ".git") {
  try {
    return await readGitBuildMetadata(gitDirectory);
  } catch (error) {
    if (error?.code === "ENOENT") {
      return { gitSha: null, channel: "self-hosted" };
    }
    throw error;
  }
}
...
export function serializeReleaseMetadata(metadata) {
  return [
    "// Generated during the Docker build; do not edit.",
