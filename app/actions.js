...
const ENTRY_OVERVIEW_SOURCE_CAMERA = "Gate Overview";
const ENTRY_OVERVIEW_SOURCE_SHORT_NAME = "Overview01";
...
function normalizedEntryOverviewHistoryRunId(value) {
  ...
  if (!Number.isSafeInteger(runId) || runId <= 0) {
    throw new Error("A valid Gate Overview history run is required.");
  }
  return runId;
}
...
function requiredEntryOverviewHistoryRunId(value) {
  ...
  if (!runId) throw new Error("A valid Gate Overview history run is required.");
  return runId;
}
...
export async function saveVehicleEntryOverviewHistoryProfiles(input = {}) {
  ...
  try {
    ...
    return visualSearchFailure(error, "Unable to save Gate Overview history anchors.");
  }
}
...
export async function previewVehicleEntryOverviewHistory(input = {}) {
  ...
  try {
    ...
    return visualSearchFailure(error, "Unable to preview Gate Overview history.");
  }
}
...
export async function confirmVehicleEntryOverviewHistory(input = {}) {
  ...
  try {
    ...
    if (!ENTRY_OVERVIEW_HISTORY_BATCH_SIZES.has(limit)) {
      throw new Error("Gate Overview history batches must contain 1, 5, 25, or 250 reads.");
    }
    const runtime = await getBlueIrisVehicleFrameRuntime();
    const currentRun = await runtime.repository.getEntryOverviewBackfillRun(runId, { jobLimit: 1 });
    if (!currentRun) throw new Error("This Gate Overview history run was not found.");
    if (currentRun.status === "paused") {
      throw new Error("Resume Gate Overview history before queuing its next batch.");
    }
    if (Number(currentRun.counts?.queued || 0) + Number(currentRun.counts?.processing || 0) > 0) {
      throw new Error("Wait for the current Gate Overview history batch to finish before queuing another.");
    }
    ...
    return visualSearchFailure(error, "Unable to confirm this Gate Overview history batch.");
  }
}
...
export async function setVehicleEntryOverviewHistoryPaused(input = {}) {
  ...
  try {
    ...
    if (!updated) throw new Error("This Gate Overview history run is no longer active.");
    if (!paused) wakeBlueIrisVehicleFrameWorker();
    const run = await runtime.repository.getEntryOverviewBackfillRun(runId, { jobLimit: 1 });
    revalidatePath("/settings/vehicle-intelligence/vehicle-views");
    return { success: true, data: { run: entryOverviewHistoryRunData(run) } };
  } catch (error) {
    return visualSearchFailure(error, "Unable to update Gate Overview history processing.");
  }
}
...
export async function cancelVehicleEntryOverviewHistory(input = {}) {
  ...
  try {
    ...
    if (!cancellation) throw new Error("This Gate Overview history run is no longer active.");
    const run = await runtime.repository.getEntryOverviewBackfillRun(runId, { jobLimit: 1 });
    revalidatePath("/settings/vehicle-intelligence/vehicle-views");
    revalidatePath("/live_feed");
    return { success: true, data: { cancellation, run: entryOverviewHistoryRunData(run) } };
  } catch (error) {
    return visualSearchFailure(error, "Unable to cancel Gate Overview history processing.");
  }
}
...
export async function retryVehicleEntryOverviewHistoryImport(input = {}) {
  ...
  try {
    ...
    return visualSearchFailure(error, "Unable to retry this failed Gate Overview import.");
  }
}
  ...
  try {
    ...
    if (overviewContext === "entry") {
      if (sourceRole !== "primary") {
        throw new Error("Gate Overview mappings must use the Primary overview role.");
      }
      if (!isEntryPlateCamera) {
        throw new Error("Gate Overview mappings are limited to Gate LPR 1 and Gate LPR 2.");
      }
      if (
        ...
        throw new Error("Gate Overview must use the Blue Iris display name Gate Overview and short name Overview01.");
      }
    } else if (sourceRole === "primary" && isEntryPlateCamera) {
      throw new Error("Gate LPR primary Vehicle Views must use the Gate overview setting.");
    }
...
export async function setVehicleEntryFallbackMode(input = {}) {
  ...
  try {
    ...
    if (!["off", "shadow", "active"].includes(mode)) {
      throw new Error("Select Off, Shadow, or Active for Gate LPR fallback.");
    }
    ...
    if (!["off", "shadow", "active"].includes(overviewPayloadMode)) {
      throw new Error("Select Off, Shadow, or Active for the Gate Overview payload.");
    }
    ...
    return visualSearchFailure(error, "Unable to update Gate LPR fallback.");
  }
}
...
export async function saveVehicleEntryRouteProfile(input = {}) {
  ...
  try {
    ...
    if (sourceCameraNames.length !== 2) {
      throw new Error("Select exactly two different Gate LPR cameras for corroboration.");
    }
    ...
    return visualSearchFailure(error, "Unable to save this Gate LPR route.");
  }
}
...
export async function deleteVehicleEntryRouteProfile(profileId) {
  ...
  try {
    const normalizedId = Number.parseInt(String(profileId), 10);
    if (!Number.isSafeInteger(normalizedId) || normalizedId <= 0) {
      throw new Error("A valid Gate LPR route profile is required.");
    }
    const runtime = await getBlueIrisVehicleFrameRuntime();
    ...
    return visualSearchFailure(error, "Unable to delete this Gate LPR route.");
  }
}
