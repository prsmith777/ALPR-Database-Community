...
export async function previewVehicleOverviewFramingRepairs(input = {}) {
  const principal = await requirePermission("maintenance.manage");
  try {
    const [{ VehicleOverviewFramingAuditService }, { VehicleOverviewFramingRepairService }, runtime] = await Promise.all([
      import("@/lib/vehicle-overview-framing-audit.mjs"),
      import("@/lib/vehicle-overview-framing-repair.mjs"),
      getBlueIrisVehicleFrameRuntime(),
    ]);
    const auditService = new VehicleOverviewFramingAuditService({
      repository: runtime.repository,
      fileStorage: runtime.queue.fileStorage,
    });
    ...
    return visualSearchFailure(error, "Unable to create this Overview framing repair preview.");
  }
}

export async function getVehicleOverviewFramingRepairStatus(input = {}) {
  await requirePermission("system.manage_settings");
  try {
    const runtime = await getBlueIrisVehicleFrameRuntime();
    const run = await runtime.repository.getOverviewFramingRepairRun(input.runId ?? null);
    return { success: true, data: { run } };
  } catch (error) {
    return visualSearchFailure(error, "Unable to load Overview framing repair status.");
  }
}

export async function confirmVehicleOverviewFramingRepairBatch(input = {}) {
  const principal = await requirePermission("maintenance.manage");
  try {
    const limit = Number.parseInt(String(input.limit), 10);
    if (![1, 5, 25].includes(limit)) {
      throw new Error("Overview framing repair batches must contain 1, 5, or 25 reads.");
    }
    const runtime = await getBlueIrisVehicleFrameRuntime();
    const run = await runtime.repository.confirmOverviewFramingRepairBatch(input.runId, {
    ...
    return visualSearchFailure(error, "Unable to confirm this Overview framing repair batch.");
  }
}

export async function cancelVehicleOverviewFramingRepair(input = {}) {
  const principal = await requirePermission("maintenance.manage");
  try {
    const runtime = await getBlueIrisVehicleFrameRuntime();
    const run = await runtime.repository.cancelOverviewFramingRepairRun(input.runId, {
      actor: principal.user,
    });
    if (!run) throw new Error("This Overview framing repair run is no longer active.");
    revalidatePath("/settings/vehicle-intelligence/vehicle-views");
    return { success: true, data: { run } };
  } catch (error) {
    return visualSearchFailure(error, "Unable to cancel Overview framing repair work.");
  }
}
