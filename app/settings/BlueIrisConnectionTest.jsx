...
export default function BlueIrisConnectionTest() {
  ...
  const testConnection = () => {
    ...
    startTransition(async () => {
      ...
      if (result.success && !camera && result.cameras?.length) {
        setCamera(result.cameras[0].id);
      }
    });
  };
  ...
  const searchAlerts = () => {
    ...
    startTransition(async () => {
      setMatch(
        await previewBlueIrisAlertMatch({
          camera,
    ...
    <div className="space-y-4 rounded-lg border border-border p-4">
      <div>
        ...
        <p className="text-sm text-muted-foreground">
          Save changes first. This lists cameras and searches alert metadata; it never changes or deletes Blue Iris recordings.
        </p>
      </div>
      ...
      {connection?.success && (
        <div className="space-y-4 rounded-md bg-muted/40 p-4">
          ...
          {match?.success && !match.matched && (
            <p className="text-sm text-muted-foreground">
              No alert matched within the selected window ({match.searchedCount} candidates searched).
            </p>
          )}
          {match?.success && match.matched && (
