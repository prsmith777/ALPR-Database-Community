  ...
  try {
    ...
    for (const plateData of plates) {
      ...
      const result = await dbClient.query(
        ...
        traffic_camera_observation AS (
          INSERT INTO public.camera_observations (
            source_key, camera_key, observation_kind,
            observation_timestamp, received_at, direction_label,
            blue_iris_timeline_path, plate_read_id, raw_payload,
            idempotency_key
          )
          SELECT 'plate-ingress', COALESCE(NULLIF(BTRIM($8::text), ''), 'unconfigured-camera'),
                 'lpr', $7, CURRENT_TIMESTAMP, $20,
                 $11, new_read.id,
                 jsonb_build_object('eventIdentity', $17::text),
                 repeat(md5('plate-read:' || new_read.id::text), 2)
          FROM new_read
